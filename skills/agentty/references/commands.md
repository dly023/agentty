...
Set inside every agentty pane, inherited by anything you launch from one.

| Variable | Meaning |
|---|---|
| `AGENTTY_PANE` | This pane's id, e.g. `71` or `%71` (both forms are accepted). The default target of `split`, `send`, `capture`, `procs`, `pane close`. |
| `AGENTTY_WS` | This pane's workspace id. The default for `run --keep`, `tab new`, `ws tree`. |
| `AGENTTY_CONFIG_DIR` | The server's config dir. How the CLI finds the right server's sockets — you never pass a socket path. |

Outside a agentty shell the address-taking verbs fail with
...
|---|---|
| 0 | success |
| 1 | the command failed; the reason is one line on stderr, prefixed `agentty:` |
...
| *other* | only from `agentty run`, which passes the child's exit code through |

Builds before this was fixed panic instead of exiting on a hung-up reader:
`agentty capture %71 | head -1` prints a Rust `failed printing to stdout: Broken
...
### `agentty run [--keep] [--cwd DIR] [--ws WORKSPACE] -- CMD...`
...
the child, so `agentty run -- cargo test --keep` passes `--keep` to cargo.

- `--keep` leaves the pane alive as a new tab afterwards. It needs a workspace,
  so it requires `--ws` or `$AGENTTY_WS`; without one it is an error, not a
  silent fallback.
- `--cwd` sets the working directory. `--ws` also sets the pane's `AGENTTY_WS`.
...
### `agentty split [%PANE] (--v|--h) [--ratio R]`
Alias of `pane split`. Splits `%PANE` (default `$AGENTTY_PANE`), spawning a shell
...
machine links exist. Adds a note when you are not inside a agentty shell.
...
one by name, by full id, or by a unique id prefix (the 8-char prefix `agentty ls`
...
Prefer `agentty new <path>` over `ws new` when you want something usable: `ws new`
leaves you with an empty workspace you then have to populate, while
`agentty new --json <path>` hands back `{"id","pane"}` — both addresses in one go.
...
`agentty-cli` for panes this CLI spawned (a workspace id otherwise), and
`orphan: true` means no workspace holds it. An interrupted `agentty run` and a
...
- bare `agentty <path>` (launch or focus the GUI) — not wired up
