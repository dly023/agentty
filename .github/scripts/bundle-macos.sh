...
TARGET="$1"
ARCH="$2"
bash script/check_package_target "$TARGET" "$ARCH" macos
...
APP="dist/macos-${ARCH}/Agentty.app"
STAGE="dist/dmg-stage-${ARCH}"
NOTARIZE_ZIP="dist/notarize-${ARCH}.zip"
# Drop only this architecture's old intermediate files and DMGs, before reading new binaries.
rm -rf "$APP" "$STAGE" "$NOTARIZE_ZIP" "dist/entitlements-${ARCH}.plist" dist/agentty-*-macos-"${ARCH}".dmg
...
SERVER_PROTOCOL="$("target/${TARGET}/release/tty7-server" --protocol)"
PLIST_CONTENT="$(printf '%s\n' "$SERVER_PROTOCOL" | ruby script/macos_bundle_metadata "$VERSION")"
...
# The CLI starts this headless sibling. GUI startup currently relaunches
# tty7-app --daemon; both executables use the shared tty7-core daemon.
cp "target/${TARGET}/release/tty7-server" "$APP/Contents/MacOS/tty7-server"
chmod +x "$APP/Contents/MacOS/tty7-server"
...
cp assets/tty7.icns "$APP/Contents/Resources/agentty.icns"
...
printf '%s\n' "$PLIST_CONTENT" > "$APP/Contents/Info.plist"
...
if [[ -n "$SIGN_ID" && -n "${APPLE_CERTIFICATE:-}" ]]; then
    ...
    ENTITLEMENTS="dist/entitlements-${ARCH}.plist"
...
ENT
    ...
    codesign --force --options runtime --timestamp \
        --sign "$SIGN_ID" "$APP/Contents/MacOS/tty7-server"
