...
rm -rf "$APPDIR" dist/agentty-*-linux-"${ARCH}".AppImage
...
cp "target/${TARGET}/release/tty7-server" "$APPDIR/usr/bin/tty7-server"
chmod +x "$APPDIR/usr/bin/tty7-server"
...
"$LINUXDEPLOY" \
  ...
  --executable "$APPDIR/usr/bin/tty7-server" \
