...
Reproducible throughput/FPS/memory benchmarks for agentty against Alacritty,
...
| **Memory** | cold launch, default shell, idle 6 s | RSS in KB (agentty = GUI + daemon) |
...
cargo build --release                   # the agentty under test

scripts/bench/run_one.sh agentty io        # ALWAYS first: records the reference grid
scripts/bench/run_one.sh alacritty io   # matches agentty's grid automatically
scripts/bench/run_one.sh ghostty io
scripts/bench/run_one.sh kitty io
scripts/bench/run_one.sh agentty fire      # same order for the fire test
...
`$AGENTTY_BENCH_DIR`, the agentty binary with `$AGENTTY_BIN`).
...
- **agentty**: an isolated `--config-dir` under the work dir whose `config.json`
  sets `shell` to the script. GUI + daemon are launched fresh and killed
  (by config-dir-scoped `pkill`) after the run — a daily-driver agentty and its
...
Grid fairness: agentty has no size flag, so its default window is the reference —
run agentty first, and the driver reads the recorded `grid:` line to size the
...
| Test | agentty (before) | agentty (after) | Alacritty | Ghostty | Kitty |
| --- | ---: | ---: | ---: | ---: | ---: |
...
"run 1 fast, later runs slow" (the ring fills during run 1). `AGENTTY_TRACE=1`
...
¹ The agentty-before numbers were measured while the machine was busy (builds +
...
- **The day's fps band matters more than the run.** The same pre-pass binary
  ...
  reproduced within ±3%. agentty's fire number is drain-rate-bound and therefore
  sensitive to ambient machine load in a way the (slower) competitors aren't.
  Only compare agentty-vs-agentty fire numbers from the same session.
- **DOOM-fire is producer-bound, not terminal-bound, at this level.** Under a
  ...
  60 MB/s → 690 fps). agentty's steady seconds already drain at 93–98 MB/s; the
