...
When you scroll above the live edge, **Back to bottom** appears inside that
pane. Click it to return to the latest output and focus the terminal without
sending input to the shell. It also cancels pending smooth scrolling, so an old
animation cannot pull you back up. The button disappears at the bottom and is
not shown in alternate-screen applications such as a full-screen editor.
