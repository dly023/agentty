...
The sidebar's workspace entries represent machines. Remaining workspace
selection names address internal tab-layout records, not directory groups.
Use the sidebar or the **+** environment menu for ordinary machine/session
navigation. The old NewWorkspace, RenameWorkspace, StopWorkspace and
DeleteWorkspace actions are no longer registered, including custom bindings.
...
| Hide Agentty · Hide Others · Minimize | <kbd>⌘ H</kbd> · <kbd>⌘ ⌥ H</kbd> · <kbd>⌘ M</kbd> | — |
...
| Internal layout selection | `SelectWorkspace1`…`SelectWorkspace9` |
...
Retired workspace shortcuts are not silently reassigned to **Disconnect** or
**Sleep**. Ordinary **Quit** does not stop the background server. The old
named-workspace create/rename forms and stop/delete menus have also been removed;
use machine controls for connections and tab controls for sessions.

In **Session History**, Up/Down selects a visible result, Enter requests Resume,
and Escape closes the manager. Those are context-local controls, not global
shortcuts. **Back to bottom** is a terminal button shown after scrolling up,
not a new default keyboard binding.
