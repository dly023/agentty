...
l10n_keys! {
    ...
    HistoryEmpty,
    SessionAgentExited,
    SessionTerminalStopped,
    SessionViewOutput,
    SessionOutputLoading,
    SessionOutputUnavailable,
    SessionSleepCurrent,
    SessionSleepTab,
    SessionSleepTabRequirement,
    SessionSleepPartial,
    SessionSleepWorking,
    SessionSleepAwaitingExit,
    SessionSleepUnknown,
    SessionResumeUnknown,
    ...
    HistoryMissing,
    HistoryOffline,
    HistoryTitleUnavailable,
    HistoryResume,
    TerminalJumpBottom,
    FileDropRemoteSource,
    FileDropForeignTerminal,
    HistoryRejoin,
    HistoryRejoinUnavailable,
    MachineSessionWindowOccupied,
    HistoryResumePending,
    HistoryDeletePlanned,
    HistoryForkPlanned,
    HistoryConvertPlanned,
    MachineExpand,
    MachineSavedConnections,
    TabMenuAddHost,
    TabMenuAllHosts,
    TabMenuSplitHint,
    TabUnnamedShell,
    ...
    RemoteWorkspaceAttachmentUnexpectedReply,
...
mod tests {
    #[test]
    fn product_branding_preserves_core_lineage() {
        use super::*;
        for language in SUPPORTED_LANGUAGES {
            for key in [
                ...
                let label = (language.translate_fn)(key).unwrap();
                assert!(label.contains("agentty"), "{}: {label}", language.code);
            }
            let cli = (language.translate_fn)(L10nKey::SettingsInstallCliOnPath).unwrap();
            assert!(cli.contains("tty7"));
            assert!(!cli.contains("agentty"));
            let server = (language.translate_fn)(L10nKey::PanelPortsUnsupported).unwrap();
            assert!(server.contains("tty7-server"));
        }
    }
    use super::*;
