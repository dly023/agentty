...
pub fn translate_zh(key: L10nKey) -> Option<&'static str> {
    Some(match key {
        ...
        L10nKey::QuitStopServerBody => {
            "这会退出 agentty 并停止后台 server，shell 里正在跑的东西都会被终止。标签页和布局会保留，下次启动时以全新的 shell 打开。（只关窗口的话应用收进托盘，shell 继续跑。）"
        }
        ...
        L10nKey::SettingsLanguageDesc => "选择 agentty 界面使用的语言。",
        ...
        L10nKey::ExplorerMenuOpenIn => "在 agentty 中打开",
        L10nKey::ExplorerMenuOpenHere => "在此处打开 agentty",
        ...
        L10nKey::SettingsInTty7 => "在 agentty 中",
        ...
        L10nKey::SettingsTypeAddressToConnect => "输入地址即可立刻连接，之后 agentty 会提示保存。",
        ...
        L10nKey::SettingsImportAliasesDesc => {
            "重新读取文件并添加新内容。你在这里做的编辑由 agentty 保存——不会写入该文件本身。"
        }
        ...
        L10nKey::SettingsImportIgnored => {
            "有 {count} 个选项在 agentty 中没有对应设置，仍留在文件里：{options}"
        }
        ...
        L10nKey::SettingsStartInDesc => {
            "新 shell 的启动目录：agentty 的启动目录、主目录或固定路径。"
        }
        ...
        L10nKey::SettingsPromptIntro => {
            "shell 提示符处的 agentty 自带编辑器与菜单。关闭某项即可把这部分交还给 shell。"
        }
        L10nKey::SettingsPromptEditor => "提示符编辑器",
        L10nKey::SettingsPromptEditorDesc => {
            "由 agentty 编辑你在 shell 提示符上敲的这一行：选择、撤销，以及下面这些菜单。关闭后交还给 shell 自己的行编辑器——ZLE、readline、fish。"
        }
        ...
        L10nKey::SettingsTabCompletionDesc => {
            "在提示符按 Tab 打开 agentty 的补全菜单。关闭后 Tab 交由 shell 自身的补全处理。"
        }
        L10nKey::SettingsHistorySearch => "历史搜索",
        L10nKey::SettingsHistorySearchDesc => {
            "在提示符按 ⌃R 打开 agentty 的模糊历史菜单。关闭后 ⌃R 交给 shell——它自带的反向搜索，或你绑定的其它功能（fzf、percol）。"
        }
        ...
        L10nKey::SettingsAgentsIntroDesc => {
            "hook 让跑这些 agent 的窗格在标签栏实时显示状态（进行中 / 等待中 / 已完成）。仅在 agentty 内生效。"
        }
        ...
        L10nKey::SettingsStartupWindowDesc => "agentty 启动时的窗口状态。",
        L10nKey::SettingsRememberWindowSize => "记住窗口大小与位置",
        L10nKey::SettingsRememberWindowSizeDesc => {
            "以 agentty 上次退出时窗口的大小和位置重新打开。关闭时以默认大小居中打开。"
        }
        ...
        L10nKey::SettingsDefaultTerminalDesc => {
            "将 agentty 设为 Unix 可执行文件、SSH 链接和 man 页面链接的 macOS 默认终端。agentty 仍可打开文件夹和脚本，但不会替换 Finder 的文件夹处理程序。自行指定终端的应用可能不会遵循此设置。"
        }
        L10nKey::SettingsDefaultTerminalSet => "设为默认终端",
        L10nKey::SettingsDefaultTerminalSetSuccess => {
            "agentty 已成为受支持终端文件和链接的默认处理程序。"
        }
        L10nKey::SettingsDefaultTerminalSetFailed => "无法将 agentty 设为默认终端：{error}",
        ...
        L10nKey::SettingsUpdateReadyNextLaunch => "下次启动 agentty 时会自动装上。",
        ...
        L10nKey::SettingsDaemonStaleDesc => {
            "agentty 是原地升级的：界面已是新版，pane 还由旧版 server 托管。重启 server 换成新版，代价是 pane 里正在跑的进程全部结束。不急，挑 pane 空闲时再重启。"
        }
        L10nKey::UpdateDialogTitle => "有可用更新",
        L10nKey::UpdateDialogDetail => {
            "agentty {version} 已发布，你现在是 {current}。安装会重启应用；后台 server 不动，pane 里的东西都还在。"
        }
        L10nKey::UpdateDialogDetailWindows => {
            "agentty {version} 已发布，你现在是 {current}。安装会重启应用和后台 server：pane 里的进程会被结束，标签页和布局以全新的 shell 恢复。"
        }
        L10nKey::UpdateDialogDetailManual => "agentty {version} 已发布，你现在是 {current}。{hint}",
        ...
        L10nKey::UpdateDialogNeedsElevation => {
            "agentty 是为所有用户安装的，安装前 Windows 会请求一次管理员批准。agentty 本身不会以管理员身份运行。"
        }
        ...
        L10nKey::SettingsUpdateUnsupportedMacos => {
            "当前副本不在可写的 agentty.app 包里，无法自我替换。请把 agentty 移到“应用程序”，或打开发布页面更新。"
        }
        ...
        L10nKey::SettingsUpdateWindowsAllUsers => {
            "agentty 是为所有用户安装的，替换需要管理员权限，而 agentty 不会自行提权。请打开发布页面，自行运行安装程序更新。"
        }
        ...
        L10nKey::SettingsUpdateMissingChecksums => {
            "该版本缺少 checksums.txt，因此 agentty 拒绝自动安装。"
        }
        ...
        L10nKey::SettingsCommandLineDesc => {
            "把自带的 tty7 命令加入 PATH，让脚本和 agent 能从任意终端驱动 agentty——在 agentty 窗格内两种情况都可用。自己构建的 agentty 不想被遮蔽就关掉。下次启动生效。"
        }
        ...
        L10nKey::SettingsAppHttpProxyDesc => {
            "仅用于 agentty 自身的更新检查和下载，不影响面板中运行的程序。留空则跟随系统代理。"
        }
        ...
        L10nKey::SettingsSearchCommandLineToolKeywords => {
            "命令行工具 cli agentty 路径 shell 命令 安装 符号链接 terminal command line tool"
        }
        ...
        L10nKey::DiffTruncatedBudget => {
            "内容未加载——已超出 agentty 的 diff 预算。在终端运行 git diff 查看此文件。"
        }
        ...
        L10nKey::DiffBudgetAndCap => "agentty 的预算和单文件上限",
        L10nKey::DiffBudget => "agentty 的预算",
        ...
        L10nKey::RemoteNoConnectionDetails => {
            "此窗口是 {machine} 上的工作区，但 agentty 没有它的连接信息了——检查其 SSH 配置或 ~/.ssh/config 条目是否还在。"
        }
        ...
        L10nKey::RemoteReplaceBody => {
            "agentty 会在 {machine} 上安装匹配的 server 并启动它。\n\
             \n\
        ...
        L10nKey::RemoteInstallDetail => {
            "agentty 会将其 server 二进制文件写入 {machine}，以便本机可以在那里托管\
        ...
        L10nKey::RemoteServerOutdated => {
            "{machine} 上的 tty7 server 太旧（{build}），当前这份 agentty 连不上它。\
             更新它才能连接。"
        }
        L10nKey::RemoteServerTooNew => {
            "{machine} 上的 tty7 server（{build}）比当前这份 agentty 还新。\
             请更新本机的 agentty，或把那边的 server 替换成匹配的版本。"
        }
        ...
        L10nKey::RemoteDaemonTooOld => {
            "本机的 tty7 守护进程版本较旧，无法重启 {machine} 上的 server。请退出 agentty（这会停止守护进程）再打开，然后重试。"
        }
        ...
        L10nKey::RemoteNoRouteToHost => "agentty 已无法到达 {machine}",
        ...
        L10nKey::CmdAboutTty7 => "关于 agentty",
        ...
        L10nKey::CmdQuitTty7 => "退出 agentty",
        ...
        L10nKey::ConfigQuarantinedStartup => {
            "config.json 无法解析。agentty 正以默认设置运行，原内容已保留为旁边的 config.json.corrupt。修好后会自动重载；在此之前，设置里的更改不会保存。"
        }
        L10nKey::ConfigQuarantinedReload => {
            "修改后的 config.json 无法解析。agentty 保留了当前在用的设置，文件内容另存为旁边的 config.json.corrupt。修好后会自动重载；在此之前保存设置会覆盖它。"
        }
        L10nKey::ConfigUnreadableStartup => {
            "config.json 读取失败。agentty 正以默认设置运行，文件原样保留。修好权限或内容后会自动重载；在此之前，设置里的更改不会保存。"
        }
        L10nKey::ConfigUnreadableReload => {
            "config.json 读取失败。agentty 保留了当前在用的设置，文件也原样保留。修好权限或内容后会自动重载；在此之前保存设置会覆盖它。"
        }
        ...
        L10nKey::AppForkNoCommand => "agentty 没有用于 {name} 的 fork 命令",
        L10nKey::AppForkLocalOnly => "{name} 会话只能从本地窗格 fork",
        L10nKey::AppForkNoSessionId => {
            "agentty 尚未在此窗格中看到 {name} 的会话 ID——请在设置 → Agents 中安装其 hook"
        }
        ...
        L10nKey::AppAgentHooksNoHomeDir => {
            "agentty 无法确定这台计算机的主目录，因此没有可安装的位置。"
        }
        ...
        L10nKey::SettingsShellDetected => "agentty 检测到的 shell",
        ...
        L10nKey::SettingsDaemonStaleDescInPlace => {
            "agentty 是原地更新的：应用是新的，面板还跑在旧版上。server 可以不停机就换成新版，shell 直接延续下来。用 agentty 内置 SSH 客户端的面板除外——那些连接会断开，需要重新打开。"
        }
        L10nKey::AppRestartServerBodyInPlace => {
            "后台 server 会原地把自己换成当前这个版本：shell 继续运行，窗口稍后自动连回去。用 agentty 内置 SSH 客户端的面板除外——那些连接会断开，需要重新打开。"
        }
        ...
        L10nKey::SettingsPerPaneHistoryDescription => {
            "上方向键翻的是这个面板里跑过的命令，而不是所有面板混在一起。新面板从已有历史开始，关闭时把新增的写回去。只对 agentty 能接管的 bash 和 zsh 面板生效；用你自己参数启动的 shell 不受影响。"
        }
        ...
        L10nKey::IntegrationNoticeNotEngaged => {
            "此窗格的 agentty shell 集成没生效，内联补全和 Ctrl+R 菜单不可用。常见原因：用自己参数启动的 shell、PTY 包装器，或不受支持的 shell。"
        }
        ...
        L10nKey::AppMenuDocumentation => "agentty 文档",
        ...
        L10nKey::TrayShowTty7 => "显示 agentty",
        ...
        L10nKey::TabMenuOtherTerminals => "其他终端",
        L10nKey::MachineConnected => "已连接",
        L10nKey::MachinePin => "固定机器",
        L10nKey::MachineUnpin => "取消固定",
        L10nKey::MachineCollapse => "收起会话",
        L10nKey::MachineExpand => "展开会话",
        L10nKey::HistoryManagerTitle => "历史会话管理",
        L10nKey::HistoryRefresh => "刷新",
        L10nKey::HistoryClose => "关闭",
        L10nKey::HistoryLoading => "正在扫描目标环境…",
        L10nKey::HistoryEmpty => "未发现可恢复会话。",
        L10nKey::SessionAgentExited => "已退出",
        L10nKey::SessionTerminalStopped => "终端已停止",
        L10nKey::SessionViewOutput => "查看输出",
        L10nKey::SessionOutputLoading => "正在读取停止会话的输出…",
        L10nKey::SessionOutputUnavailable => "暂时无法读取保存的输出，仍可恢复此会话。",
        L10nKey::SessionSleepCurrent => "休眠当前会话",
        L10nKey::SessionSleepTab => "休眠整个标签",
        L10nKey::SessionSleepTabRequirement => {
            "需要确认每个窗格的恢复身份，且至少有一个正在运行。普通 shell 或身份未知的窗格不会被跳过或结束。"
        }
        L10nKey::SessionSleepPartial => {
            "已请求停止 {accepted} 个；{stopped} 个已停止。请刷新其余状态。"
        }
        L10nKey::SessionSleepWorking => "正在请求停止会话…",
        L10nKey::SessionSleepAwaitingExit => "已请求停止，等待终端退出",
        L10nKey::SessionSleepUnknown => "停止结果未知，请先刷新目标状态再重试。",
        L10nKey::SessionResumeUnknown => "尚未确认恢复结果，请刷新目标状态。不会自动重新启动会话。",
        L10nKey::SessionResumeUnavailable => "会话尚未恢复。请确认目标连接和会话文件后重试。",
        L10nKey::HistorySearch => "搜索会话标题、目录、Agent 或 ID…",
        L10nKey::HistorySelect => "选择会话查看详情。",
        L10nKey::HistoryLimited => "仅显示有数量上限的近期历史。",
        L10nKey::HistoryMissing => "部分 Agent 的历史存储不可用。",
        L10nKey::HistoryTitleUnavailable => "暂时无法实时更新标题，已保留原有标题。可刷新重试。",
        L10nKey::HistoryOffline => "机器离线或连接已更换，请连接后刷新。",
        L10nKey::HistoryResume => "恢复会话",
        L10nKey::TerminalJumpBottom => "回到底部",
        L10nKey::FileDropRemoteSource => "暂不支持从远端文件树复制，请拖入本地桌面文件。",
        L10nKey::FileDropForeignTerminal => "该路径属于另一台机器，请拖入对应机器的终端。",
        L10nKey::HistoryRejoin => "接回运行中会话",
        L10nKey::MachineSessionWindowOccupied => {
            "此会话正由其他窗口使用，请先在那里释放，再在当前窗口打开。"
        }
        L10nKey::HistoryRejoinUnavailable => "当前窗口没有此会话唯一对应的运行中终端。",
        L10nKey::HistoryResumePending => "未能打开会话。请先激活此机器并等待布局加载完成，再重试。",
        L10nKey::HistoryDeletePlanned => "删除会话文件 · 规划中",
        L10nKey::HistoryForkPlanned => "Fork 新会话 · 规划中",
        L10nKey::HistoryConvertPlanned => "转换 Agent · 规划中",
        L10nKey::MachineSavedConnections => "已保存的连接",
...
pub fn translate_variant_zh(key: L10nKey, branch: &'static str) -> Option<&'static str> {
    let res = match (key, branch) {
        ...
        (L10nKey::SettingsImportIgnored, "zero") => "文件里的每个选项在 agentty 中都有对应设置。",
        (L10nKey::SettingsImportIgnored, "one") => {
            "有 1 个选项在 agentty 中没有对应设置，仍留在文件里：{options}"
        }
