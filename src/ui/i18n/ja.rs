...
pub fn translate_ja(key: L10nKey) -> Option<&'static str> {
    Some(match key {
        ...
        L10nKey::Close => "閉じる",
        L10nKey::QuitStopServerTitle => "agentty を終了してサーバーを停止しますか？",
        L10nKey::QuitStopServerBody => {
            "agentty を終了してバックグラウンドサーバーを停止します。シェルで実行中のものはすべて終了します。タブとレイアウトは次回起動時に新しいシェルで開きます。（ウィンドウを閉じるだけならトレイに退避し、シェルは動き続けます）"
        }
        ...
        L10nKey::SettingsLanguageDesc => "agentty の表示言語を選択します",
        ...
        L10nKey::ExplorerMenuOpenIn => "agentty で開く",
        L10nKey::ExplorerMenuOpenHere => "ここで agentty を開く",
        L10nKey::SettingsCustomThemesIntro => {
            "テーマを複製して色を編集するか、agentty の YAML テーマや iTerm2 の .itermcolors をテーマフォルダに置いてください"
        }
        ...
        L10nKey::SettingsInTty7 => "agentty 内",
        ...
        L10nKey::SettingsTypeAddressToConnect => {
            "アドレスを入力するとすぐに接続できます。agentty はあとで保存するか尋ねます"
        }
        ...
        L10nKey::SettingsImportAliasesDesc => {
            "ファイルを再読み込みして新しい項目を追加します。ここでの編集は agentty が保存します — ファイル自体には書き込まれません"
        }
        ...
        L10nKey::SettingsImportIgnored => {
            "agentty に設定のないオプションが {count} 件あり、ファイルに残されています: {options}"
        }
        ...
        L10nKey::SettingsStartInDesc => {
            "新しいシェルの開始場所: agentty の起動ディレクトリ、ホームフォルダ、または固定パス"
        }
        ...
        L10nKey::SettingsPromptIntro => {
            "シェルプロンプトでの agentty 独自のエディターとメニュー。オフにするとその分がシェルに渡されます"
        }
        L10nKey::SettingsPromptEditor => "プロンプトエディター",
        L10nKey::SettingsPromptEditorDesc => {
            "シェルプロンプトで入力する行を agentty が編集します — 選択、取り消し、下のメニュー。オフにするとシェル自身の行エディター（ZLE、readline、fish）に戻ります"
        }
        ...
        L10nKey::SettingsTabCompletionDesc => {
            "プロンプトで Tab を押すと agentty の補完メニューが開きます。オフの場合、Tab はシェル自身の補完に渡されます"
        }
        L10nKey::SettingsHistorySearch => "履歴検索",
        L10nKey::SettingsHistorySearchDesc => {
            "プロンプトで ⌃R を押すと agentty のファジー履歴メニューが開きます。オフなら ⌃R はシェルへ — 逆方向検索や、そこでバインドしたもの（fzf、percol）"
        }
        ...
        L10nKey::SettingsAgentsIntroDesc => {
            "フックにより、これらのエージェントを実行するペインの状態（作業中 / 待機中 / 完了）がタブバーに表示されます。agentty 内でのみ有効"
        }
        ...
        L10nKey::SettingsStartupWindowDesc => "agentty 起動時のウィンドウ状態",
        L10nKey::SettingsRememberWindowSize => "ウィンドウサイズと位置を記憶",
        L10nKey::SettingsRememberWindowSizeDesc => {
            "agentty が最後に終了したときのサイズと位置で開き直します。オフならデフォルトサイズで中央に開きます"
        }
        ...
        L10nKey::SettingsDefaultTerminalDesc => {
            "agentty を Unix 実行ファイル、SSH リンク、man ページリンク用の macOS のデフォルトターミナルにします。agentty はフォルダとスクリプトも開けますが、Finder のフォルダハンドラは置き換えません。独自のターミナルを指定するアプリはこの設定を無視することがあります。"
        }
        L10nKey::SettingsDefaultTerminalSet => "デフォルトのターミナルに設定",
        L10nKey::SettingsDefaultTerminalSetSuccess => {
            "agentty を対応するターミナルファイルとリンクのデフォルトハンドラに設定しました。"
        }
        L10nKey::SettingsDefaultTerminalSetFailed => {
            "agentty をデフォルトのターミナルに設定できませんでした: {error}"
        }
        ...
        L10nKey::SettingsUpdateReadyNextLaunch => "次回 agentty を起動したときに適用されます。",
        ...
        L10nKey::SettingsDaemonStaleDesc => {
            "agentty はその場で更新されました。アプリは新しく、ペインはまだ以前のビルドのサーバーが処理しています。再起動すると新しいビルドに切り替わり、ペインで動いているプロセスはすべて終了します。急ぐ必要はなく、ペインが空いているときにどうぞ"
        }
        L10nKey::UpdateDialogTitle => "アップデートがあります",
        L10nKey::UpdateDialogDetail => {
            "agentty {version} が利用できます（現在 {current}）。インストールするとアプリが再起動します。バックグラウンドサーバーは動いたままなので、ペインの中身は残ります"
        }
        L10nKey::UpdateDialogDetailWindows => {
            "agentty {version} が利用できます（現在 {current}）。インストールするとアプリとバックグラウンドサービスが再起動します。ペインのプロセスは終了し、タブとレイアウトは新しいシェルで復元されます"
        }
        L10nKey::UpdateDialogDetailManual => {
            "agentty {version} が利用できます（現在 {current}）。{hint}"
        }
        ...
        L10nKey::UpdateDialogNeedsElevation => {
            "agentty は全ユーザー向けにインストールされているため、インストール前に Windows の管理者承認が一度求められます。agentty 自体が管理者権限で実行されることはありません"
        }
        ...
        L10nKey::SettingsUpdateUnsupportedMacos => {
            "この agentty は書き込み可能な agentty.app バンドルにないため、自分自身を置き換えられません。「アプリケーション」へ移動するか、リリースページから更新してください"
        }
        ...
        L10nKey::SettingsUpdateUnsupportedWindows => {
            "この agentty は認識可能な Inno Setup 版でもポータブル ZIP 版でもないため、自動更新できません。リリースページを開いて手動で更新してください"
        }
        L10nKey::SettingsUpdateWindowsAllUsers => {
            "agentty はすべてのユーザー向けにインストールされており、置き換えには管理者権限が必要ですが、agentty は自ら昇格を要求しません。リリースページからインストーラーを実行して更新してください"
        }
        ...
        L10nKey::SettingsUpdateMissingChecksums => {
            "このリリースには checksums.txt がないため、agentty は自動インストールを行いません"
        }
        ...
        L10nKey::SettingsCommandLineDesc => {
            "同梱の tty7 コマンドを PATH に入れ、スクリプトやエージェントが任意のターミナルから agentty を操作できるようにします（ペイン内ではどちらでも動きます）。自分でビルドした agentty を優先したい場合はオフに。次回起動時に有効"
        }
        ...
        L10nKey::SettingsAppHttpProxyDesc => {
            "agentty 自身の更新チェックとダウンロードにのみ使用し、ペインで実行中のプログラムには影響しません。空欄ならシステムのプロキシに従います"
        }
        ...
        L10nKey::SettingsSearchCommandLineToolKeywords => {
            "cli agentty パス シェル コマンド インストール シンボリックリンク ターミナル iterm エージェント スクリプト command line tool"
        }
        ...
        L10nKey::DiffTruncatedBudget => {
            "内容は読み込まれていません — agentty の Diff 予算を超えています。ターミナルで `git diff` を実行してください"
        }
        ...
        L10nKey::DiffBudgetAndCap => "agentty の予算とファイルごとの上限",
        L10nKey::DiffBudget => "agentty の予算",
        ...
        L10nKey::RemoteNoConnectionDetails => {
            "このウィンドウは {machine} 上のワークスペースですが、agentty に接続情報がありません。SSH プロファイルか ~/.ssh/config の項目が残っているか確認してください"
        }
        ...
        L10nKey::RemoteServerOutdated => {
            "{machine} の tty7 サーバーが古く（{build}）、この agentty からは通信できません。更新すると接続できます"
        }
        L10nKey::RemoteServerTooNew => {
            "{machine} の tty7 サーバー（{build}）は、この agentty より新しいバージョンです。このコンピューターの agentty を更新するか、向こうのサーバーを対応するものに置き換えてください"
        }
        ...
        L10nKey::RemoteDaemonTooOld => {
            "このマシンのデーモンは古いビルドのため、{machine} 上のサーバーを再起動できません。agentty を終了（デーモンも停止します）して開き直し、再試行してください"
        }
        ...
        L10nKey::RemoteNoRouteToHost => "agentty は {machine} に到達する手段を失いました",
        L10nKey::RemoteWorkspaceAttachmentUnexpectedReply => {
            "サーバーが要求されたワークスペースの接続所有権を確認しませんでした。"
        }
        ...
        L10nKey::CmdAboutTty7 => "agentty について",
        ...
        L10nKey::CmdQuitTty7 => "agentty を終了",
        ...
        L10nKey::AppForkNoCommand => "agentty には {name} 用のフォークコマンドがありません",
        L10nKey::AppForkLocalOnly => {
            "{name} のセッションはローカルペインからしかフォークできません"
        }
        L10nKey::AppForkNoSessionId => {
            "agentty はこのペインで {name} のセッション ID を確認できていません — 設定 → エージェントでフックをインストールしてください"
        }
        ...
        L10nKey::AppAgentHooksNoHomeDir => {
            "agentty はこのコンピュータのホームディレクトリを特定できなかったため、インストール先がありません"
        }
        ...
        L10nKey::SettingsShellDetected => "agentty が見つけたシェル",
        ...
        L10nKey::SettingsDaemonStaleDescInPlace => {
            "agentty はその場で更新されました。アプリは新しく、ペインはまだ前のビルドで動いています。サーバーは停止せずに新しいビルドへ置き換えられるので、シェルはそのまま引き継がれます。agentty 内蔵の SSH クライアントを使うペインだけは例外で、その接続は閉じられ、開き直しが必要です"
        }
        L10nKey::AppRestartServerBodyInPlace => {
            "サーバーは停止せずに自分自身をこのビルドへ置き換えます。シェルは動いたままで、ウィンドウはすぐに再接続します。agentty 内蔵の SSH クライアントを使うペインだけは例外で、その接続は閉じられ、開き直しが必要です"
        }
        ...
        L10nKey::SettingsPerPaneHistoryDescription => {
            "上キーでたどるのは、全ペインが混ざったものではなくこのペインで実行したコマンドです。新しいペインは既存の履歴から始まり、追加分は閉じるときに書き戻されます。対象は agentty が設定できる bash と zsh のペインで、独自の引数で起動したシェルはそのままです"
        }
        ...
        L10nKey::IntegrationNoticeNotEngaged => {
            "このペインでは agentty シェル統合が有効になっておらず、インライン補完と Ctrl+R メニューは使えません。よくある原因は、独自の引数で起動したシェル、PTY ラッパー、未対応のシェルです。"
        }
        ...
        L10nKey::AppMenuDocumentation => "agentty ドキュメント",
        ...
        L10nKey::TrayShowTty7 => "agentty を表示",
        ...
        L10nKey::TabMenuOtherTerminals => "その他のターミナル",
        L10nKey::MachineConnected => "接続済み",
        L10nKey::MachinePin => "マシンを固定",
        L10nKey::MachineUnpin => "固定を解除",
        L10nKey::MachineCollapse => "セッションを折りたたむ",
        L10nKey::MachineExpand => "セッションを展開",
        L10nKey::HistoryManagerTitle => "セッション履歴管理",
        L10nKey::HistoryRefresh => "更新",
        L10nKey::HistoryClose => "閉じる",
        L10nKey::HistoryLoading => "接続先を検索中…",
        L10nKey::HistoryEmpty => "再開可能なセッションがありません。",
        L10nKey::SessionAgentExited => "終了済み",
        L10nKey::SessionTerminalStopped => "端末停止",
        L10nKey::SessionViewOutput => "出力を表示",
        L10nKey::SessionOutputLoading => "停止したセッションの出力を読み込み中…",
        L10nKey::SessionOutputUnavailable => {
            "保存された出力を読み込めません。セッションの再開は可能です。"
        }
        L10nKey::SessionSleepCurrent => "現在のセッションを休止",
        L10nKey::SessionSleepTab => "タブ全体を休止",
        L10nKey::SessionSleepTabRequirement => {
            "全ペインの再開用 ID と、実行中のペインが必要です。通常のシェルや状態不明のペインを無視したり停止したりしません。"
        }
        L10nKey::SessionSleepPartial => {
            "{accepted} 件の停止を要求、{stopped} 件は停止済みです。残りの状態を更新してください。"
        }
        L10nKey::SessionSleepWorking => "セッションの停止を要求中…",
        L10nKey::SessionSleepAwaitingExit => "停止を要求しました。端末の終了を待っています",
        L10nKey::SessionSleepUnknown => {
            "停止結果を確認できません。再試行の前に接続先の状態を更新してください。"
        }
        L10nKey::SessionResumeUnknown => {
            "再開結果を確認できません。対象の状態を更新してください。自動的には再起動しません。"
        }
        L10nKey::SessionResumeUnavailable => {
            "セッションは再開されていません。接続とセッションファイルを確認して再試行してください。"
        }
        L10nKey::HistorySearch => "タイトル、ディレクトリ、エージェント、ID を検索…",
        L10nKey::HistorySelect => "セッションを選択してください。",
        L10nKey::HistoryLimited => "件数を制限した最近の履歴です。",
        L10nKey::HistoryMissing => "一部の Agent の履歴が利用できません。",
        L10nKey::HistoryTitleUnavailable => {
            "タイトルの自動更新が利用できません。現在のタイトルは保持されています。更新して再試行してください。"
        }
        L10nKey::HistoryOffline => {
            "マシンがオフラインか接続が変更されました。再接続後に更新してください。"
        }
        L10nKey::HistoryResume => "再開",
        L10nKey::TerminalJumpBottom => "最下部へ",
        L10nKey::FileDropRemoteSource => {
            "リモートツリーからのコピーは未対応です。ローカルファイルをドロップしてください。"
        }
        L10nKey::FileDropForeignTerminal => {
            "別のマシンのパスです。そのマシンの端末にドロップしてください。"
        }
        L10nKey::HistoryRejoin => "実行中のセッションに再接続",
        L10nKey::MachineSessionWindowOccupied => {
            "このセッションは別のウィンドウで使用中です。そちらで解放してから、ここで開いてください。"
        }
        L10nKey::HistoryRejoinUnavailable => {
            "現在のウィンドウに、このセッションに一意に対応する実行中の端末がありません。"
        }
        L10nKey::HistoryResumePending => {
            "セッションを開けませんでした。このマシンを選択し、レイアウトの読み込み後に再試行してください。"
        }
        L10nKey::HistoryDeletePlanned => "履歴ファイルの削除 · 計画中",
        L10nKey::HistoryForkPlanned => "新規セッションへ Fork · 計画中",
        L10nKey::HistoryConvertPlanned => "Agent 変換 · 計画中",
        L10nKey::MachineSavedConnections => "保存済みの接続",
...
pub fn translate_variant_ja(key: L10nKey, branch: &'static str) -> Option<&'static str> {
    let res = match (key, branch) {
        ...
        (L10nKey::SettingsImportIgnored, "zero") => {
            "ファイル内のすべてのオプションに agentty 側の設定があります"
        }
        (L10nKey::SettingsImportIgnored, "one") => {
            "agentty に設定のないオプションが 1 件あり、ファイルに残されています: {options}"
        }
...
mod tests {
    ...
    fn japanese_covers_every_key() {
        ...
        assert!(translate_variant_ja(L10nKey::HomeTimeMinutesAgo, "other").is_some());
    }
}
