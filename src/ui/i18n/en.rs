...
pub fn translate_en(key: L10nKey) -> &'static str {
    match key {
        ...
        L10nKey::QuitStopServerBody => {
            "This quits agentty and stops the background server; anything running in your shells is terminated. Your tabs and layout reopen with fresh shells next launch. (Closing the window only retires agentty to the tray — the shells keep running.)"
        }
        ...
        L10nKey::SettingsLanguageDesc => "Choose the language used for the agentty interface.",
        ...
        L10nKey::ExplorerMenuOpenIn => "Open in agentty",
        L10nKey::ExplorerMenuOpenHere => "Open agentty here",
        ...
        L10nKey::SettingsInTty7 => "In agentty",
        ...
        L10nKey::SettingsImportAliasesDesc => {
            "Re-reads the file and adds anything new. Edits you make here are stored by agentty — the file itself is never written."
        }
        ...
        L10nKey::SettingsImportIgnored => {
            "{count} options have no setting in agentty and were left in the file: {options}"
        }
        ...
        L10nKey::SettingsPromptIntro => {
            "agentty's own editor and menus at the shell prompt. Turn one off to hand that much back to the shell."
        }
        L10nKey::SettingsPromptEditor => "Prompt editor",
        L10nKey::SettingsPromptEditorDesc => {
            "agentty edits the line you type at the shell prompt: selection, undo, and the menus below. Off hands the prompt back to the shell's own editor — ZLE, readline, fish."
        }
        ...
        L10nKey::SettingsTabCompletionDesc => {
            "Tab at the prompt opens agentty's completion menu. When off, Tab goes to the shell's own completion instead."
        }
        L10nKey::SettingsHistorySearch => "History search",
        L10nKey::SettingsHistorySearchDesc => {
            "⌃R at the prompt opens agentty's fuzzy history menu. Off sends ⌃R to the shell — its own reverse-i-search, or whatever you bound there (fzf, percol)."
        }
        ...
        L10nKey::SettingsAgentsIntroDesc => {
            "Hooks give panes running these agents live status (working / waiting / done) in the tab bar. Only inside agentty."
        }
        ...
        L10nKey::SettingsStartupWindowDesc => "Window state when agentty launches.",
        ...
        L10nKey::SettingsDefaultTerminalDesc => {
            "Make agentty the macOS default terminal for Unix executables, SSH links, and man-page links. agentty can also open folders and scripts, but does not replace Finder's folder handler. Apps that choose their own terminal may ignore this setting."
        }
        L10nKey::SettingsDefaultTerminalSet => "Set as Default Terminal",
        L10nKey::SettingsDefaultTerminalSetSuccess => {
            "agentty is now the default handler for supported terminal files and links."
        }
        L10nKey::SettingsDefaultTerminalSetFailed => {
            "Could not set agentty as the default terminal: {error}"
        }
        ...
        L10nKey::SettingsUpdateReadyNextLaunch => {
            "It will be installed the next time you start agentty."
        }
        ...
        L10nKey::SettingsDaemonStaleDesc => {
            "agentty was updated in place: the app is new, your panes are still served by the old build. Restarting the server picks up the new one and ends everything running in your panes. No hurry — do it when they're idle."
        }
        L10nKey::UpdateDialogTitle => "Update available",
        L10nKey::UpdateDialogDetail => {
            "agentty {version} is available — you're on {current}. Installing restarts the app; the background server keeps running, so your panes survive."
        }
        L10nKey::UpdateDialogDetailWindows => {
            "agentty {version} is available — you're on {current}. Installing restarts the app and the background service: processes in your panes are ended, and your tabs and layout come back with fresh shells."
        }
        L10nKey::UpdateDialogDetailManual => {
            "agentty {version} is available — you're on {current}. {hint}"
        }
        ...
        L10nKey::UpdateDialogNeedsElevation => {
            "agentty is installed for all users, so Windows asks for administrator approval once before installing. agentty itself never runs elevated."
        }
        ...
        L10nKey::SettingsUpdateUnsupportedMacos => {
            "This copy is not in a writable agentty.app bundle, so it cannot replace itself. Move agentty to Applications, or open the release page to update."
        }
        ...
        L10nKey::SettingsUpdateWindowsAllUsers => {
            "agentty is installed for all users, so replacing it needs administrator rights that agentty will not ask for itself. Open the release page and run the installer to update."
        }
        ...
        L10nKey::SettingsUpdateMissingChecksums => {
            "The release has no checksums.txt, so agentty refuses to install it automatically."
        }
        ...
        L10nKey::SettingsCommandLineDesc => {
            "Put the bundled tty7 command on your PATH, so scripts and agents can drive agentty from any terminal — inside a pane it works either way. Turn off to keep your own build unshadowed. Applies at next launch."
        }
        ...
        L10nKey::SettingsAppHttpProxyDesc => {
            "Used only for agentty's update checks and downloads, not for programs in your panes. Empty follows the system proxy."
        }
        ...
        L10nKey::SettingsSearchCommandLineToolKeywords => {
            "command line tool cli agentty path shell command install symlink terminal iterm agent script"
        }
        ...
        L10nKey::DiffTruncatedBudget => {
            "Body not loaded — past agentty's diff budget. Run git diff in the terminal for this file."
        }
        ...
        L10nKey::DiffBudgetAndCap => "agentty's budget and the per-file cap",
        L10nKey::DiffBudget => "agentty's budget",
        ...
        L10nKey::RemoteNoConnectionDetails => {
            "This window is a workspace on {machine}, but agentty has no connection details for it — check its SSH profile or ~/.ssh/config entry still exists."
        }
        ...
        L10nKey::RemoteReplaceBody => {
            "agentty will install a matching server on {machine} and start it.\n\
             \n\
        ...
        L10nKey::RemoteServerOutdated => {
            "{machine} is running an old tty7 server ({build}) that this copy of agentty \
             cannot talk to. Update it to connect."
        }
        L10nKey::RemoteServerTooNew => {
            "{machine} is running a newer tty7 server ({build}) than this copy of agentty. \
             Update agentty on this computer, or replace the server there with a matching one."
        }
        ...
        L10nKey::RemoteDaemonTooOld => {
            "this machine's daemon is an older build and cannot restart the server on {machine}. Quit agentty (that stops the daemon), open it again, and retry."
        }
        ...
        L10nKey::RemoteNoRouteToHost => "agentty no longer has a way to reach {machine}",
        L10nKey::RemoteMachineTreeUnexpectedReply => {
            "the server answered a machine tree with {reply}"
        }
        L10nKey::RemoteWorkspaceAttachmentUnexpectedReply => {
            "The server did not confirm ownership of the requested workspace."
        }
        ...
        L10nKey::CmdAboutTty7 => "About agentty",
        ...
        L10nKey::CmdQuitTty7 => "Quit agentty",
        ...
        L10nKey::ConfigQuarantinedStartup => {
            "config.json could not be parsed. agentty is on default settings and kept the file's contents beside it as config.json.corrupt. Fix it and agentty reloads; until then, Settings changes are not saved."
        }
        L10nKey::ConfigQuarantinedReload => {
            "The edited config.json could not be parsed. agentty kept the settings it is running on and set the file's contents aside as config.json.corrupt. Fix it and agentty reloads; saving a setting first overwrites it."
        }
        L10nKey::ConfigUnreadableStartup => {
            "config.json could not be read. agentty is on default settings and left the file exactly as it is. Fix its permissions or contents and agentty reloads; until then, Settings changes are not saved."
        }
        L10nKey::ConfigUnreadableReload => {
            "config.json could not be read. agentty kept the settings it is running on and left the file exactly as it is. Fix its permissions or contents and agentty reloads; saving a setting first overwrites it."
        }
        ...
        L10nKey::AppForkNoCommand => "agentty has no fork command for {name}",
        L10nKey::AppForkLocalOnly => "{name} sessions can only be forked from a local pane",
        L10nKey::AppForkNoSessionId => {
            "agentty hasn't seen a {name} session id in this pane — install its hooks in Settings → Agents"
        }
        ...
        L10nKey::AppAgentHooksNoHomeDir => {
            "agentty could not work out this computer's home directory, so there is nowhere to install to."
        }
        ...
        L10nKey::SettingsShellDetected => "Shells agentty found",
        ...
        L10nKey::SettingsDaemonStaleDescInPlace => {
            "agentty was updated in place: the app is new, your panes still run on the old build. The server can swap itself for the new one without stopping, so your shells carry straight over. Panes on agentty's built-in SSH client are the exception — those close and need reopening."
        }
        L10nKey::AppRestartServerBodyInPlace => {
            "The server swaps itself for this build in place: your shells keep running, and the window reconnects a moment later. Panes on agentty's built-in SSH client are the exception — those close and need reopening."
        }
        ...
        L10nKey::SettingsPerPaneHistoryDescription => {
            "Up walks through what you ran in this pane, not every pane interleaved. A new pane starts from your existing history and writes back what it adds when it closes. Applies to bash and zsh panes agentty can set up; a shell started with your own arguments is left alone."
        }
        ...
        L10nKey::IntegrationNoticeNotEngaged => {
            "agentty shell integration hasn't engaged in this pane, so inline completion and the Ctrl+R menu are unavailable. Usual causes: a shell you started with your own arguments, a PTY wrapper, or an unsupported shell."
        }
        ...
        L10nKey::TrayTooltipAgents => "agentty — {parts}",
        ...
        L10nKey::AppMenuDocumentation => "agentty Documentation",
        ...
        L10nKey::TrayShowTty7 => "Show agentty",
        ...
        L10nKey::TabMenuOtherTerminals => "Other Terminals",
        L10nKey::MachineConnected => "Connected",
        L10nKey::MachinePin => "Pin Machine",
        L10nKey::MachineUnpin => "Unpin Machine",
        L10nKey::MachineCollapse => "Collapse Sessions",
        L10nKey::MachineExpand => "Expand Sessions",
        L10nKey::HistoryManagerTitle => "Session History",
        L10nKey::HistoryRefresh => "Refresh",
        L10nKey::HistoryClose => "Close",
        L10nKey::HistoryLoading => "Scanning target…",
        L10nKey::HistoryEmpty => "No resumable sessions found.",
        L10nKey::SessionAgentExited => "Exited",
        L10nKey::SessionTerminalStopped => "Terminal stopped",
        L10nKey::SessionViewOutput => "View output",
        L10nKey::SessionOutputLoading => "Loading stopped session output…",
        L10nKey::SessionOutputUnavailable => {
            "Saved output is unavailable. You can still resume this session."
        }
        L10nKey::SessionSleepCurrent => "Sleep current session",
        L10nKey::SessionSleepTab => "Sleep entire tab",
        L10nKey::SessionSleepTabRequirement => {
            "Every pane needs a confirmed recovery identity, with at least one running. Plain shells and unknown panes are neither skipped nor stopped."
        }
        L10nKey::SessionSleepPartial => {
            "Stop requested for {accepted}; {stopped} already stopped. Refresh to check the remaining state."
        }
        L10nKey::SessionSleepWorking => "Requesting session stop…",
        L10nKey::SessionSleepAwaitingExit => "Stop requested; waiting for terminal exit",
        L10nKey::SessionSleepUnknown => {
            "Stop outcome unknown. Refresh the target before trying again."
        }
        L10nKey::SessionResumeUnknown => {
            "Resume outcome is unconfirmed. Refresh the target state; the session will not be restarted automatically."
        }
        L10nKey::SessionResumeUnavailable => {
            "The session has not resumed. Check the target connection and session files before retrying."
        }
        L10nKey::HistorySearch => "Search session title, directory, agent or ID…",
        L10nKey::HistorySelect => "Select a session to view details.",
        L10nKey::HistoryLimited => "Showing a bounded recent history window.",
        L10nKey::HistoryMissing => "Some provider history stores are unavailable.",
        L10nKey::HistoryTitleUnavailable => {
            "Live title updates are unavailable. Existing titles are kept; refresh to retry."
        }
        L10nKey::HistoryOffline => {
            "Machine is offline or its connection changed. Reconnect and refresh."
        }
        L10nKey::HistoryResume => "Resume",
        L10nKey::TerminalJumpBottom => "Back to bottom",
        L10nKey::FileDropRemoteSource => {
            "Copying from a remote tree is not supported. Drop a local desktop file instead."
        }
        L10nKey::FileDropForeignTerminal => {
            "This path belongs to another machine. Drop it into a terminal on that machine."
        }
        L10nKey::HistoryRejoin => "Rejoin running session",
        L10nKey::MachineSessionWindowOccupied => {
            "This session is in use in another window. Release it there before opening it here."
        }
        L10nKey::HistoryRejoinUnavailable => {
            "No unique live pane for this session in the current window."
        }
        L10nKey::HistoryResumePending => {
            "Could not open the session. Activate this machine and wait for its layout to load, then retry."
        }
        L10nKey::HistoryDeletePlanned => "Delete session files · Planned",
        L10nKey::HistoryForkPlanned => "Fork new session · Planned",
        L10nKey::HistoryConvertPlanned => "Convert agent · Planned",
        L10nKey::MachineSavedConnections => "Saved Connections",
...
pub fn translate_variant_en(key: L10nKey, branch: &'static str) -> Option<&'static str> {
    let res = match (key, branch) {
        ...
        (L10nKey::SettingsImportIgnored, "zero") => {
            "Every option in the file has a agentty setting."
        }
        (L10nKey::SettingsImportIgnored, "one") => {
            "1 option has no setting in agentty and was left in the file: {options}"
        }
