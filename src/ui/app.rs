...
impl Tab {
    ...
    pub(crate) fn label_view(
        ...
        let view = tty7_core::core::tab_view::TabView {
            ...
            provider_title: crate::ui::machine_mirror::MachineMirrors::machine(cx, leaf.host_id())
                .and_then(|machine| machine.panes.iter().find(|p| p.id == leaf.pane_id))
                .filter(|pane| match pane.agent.as_ref() {
                    Some(facts) => {
                        Some(facts.agent) == leaf.agent()
                            && facts.session_id == leaf.agent_session().and_then(|s| s.session_id)
                    }
                    None => leaf.agent().is_none() && leaf.agent_session().is_none(),
                })
                .and_then(|pane| pane.provider_title_text().map(str::to_string)),
...
pub struct Tty7App {
    ...
    pub(super) workspace_view_epoch: std::cell::Cell<uuid::Uuid>,
    pub(super) sleep_ui: Option<super::session_recovery::SleepUi>,
    pub(super) workspace_use: std::cell::RefCell<
        Option<(
            std::sync::Arc<tty7_core::daemon::control::ControlClient>,
            tty7_core::daemon::control::WorkspaceUse,
        )>,
    >,
    ...
    pub(crate) history_manager: Option<crate::ui::history_manager::HistoryManager>,
...
impl Tty7App {
    ...
    fn with_session_at(
        ...
        cx.on_release(|app, cx| app.retire_workspace_use(cx))
            .detach();
        ...
        let (tabs, active) = match session {
            ...
            some => {
                let (tabs, active, dropped) = tabs_from_session(
                    ...
                    some,
                    font_size,
                    crate::terminal::RestorePolicy::ReplaceMissing,
                    window,
                    cx,
                );
                if dropped > 0 {
        ...
        let mut app = Self {
            ...
            workspace,
            workspace_use: std::cell::RefCell::new(None),
            workspace_view_epoch: std::cell::Cell::new(uuid::Uuid::new_v4()),
            sleep_ui: None,
    ...
    pub(crate) fn switch_workspace(
        ...
        self.switch_workspace_inner(id, None, window, cx);
    }

    pub(crate) fn switch_workspace_prepared(
        &mut self,
        id: WorkspaceId,
        mut prepared: PreparedWorkspace,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) -> Result<(), PreparedWorkspace> {
        if !cx.has_global::<WorkspaceStore>()
            || !cx.has_global::<crate::ui::windows::WindowRegistry>()
            || !prepared.ready_to_commit()
        {
            return Err(prepared);
        }
        self.switch_workspace_inner(Some(id), Some(prepared), window, cx);
        Ok(())
    }
    ...
    fn switch_workspace_inner(
        ...
        if id == Some(previous) && prepared.is_none() {
            return;
        }
        self.retire_workspace_use(cx);
        ...
        if id == Some(previous) {
            // A staged replacement of this carrier must not save or remove
            // its previous (possibly stale or empty) client projection.
        } else if self.tabs.is_empty()
            && crate::ui::tree_sync::workspace_is_disposable(cx, previous)
        {
        ...
        if let Some(mut prepared) = prepared {
            let session = crate::ui::tree_sync::install_prepared_snapshot(
                cx,
                claimed,
                prepared.machine,
                prepared.target_workspace,
            );
            self.adopt_workspace_with_parts(
                claimed,
                session,
                prepared.policy,
                Some(&mut prepared.parts),
                window,
                cx,
            );
            return;
    ...
    pub(crate) fn adopt_workspace(
        ...
        if id == self.workspace && self.has_resume_pending(cx) {
            return;
        }
        self.adopt_workspace_with_parts(
            id,
    ...
    fn adopt_workspace_with_parts(
        &mut self,
        id: WorkspaceId,
        session: Session,
        policy: crate::terminal::RestorePolicy,
        parts: Option<&mut PreparedParts>,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
        ...
        let (tabs, active, dropped) = tabs_from_session_with_parts(
            pane_ws.as_ref(),
            self.workspace,
            Some(session),
            font_size,
    ...
    pub(super) fn land_pane(
        ...
        if !still_there {
            ...
            cleanup_cancelled_attachment(parts.restored, || {
                kill_pane_off_thread(route, parts.pane_id, cx);
            });
            return;
        }
        let was_focused = pending.read(cx).focus_handle.contains_focused(window, cx);
        let automatic = cx.global::<Config>().restore_agent_sessions;
        let resume = pending.update(cx, |pending, _| {
            pending.spawn.take_resume_command(parts.restored, automatic)
        });
        let view = build_terminal_view(parts, font_size, window, cx);
        if self.maximized.as_ref().is_some_and(|maximized| {
            pending
                .read(cx)
                .retained_view
                .as_ref()
                .is_some_and(|old| old == maximized)
        }) {
            self.maximized = Some(view.clone());
        }
    ...
    /// Called only after target-side history validation. Never route through
    /// focused_leaf: asynchronous creation may not have a terminal view yet.
    pub(crate) fn resume_history_session(
        &mut self,
        ...
        if self.spawn_host(cx) != host
            || !crate::ui::tree_sync::workspace_is_ready(cx, self.workspace)
        {
            return false;
        }
        ...
        let Some(invocation) = agent.resume_invocation(&session.id) else {
            return false;
        };
        let Some(slot) = self.new_tab_with_cwd(Some(session.cwd.clone().into()), None, window, cx)
        else {
            return false;
        };
        match slot {
            PaneSlot::Ready(view) => view.read(cx).run_command_line(&invocation.command_line()),
            PaneSlot::Connecting(pending) => pending.update(cx, |pending, _| {
                pending.spawn.agent = Some(agent);
                pending.spawn.agent_session_id = Some(session.id.clone());
                pending.spawn.explicit_resume = Some(invocation);
            }),
        }
        self.save_session(cx);
        true
    ...
    fn on_child_exited(
        ...
        if !view.read(cx).terminal.child_exited() || !self.owns_leaf(view.entity_id().as_u64()) {
            return;
        }
        // The terminal already owns its final grid and exit presentation.
        // An observed exit changes runtime state, not the user's layout.
        self.save_session(cx);
        cx.notify();
    }
    ...
    fn deliver_agent_prompt(&mut self, prompt: &str, window: &mut Window, cx: &mut Context<Self>) {
        let Some(target) = self.agent_target_leaf(cx) else {
            crate::terminal::notify_desktop(Some("agentty"), t(L10nKey::AppNoRunningCodingAgent));
            return;
        };
    ...
    fn send_selection_to_agent(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        let Some(selection) = selection else {
            crate::terminal::notify_desktop(Some("agentty"), t(L10nKey::AppNothingSelected));
            return;
        };
    ...
    fn send_git_diff_to_agent(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        let Some((host, cwd)) = target else {
            crate::terminal::notify_desktop(Some("agentty"), t(L10nKey::AppPaneNoKnownDirectory));
            return;
        };
        crate::ui::host_ops::HostOps::run_in(
            host,
...
#[cfg(test)]
mod machine_group_retirement_tests {
    use super::*;

    #[gpui::test]
    fn detached_pane_does_not_inherit_legacy_group(cx: &mut gpui::TestAppContext) {
        let (app, mut vcx, _streams) = test_window::harness_with_tabs(cx, 2);
        app.update_in(&mut vcx, |app, window, cx| {
            let second = app.tabs.remove(1);
            let first = app.tabs[0].pane.deep_clone();
            let kept = first.first_leaf().unwrap().entity_id();
            let moved = second.pane.first_leaf().unwrap().entity_id();
            let original = app.tabs[0].tree_id.get();
            let legacy = crate::core::group_key::GroupKey::custom("old-project");
            *app.tabs[0].sidebar_group.borrow_mut() = legacy.clone();
            app.tabs[0].pane = Pane::split_node(Axis::Horizontal, 0.5, first, second.pane);
            app.active = 0;
            app.detach_pane(moved, 1, window, cx);
            assert_eq!(app.tabs.len(), 2);
            assert_eq!(app.tabs[0].tree_id.get(), original);
            assert_eq!(*app.tabs[0].sidebar_group.borrow(), legacy);
            assert_eq!(app.tabs[0].pane.first_leaf().unwrap().entity_id(), kept);
            assert_eq!(app.tabs[1].pane.first_leaf().unwrap().entity_id(), moved);
            assert!(
                app.tabs[1].sidebar_group.borrow().is_none(),
                "new tabs must not inherit legacy groups"
            );
            let fresh = app.tabs[1].tree_id.get();
            assert_ne!(fresh, original);
            app.detach_pane(moved, 0, window, cx);
            assert_eq!(app.tabs.len(), 2, "sole pane cannot be detached");
            assert_eq!(app.tabs[1].tree_id.get(), fresh);
        });
    }
}
...
impl Render for Tty7App {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        if cx.has_active_drag() {
            ...
            self.validate_sidebar_reorder(window, cx);
        ...
        let body = match self.tabs.get(self.active) {
            ...
            Some(active_tab) => {
                let maximized = self.maximized.as_ref().and_then(|leaf| {
                    active_tab
                        .pane
                        .leaves()
                        .into_iter()
                        .find(|slot| slot.presents_terminal(leaf.entity_id(), cx))
                });
                match maximized {
                    Some(leaf) => div()
                        .size_full()
                        .overflow_hidden()
                        .map(|d| match leaf {
                            PaneSlot::Ready(view) => d.child(view),
                            PaneSlot::Connecting(pending) => d.child(pending),
                        })
                        .into_any_element(),
                    None => {
            ...
            .when_some(
                crate::ui::notice::anchor(
                    [
                        ...
                        self.render_retained_resume(window, cx),
                        self.render_session_sleep(window, cx),
                        ssh_status,
                    ]
                    .into_iter()
                    .flatten()
        ...
        let root =
            div()
                ...
                .children(self.render_history_manager(window, cx))
...
fn pane_to_session(pane: &Pane, cx: &App) -> SessionPane {
    match pane {
        ...
        Pane::Leaf(PaneSlot::Ready(view)) => {
            let view = view.read(cx);
            let may_restore_agent = !view.terminal.child_exited();
            let agent_session = may_restore_agent.then(|| view.agent_session()).flatten();
            SessionPane::Leaf {
                ...
                agent: may_restore_agent.then(|| view.agent()).flatten(),
                agent_session_id: agent_session.as_ref().and_then(|s| s.session_id.clone()),
                agent_launch_argv: agent_session.and_then(|s| s.launch_argv),
            }
        }
        Pane::Split {
            axis, a, b, ratio, ..
...
pub(crate) type PreparedParts = std::collections::BTreeMap<u64, crate::terminal::view::ShellParts>;

pub(crate) struct PreparedWorkspace {
    pub target_workspace: WorkspaceId,
    pub machine: tty7_core::core::machine::Machine,
    pub policy: crate::terminal::RestorePolicy,
    pub parts: PreparedParts,
}

impl PreparedWorkspace {
    fn ready_to_commit(&mut self) -> bool {
        let crate::terminal::RestorePolicy::Checked(guards) = &self.policy else {
            return false;
        };
        let Some(workspace) = self
            ...
            .workspaces
            .iter()
            .find(|w| w.id == self.target_workspace)
        else {
            return false;
        };
        let ids: Vec<_> = workspace
            ...
            .collect();
        !ids.is_empty()
            && ids.len() == self.parts.len()
            && ids.len() == guards.len()
            && ids.iter().collect::<std::collections::BTreeSet<_>>().len() == ids.len()
            && ids
                .into_iter()
                .all(|id| match (self.parts.get_mut(&id), guards.get(&id)) {
                    (Some(parts), Some(guard)) => {
                        parts.pane_id == id
                            && guard.proof.workspace == self.target_workspace
                            && parts.matches_checked_identity(&guard.identity)
                    }
                    _ => false,
                })
    }
...
fn tabs_from_session(
    ...
    session: Option<Session>,
    font_size: f32,
    restore_policy: crate::terminal::RestorePolicy,
    window: &mut Window,
    cx: &mut Context<Tty7App>,
) -> (Vec<Tab>, usize, usize) {
    tabs_from_session_with_parts(
        workspace,
        owner,
        session,
        font_size,
...
fn tabs_from_session_with_parts(
    workspace: Option<&crate::terminal::PaneWorkspace>,
    owner: WorkspaceId,
    session: Option<Session>,
    font_size: f32,
    restore_policy: crate::terminal::RestorePolicy,
    mut parts: Option<&mut PreparedParts>,
    window: &mut Window,
    cx: &mut Context<Tty7App>,
) -> (Vec<Tab>, usize, usize) {
    ...
    let alive = if parts.is_some() || session.tabs.iter().all(|tab| tab.pane.is_fully_stopped()) {
        None
    } else {
        alive_panes_on(&crate::terminal::PaneRoute::for_workspace(workspace))
    };
    let mut tabs: Vec<Tab> = Vec::with_capacity(session.tabs.len());
    let mut dropped = 0usize;
    for st in &session.tabs {
        let Some(pane) = session_to_pane_with_parts(
            workspace,
            owner,
            &st.pane,
            alive.as_ref(),
...
fn session_to_pane_with_parts(
    ...
    match sp {
        SessionPane::Stopped {
            ...
            // Checked Rejoin only consumes previously attached live resources.
            if parts.is_some()
                || matches!(restore_policy, crate::terminal::RestorePolicy::Checked(_))
            {
                return None;
            }
            ...
            let machine = pending_machine_label(&spawn, cx);
            let pending = cx.new(|cx| {
                let mut pending = crate::ui::pending_pane::PendingPane::new(machine, spawn, cx);
                pending.state = crate::ui::pending_pane::PendingState::Stopped;
                pending
            });
            cx.subscribe_in(
                &pending,
                window,
                |app, pending, event: &crate::ui::session_recovery::ResumeRequested, window, cx| {
                    app.resume_stopped(pending.clone(), event, window, cx);
                },
            )
            .detach();
            // Retry remains inert while stopped; after explicit Resume it
            // retries only the already-allocated successor's Checked attach.
            cx.subscribe_in(
                &pending,
                window,
            ...
            .detach();
            Some(Pane::leaf(PaneSlot::Connecting(pending)))
        }
        SessionPane::Leaf {
            cwd,
            pane_id,
            ...
            if let Some(parts) = parts {
                let mut prepared = parts.remove(&(*pane_id)?)?;
                prepared.owner = Some(owner);
                prepared.workspace = workspace.cloned();
                return Some(Pane::leaf(PaneSlot::Ready(build_terminal_view(
                    prepared, font_size, window, cx,
                ))));
            }
            let same_daemon =
            ...
            if restore.is_none() && restore_policy.attach_only() {
                // Passive tree facts cannot launch a shell or native SSH
                // replacement when the target identity is unavailable.
                return None;
            }
            if restore.is_none() {
            ...
            let view = match new_terminal_with_restore_policy(
                workspace.cloned(),
                Some(owner),
                font_size,
                cwd.clone(),
...
pub(crate) fn new_terminal(
    ...
    new_terminal_with_restore_policy(
        workspace,
        owner,
        font_size,
        working_directory,
...
pub(crate) fn new_terminal_with_restore_policy(
    workspace: Option<crate::terminal::PaneWorkspace>,
    owner: Option<WorkspaceId>,
    font_size: f32,
    working_directory: Option<std::path::PathBuf>,
    ...
    anyhow::ensure!(
        !restore_policy.attach_only() || restore_pane.is_some(),
        "AttachOnly requires an existing pane ID"
    );
    let spawn = crate::ui::pending_pane::PendingSpawn {
        restore_policy,
        workspace,
    ...
    if matches!(
        crate::terminal::PaneRoute::for_workspace(spawn.workspace.as_ref()),
        ...
        let parts = TerminalView::spawn_shell_terminal_in(
            ...
            spawn.shell.clone(),
            spawn.owner,
            spawn.restore_policy.clone(),
        );
        return finish_local_terminal_attempt(spawn, parts, window, cx);
    }
    pending_terminal_slot(spawn, None, window, cx)
}

fn cleanup_cancelled_attachment(restored: bool, kill_created: impl FnOnce()) {
    if !restored {
        kill_created();
    }
}

fn finish_local_terminal_attempt(
    spawn: crate::ui::pending_pane::PendingSpawn,
    parts: anyhow::Result<crate::terminal::view::ShellParts>,
    window: &mut Window,
    cx: &mut Context<Tty7App>,
) -> anyhow::Result<PaneSlot> {
    match parts {
        Ok(parts) => Ok(PaneSlot::Ready(build_terminal_view(
            parts,
            spawn.font_size,
            window,
            cx,
        ))),
        Err(error) if spawn.restore_pane.is_some() => {
            pending_terminal_slot(spawn, Some(format!("{error:#}")), window, cx)
        }
        Err(error) => Err(error),
    }
}

fn pending_machine_label(spawn: &crate::ui::pending_pane::PendingSpawn, cx: &App) -> String {
...
fn pending_terminal_slot(
    ...
    if let Some(failure) = failure {
        pending.update(cx, |pending, cx| pending.fail(failure, cx));
    } else {
        start_pane_spawn(pending.clone(), window, cx);
    }
...
pub(super) fn start_pane_spawn(
    ...
    cx.spawn_in(window, async move |this, cx| {
        let Some(parts) = super::host_ops::off_thread(move || {
...
#[cfg(test)]
mod local_restore_failure_tests {
    use super::*;
    use crate::ui::pending_pane::{PendingSpawn, PendingState};

    fn prepared_shell_workspace() -> (PreparedWorkspace, Vec<crate::daemon::transport::Stream>) {
        use crate::daemon::protocol::{CheckedPaneAttachment, PaneAttachExpectation};
        use tty7_core::core::machine::{Machine, PaneNode, PaneRecord, Tab as TreeTab, Workspace};
        let mut tab = TreeTab::leaf(71);
        tab.root = PaneNode::Split {
            axis: tty7_core::core::machine::Axis::Horizontal,
            ratio: 0.5,
        ...
        let target_workspace = workspace.id;
        let proof = tty7_core::daemon::control::AttachmentProof {
            workspace: target_workspace,
            nonce: uuid::Uuid::new_v4(),
        };
        let mut parts = PreparedParts::new();
        let mut peers = Vec::new();
        for id in [71, 72] {
            let (part, peer) = crate::terminal::view::quiet_test_shell_parts(id);
            ...
            .map(|id| {
                (
                    ...
                    CheckedPaneAttachment {
                        proof: proof.clone(),
                        tab: tab.id,
                        identity: PaneAttachExpectation::Shell,
                    },
                )
            })
            .collect();
        (
            PreparedWorkspace {
                ...
                machine: Machine {
                    ...
                    panes: vec![PaneRecord::new(71), PaneRecord::new(72)],
                },
                policy: crate::terminal::RestorePolicy::Checked(std::sync::Arc::new(guards)),
                parts,
            },
            peers,
        )
    ...
    #[gpui::test]
    fn prepared_rejoin_commit_consumes_existing_parts_without_attach(
        cx: &mut gpui::TestAppContext,
    ) {
        cx.update(crate::ui::windows::WindowRegistry::init);
        let (app, mut vcx, _streams) = test_window::harness_with_tabs(cx, 3);
        vcx.update(|_, cx| WorkspaceStore::install_for_test(cx, Default::default()));
        let (prepared, mut peers) = prepared_shell_workspace();
        app.update_in(&mut vcx, |app, window, cx| {
            let target = prepared.target_workspace;
            assert!(
                app.switch_workspace_prepared(target, prepared, window, cx)
                    .is_ok()
            );
            assert_eq!(app.workspace, target);
            assert_eq!(app.tabs.len(), 1);
            let ids: Vec<_> = app.tabs[0]
                .pane
                .leaves()
                .into_iter()
                .map(|slot| {
                    let PaneSlot::Ready(view) = slot else {
                        panic!("prepared pane reconnected")
                    };
                    assert!(view.read(cx).restored());
                    view.read(cx).pane_id
                })
                .collect();
            assert_eq!(ids, vec![71, 72]);
            assert!(crate::ui::tree_sync::workspace_is_ready(cx, target));
        });
        for peer in &mut peers {
            peer.set_read_timeout(Some(std::time::Duration::from_millis(20)))
                .unwrap();
            while let Ok(message) = crate::daemon::protocol::ClientMsg::read(peer) {
                assert!(
                    !matches!(
                        message,
                        crate::daemon::protocol::ClientMsg::Attach { .. }
                            | crate::daemon::protocol::ClientMsg::AttachChecked { .. }
                            | crate::daemon::protocol::ClientMsg::Spawn { .. }
                            | crate::daemon::protocol::ClientMsg::Input(_)
                    ),
                    "commit issued terminal effects: {message:?}"
                );
            }
        }
    }
    ...
    #[gpui::test]
    fn prepared_rejoin_incomplete_commit_preserves_current_window(cx: &mut gpui::TestAppContext) {
        cx.update(crate::ui::windows::WindowRegistry::init);
        let (app, mut vcx, _streams) = test_window::harness_with_tabs(cx, 3);
        vcx.update(|_, cx| WorkspaceStore::install_for_test(cx, Default::default()));
        let (mut prepared, _peers) = prepared_shell_workspace();
        prepared.parts.remove(&72);
        app.update_in(&mut vcx, |app, window, cx| {
            let previous = app.workspace;
            let ids: Vec<_> = app.tabs.iter().map(|t| t.tree_id.get()).collect();
            assert!(
                app.switch_workspace_prepared(prepared.target_workspace, prepared, window, cx)
                    .is_err()
            );
            assert_eq!(app.workspace, previous);
            assert_eq!(
    ...
    #[gpui::test]
    fn target_tree_split_restore_keeps_attach_only_intent(cx: &mut gpui::TestAppContext) {
        let (app, mut vcx, _streams) = test_window::harness_with_tabs(cx, 3);
        app.update_in(&mut vcx, |app, window, cx| {
            let workspace = crate::terminal::PaneWorkspace {
                workspace: WorkspaceId::new(),
                target: crate::core::session::RemoteTarget::Alias {
                    alias: "unroutable-fixture".into(),
                },
                spec: None,
                label: None,
                resize_echo: false,
            ...
            assert!(matches!(
                crate::terminal::PaneRoute::for_workspace(Some(&workspace)),
                crate::terminal::PaneRoute::Unroutable(_)
            ));
            let leaf = |id| SessionPane::Leaf {
                cwd: None,
                pane_id: Some(id),
                shell: None,
                ssh_spec: None,
            ...
            for policy in [
                crate::terminal::RestorePolicy::AttachOnly,
                crate::terminal::RestorePolicy::Checked(std::sync::Arc::new(guards)),
            ] {
                ...
                .unwrap();
                let mut ids = Vec::new();
                for slot in pane.leaves() {
                    let PaneSlot::Connecting(pending) = slot else {
                        panic!("unroutable target cannot be ready")
                    };
                    let spawn = &pending.read(cx).spawn;
                    assert_eq!(spawn.restore_policy, policy);
                    ids.push(spawn.restore_pane.unwrap());
                }
                assert_eq!(ids, vec![71, 72]);
            }
    ...
    #[gpui::test]
    fn confirmed_stopped_hydration_preserves_split_without_transport(
        cx: &mut gpui::TestAppContext,
    ) {
        use tty7_core::core::machine::{PaneExit, PaneNode, PaneRecord, Tab as TreeTab};
        let (app, mut vcx, _streams) = test_window::harness_with_tabs(cx, 3);
        app.update_in(&mut vcx, |app, window, cx| {
            let mut target = tty7_core::core::machine::Workspace::default();
            let id = tty7_core::core::machine::TabId::new();
            target.tabs.push(TreeTab {
                id,
            ...
            let records: Vec<_> = [71, 72]
                ...
                .collect();
            let session = crate::ui::tree_sync::session_from_tree(&target, &records);
            assert_eq!(session.tabs[0].tree_id, Some(id));
            assert_eq!(session.tabs[0].name.as_deref(), Some("retained"));
            let workspace = crate::terminal::PaneWorkspace {
                workspace: target.id,
                target: crate::core::session::RemoteTarget::Alias {
                    alias: "unroutable-fixture".into(),
            ...
            let Pane::Split { ratio, .. } = &pane else {
                panic!("split was lost")
            };
            assert_eq!(ratio.get(), 0.3);
            for (slot, id) in pane.leaves().into_iter().zip([71, 72]) {
                let PaneSlot::Connecting(pending) = slot else {
                    panic!("no terminal may start")
                };
                start_pane_spawn(pending.clone(), window, cx);
                pending.update(cx, |pending, cx| {
                    pending.retrying(cx);
                    pending.fail("late failure", cx);
                });
                let pending = pending.read(cx);
                assert_eq!(pending.spawn.restore_pane, Some(id));
                assert!(
                    !matches!(
                        pending.state,
                        crate::ui::pending_pane::PendingState::Connecting
                            | crate::ui::pending_pane::PendingState::Failed(_)
                    ),
                    "stopped hydration must not schedule an attachment attempt"
                );
                assert!(pending.spawn.agent.is_none());
            }
            let snapshot = pane_to_session(&pane, cx);
            let encoded = serde_json::to_string(&snapshot).unwrap();
            assert!(!encoded.contains("Stopped"));
            let SessionPane::Split { a, b, .. } = snapshot else {
                panic!("split locator")
            };
            for leaf in [a, b] {
                assert!(matches!(
            ...
            assert!(
                session_to_pane(
                    ...
                    None,
                    14.,
                    crate::terminal::RestorePolicy::Checked(
            ...
            assert_eq!(tabs[0].tree_id.get(), id);
            assert_eq!(tabs[0].name.as_deref(), Some("retained"));
            assert_eq!(active, 0);
            app.tabs = tabs;
            app.active = active;
            cx.notify();
        ...
        vcx.run_until_parked();
        assert!(
            vcx.debug_bounds("stopped-pane").is_some(),
            "stopped status is rendered"
        );
        assert!(vcx.debug_bounds("pending-pane-retry").is_none());
    }

    #[gpui::test]
    fn attach_only_without_identity_cannot_allocate_a_slot(cx: &mut gpui::TestAppContext) {
        let (app, mut vcx, _streams) = test_window::harness_with_tabs(cx, 3);
        app.update_in(&mut vcx, |app, window, cx| {
            let ids: Vec<_> = app.tabs.iter().map(|tab| tab.tree_id.get()).collect();
            let result = new_terminal_with_restore_policy(
                None,
                Some(app.workspace),
                14.,
                None,
    ...
    #[test]
    fn cancelled_attachment_never_kills_existing_pane() {
        let mut killed = Vec::new();
        cleanup_cancelled_attachment(true, || killed.push(77));
        assert!(
            killed.is_empty(),
            "an attached existing process is not ours to kill"
        );
        cleanup_cancelled_attachment(false, || killed.push(88));
        assert_eq!(killed, vec![88], "only the new orphan is cleaned up");
    }

    #[gpui::test]
    fn failed_local_restore_retains_retry_identity_without_losing_neighbors(
        cx: &mut gpui::TestAppContext,
    ) {
        let (app, mut vcx, _streams) = test_window::harness_with_tabs(cx, 3);
        app.update_in(&mut vcx, |app, window, cx| {
            ...
            let spawn = PendingSpawn {
                restore_policy: crate::terminal::RestorePolicy::ReplaceMissing,
                workspace: None, working_directory: Some("/original/cwd".into()),
                restore_pane: Some(77), shell: None,
                agent: Some(crate::core::cli_agent::CLIAgent::Codex),
                agent_session_id: Some("01900000-0000-7000-8000-000000000001".into()),
                agent_launch_argv: None, explicit_resume: None,
                owner: Some(app.workspace), font_size: 14.,
            };
            let slot = finish_local_terminal_attempt(spawn.clone(), Err(anyhow::anyhow!("attach timed out")), window, cx)
                .expect("failed restoration must retain a retryable slot");
            let PaneSlot::Connecting(pending) = &slot else { panic!("failure cannot be Ready") };
            let pending = pending.clone();
            assert!(matches!(&pending.read(cx).state, PendingState::Failed(reason) if reason.as_ref() == "attach timed out"));
            assert_eq!(pending.read(cx).spawn.restore_pane, Some(77));
            assert_eq!(pending.read(cx).spawn.owner, Some(app.workspace));
            app.tabs[1].pane = Pane::leaf(slot);
            app.land_pane(pending.entity_id(), &pending, Err("still offline".into()), 14., window, cx);
            let SessionPane::Leaf { pane_id, cwd, agent_session_id, .. } = pane_to_session(&app.tabs[1].pane, cx) else { panic!("same leaf") };
            assert_eq!(pane_id, Some(77));
            assert_eq!(cwd, spawn.working_directory);
            assert_eq!(agent_session_id, spawn.agent_session_id);
            assert_eq!(app.tabs.iter().map(|tab| tab.tree_id.get()).collect::<Vec<_>>(), ids);
            assert_eq!(app.active, 0);
            let mut fresh = spawn;
            fresh.restore_pane = None;
            assert!(finish_local_terminal_attempt(fresh, Err(anyhow::anyhow!("new terminal failed")), window, cx).is_err());
        });
    }
}
...
#[cfg(test)]
mod exit_retention_gpui_tests {
    use super::*;
    use crate::daemon::protocol::DaemonMsg;
    use gpui::TestAppContext;

    #[gpui::test]
    fn retained_resume_landing_preserves_maximized_focus(cx: &mut TestAppContext) {
        let (app, mut vcx, _streams) = test_window::harness_with_tabs(cx, 1);
        let (_pending, _peer) = app.update_in(&mut vcx, |app, window, cx| {
            let old = app.tabs[0].pane.leaves()[0].terminal().unwrap().clone();
            let tab_id = app.tabs[0].tree_id.get();
            window.focus(&old.read(cx).focus_handle.clone(), cx);
            ...
            let PaneSlot::Connecting(pending) = pane.leaves()[0].clone() else {
                panic!("expected stopped controller");
            };
            pending.update(cx, |pending, _| {
                pending.retained_view = Some(old.clone());
                pending.state = crate::ui::pending_pane::PendingState::Connecting;
                pending.spawn.restore_pane = Some(99);
            });
            app.tabs[0].pane = pane;
            app.maximized = Some(old);
            let (mut parts, peer) = crate::terminal::view::quiet_test_shell_parts(99);
            parts.owner = Some(app.workspace);
            // Completion can beat the first draw of the retained-output wrapper.
            app.land_pane(pending.entity_id(), &pending, Ok(parts), 14., window, cx);
            let new = app.tabs[0].pane.leaves()[0].terminal().unwrap().clone();
            assert_eq!(new.read(cx).pane_id, 99);
            assert_eq!(app.tabs[0].tree_id.get(), tab_id);
            assert_eq!(app.maximized.as_ref(), Some(&new));
            assert!(
                new.read(cx).focus_handle.contains_focused(window, cx),
                "fast landing lost original focus"
            );
            (pending, peer)
        });
    }
    ...
    #[gpui::test]
    fn child_exit_keeps_tab_split_and_selection(cx: &mut TestAppContext) {
        let (app, mut vcx, mut streams) = test_window::harness_with_tabs(cx, 3);
        let (ended, tab_ids, pane_ids, split_stream) =
            app.update_in(&mut vcx, |app, window, cx| {
                let ended = app.tabs[0]
                    .pane
                    .first_leaf()
                    .unwrap()
                    .terminal()
                    .unwrap()
                    .clone();
                let (neighbor, split_stream) =
                    crate::terminal::view::quiet_test_pane(89, window, cx);
                app.tabs[0].pane = Pane::split_node(
                    Axis::Horizontal,
                    0.4,
                    Pane::leaf(PaneSlot::Ready(ended.clone())),
                    Pane::leaf(PaneSlot::Ready(neighbor)),
                );
                app.tabs[0].name = Some("保留此会话".into());
                app.active = 2;
                let tab_ids = app.tabs.iter().map(|t| t.tree_id.get()).collect::<Vec<_>>();
                let pane_ids = app.tabs[0]
                    .pane
                    .leaves()
                    .iter()
                    .map(|p| p.entity_id())
                    .collect::<Vec<_>>();
                cx.subscribe_in(&ended, window, |app, view, _: &ChildExited, window, cx| {
                    app.on_child_exited(view.clone(), window, cx)
                })
                .detach();
                (ended, tab_ids, pane_ids, split_stream)
            });
        let _split_stream = split_stream;
        DaemonMsg::Output(b"last output before exit\r\n".to_vec())
        ...
        for _ in 0..200 {
            vcx.background_executor.run_until_parked();
            if ended.read_with(&vcx, |view, _| view.terminal.exited) {
                break;
            }
            std::thread::sleep(std::time::Duration::from_millis(5));
        }
        app.update_in(&mut vcx, |app, window, cx| {
            assert!(ended.read(cx).terminal.exited);
            {
                use alacritty_terminal::grid::Dimensions as _;
                use alacritty_terminal::index::{Column, Line};
                let term = ended.read(cx).terminal.term.lock();
                let grid = term.grid();
                let output = (0..grid.columns())
                    .map(|c| grid[Line(0)][Column(c)].c)
                    .collect::<String>();
                assert!(
                    output.starts_with("last output before exit"),
                    "the final grid stays readable"
                );
            }
            assert_eq!(
                app.tabs.iter().map(|t| t.tree_id.get()).collect::<Vec<_>>(),
            ...
            assert_eq!(app.tabs[0].name.as_deref(), Some("保留此会话"));
            assert_eq!(app.active, 2);
            assert!(app.closed.is_empty());
            assert!(matches!(&app.tabs[0].pane, Pane::Split { ratio, .. } if ratio.get() == 0.4));
            app.on_child_exited(ended.clone(), window, cx);
            assert_eq!(app.tabs.len(), 3, "duplicate event preserves rows");
            assert_eq!(app.active, 2);
            app.tabs.remove(0);
            app.active = 1;
            app.on_child_exited(ended.clone(), window, cx);
            assert_eq!(
                app.tabs.len(),
                2,
                "late event cannot recreate a removed row"
            );
    ...
    #[gpui::test]
    fn child_exit_snapshot_distinguishes_disconnection(cx: &mut TestAppContext) {
        use crate::core::cli_agent::{AgentSessionState, CLIAgent};
        let (app, mut vcx, mut streams) = test_window::harness_with_tabs(cx, 1);
        ...
        DaemonMsg::AgentStatus(Some(AgentSessionState {
            session_id: Some("running-before-disconnect".into()),
            ..Default::default()
        }))
        .encode(&mut streams[0])
        .unwrap();
        for _ in 0..200 {
            if view.read_with(&vcx, |v, _| v.agent_session().is_some()) {
                break;
            }
            std::thread::sleep(std::time::Duration::from_millis(5));
        }
        ...
        for _ in 0..200 {
            vcx.background_executor.run_until_parked();
            if view.read_with(&vcx, |v, _| v.terminal.exited) {
                break;
            }
            std::thread::sleep(std::time::Duration::from_millis(5));
        }
        app.update_in(&mut vcx, |app, _, cx| {
            assert!(view.read(cx).terminal.exited);
            ...
            assert_eq!(agent, Some(CLIAgent::Codex));
            assert_eq!(
                agent_session_id.as_deref(),
                Some("running-before-disconnect")
            );
        });
    }

    #[gpui::test]
    fn child_exit_snapshot_has_no_automatic_resume_identity(cx: &mut TestAppContext) {
        use crate::core::cli_agent::{AgentSessionState, CLIAgent};
        let (app, mut vcx, mut streams) = test_window::harness_with_tabs(cx, 1);
        let view = app.read_with(&vcx, |app, _| {
            app.tabs[0]
        ...
        DaemonMsg::Agent(Some(CLIAgent::Codex))
            ...
            .unwrap();
        DaemonMsg::AgentStatus(Some(AgentSessionState {
            session_id: Some("still-cached".into()),
        ...
        for _ in 0..200 {
            vcx.background_executor.run_until_parked();
            if view.read_with(&vcx, |v, _| v.terminal.child_exited()) {
                break;
            }
            std::thread::sleep(std::time::Duration::from_millis(5));
        }
