...
pub(crate) fn default_bindings() -> Vec<(&'static str, &'static str)> {
    vec![
        ...
        // Preserve the existing New Window shortcut; retired workspace chords
        // are not silently reassigned to another operation.
...
mod tests {
    ...
    #[test]
    fn machine_keymap_rejects_legacy_workspace_mutations() {
        assert!(make_binding("NewTab", "secondary-t").is_some());
        assert!(make_binding("OpenSshProfiles", "secondary-shift-h").is_some());
        for action in [
            "NewWorkspace",
            "RenameWorkspace",
            "StopWorkspace",
            ...
            assert!(
                make_binding(action, "secondary-shift-n").is_none(),
                "retired action remains dispatchable through a custom binding: {action}"
            );
            assert!(
                !default_bindings().iter().any(|(name, _)| *name == action),
                "retired action remains in the default binding inventory: {action}"
            );
            assert!(
                dispatched(
                    &[(action.into(), "secondary-shift-n".into())],
                    "secondary-shift-n",
                    "Terminal"
                )
                .is_empty(),
                "retired action still reaches the real keymap: {action}"
            );
        }
    }
