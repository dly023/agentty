...
/// Client-side eligibility only; target ownership is still acquired by IfFree.
pub(crate) fn remote_workspace_claimable(
    cx: &mut gpui::App,
    host: HostId,
    workspace: WorkspaceId,
    current: Option<WorkspaceId>,
) -> bool {
    if !cx.has_global::<WorkspaceStore>() || !cx.has_global::<crate::ui::windows::WindowRegistry>()
    {
        return false;
    }
    let mapped: Vec<_> = WorkspaceStore::all(cx)
        ...
        .iter()
        .filter(|view| {
            view.host.as_ref().is_some_and(|reference| {
                reference.host_id() == host && reference.workspace == workspace
            })
        })
        .map(|view| (view.id, view.open))
        .collect();
    mapped.len() <= 1
        && mapped.into_iter().all(|(id, open)| {
            !workspace_is_preempted(cx, id)
                && (Some(id) == current
                    || (!open && crate::ui::windows::WindowRegistry::window_for(cx, id).is_none()))
        })
}
...
impl Tty7App {
    ...
    /// An approved replacement restores supervised views and retries only a
    /// matching explicit connection intent. It carries no workspace creation.
    fn server_replaced(
        &mut self,
        target: &RemoteTarget,
        label: &str,
    ...
    pub(crate) fn open_remote_workspace(
        ...
        let expected_host = target.host_id();
        if !remote_workspace_claimable(cx, expected_host, row.id, Some(self.workspace)) {
        ...
        self.workspace == id && self.spawn_host(cx) == expected_host
    }
...
pub(crate) struct RemoteLinks {
    ...
    attachment_scopes: std::collections::HashMap<WorkspaceId, AttachmentScope>,
    /// Reconnect observed a restart/takeover; rebuild only after acquisition.
    rebuild_after_attach: std::collections::HashSet<WorkspaceId>,
...
impl RemoteLinks {
    ...
    pub(crate) fn disconnect(cx: &mut gpui::App, host: HostId) {
        ...
        for (workspace, _) in workspaces_on(cx, host) {
            ...
            if let Some(app) = crate::ui::windows::WindowRegistry::app_for(cx, workspace)
                .and_then(|app| app.upgrade())
            {
                app.update(cx, |app, cx| app.retire_workspace_use(cx));
            }
            let links = cx.default_global::<RemoteLinks>();
            links.preempted.remove(&workspace);
            links.reclaiming.remove(&workspace);
            links.attaching.remove(&workspace);
            links.attachment_scopes.remove(&workspace);
            links.rebuild_after_attach.remove(&workspace);
        }
...
fn pump_tick(cx: &mut gpui::App) -> bool {
    ...
    if bound.is_empty() {
        ...
        links.attachment_scopes.clear();
        links.rebuild_after_attach.clear();
...
enum AttachmentAcquired {
    Tracked(tty7_core::daemon::control::WorkspaceUse),
    TakenBack(tty7_core::daemon::control::WorkspaceUse, Option<String>),
}

impl AttachmentAcquired {
    fn into_parts(self) -> (tty7_core::daemon::control::WorkspaceUse, Option<String>) {
        match self {
            Self::Tracked(usage) => (usage, None),
            Self::TakenBack(usage, who) => (usage, who),
        }
    }
}

fn acquire_attachment(
    client: &crate::daemon::control::ControlClient,
    key: &str,
    explicit_take_back: bool,
) -> Result<AttachmentAcquired, String> {
    let unexpected = || t(L10nKey::RemoteWorkspaceAttachmentUnexpectedReply).to_string();
    let workspace = key.parse::<WorkspaceId>().map_err(|_| unexpected())?;
    let outcome = if explicit_take_back {
        client
            .take_back_workspace_use(workspace)
            .map(|(usage, who)| AttachmentAcquired::TakenBack(usage, who))
    } else {
        client
            .acquire_workspace_use(workspace)
            .map(AttachmentAcquired::Tracked)
    };
...
impl AttachmentScope {
    fn capture(cx: &mut gpui::App, workspace: WorkspaceId) -> Option<Self> {
        let app = crate::ui::windows::WindowRegistry::app_for(cx, workspace)?.upgrade()?;
        let view = app.read(cx);
        (view.workspace == workspace).then(|| Self {
            workspace,
            entity: app.entity_id(),
            epoch: view.workspace_view_epoch.get(),
        })
    }

    fn is_current(&self, cx: &mut gpui::App) -> bool {
        Self::capture(cx, self.workspace).as_ref() == Some(self)
    }
}

fn refresh_attachment_scopes(cx: &mut gpui::App, host: HostId) {
    for (workspace, _) in workspaces_on(cx, host) {
        let scope = AttachmentScope::capture(cx, workspace);
        let links = cx.default_global::<RemoteLinks>();
        let old = links.attachment_scopes.get(&workspace).copied();
        if old == scope {
            continue;
        }
        links.attaching.remove(&workspace);
        if let Some(link) = links.machines.get_mut(&host) {
        ...
        if let Some(scope) = scope {
            links.attachment_scopes.insert(workspace, scope);
        } else {
            links.attachment_scopes.remove(&workspace);
        }
    }
...
fn pump_attachments(cx: &mut gpui::App, host: HostId) {
    ...
    refresh_attachment_scopes(cx, host);
    for (workspace, key) in reclaims_due(cx, host) {
        ...
        cx.spawn(async move |cx| {
            ...
            let outcome = crate::ui::host_ops::off_thread(move || {
                acquire_attachment(&worker, &worker_key, explicit_take_back)
            })
            .await
            .unwrap_or_else(|| Err(t(L10nKey::RemoteWorkspaceAttachmentUnexpectedReply).into()));
            cx.update(|cx| {
                finish_attachment(cx, host, workspace, &key, &client, Some(scope), outcome)
            });
        })
        .detach();
    }
...
fn finish_attachment(
    ...
    workspace: WorkspaceId,
    key: &str,
    client: &Arc<crate::daemon::control::ControlClient>,
    scope: Option<AttachmentScope>,
    outcome: Result<AttachmentAcquired, String>,
) {
    let same_client = remote_connect::HostLinks::get(cx, host)
        .is_some_and(|current| Arc::ptr_eq(current.client(), client) && client.is_connected());
    let mapped = same_client
        && workspaces_on(cx, host)
            .iter()
            .any(|(id, target)| *id == workspace && target == key);
    if same_client && !mapped {
        cx.default_global::<RemoteLinks>()
            .attaching
            .remove(&workspace);
    }
    ...
    if !current {
        if let Ok(acquired) = outcome {
            let (usage, _) = acquired.into_parts();
            crate::ui::session_recovery::release(client.clone(), usage, cx);
        }
        return;
    }
    let outcome = outcome.map(|acquired| {
        let (usage, who) = acquired.into_parts();
        // Scope validation and handoff are one UI transaction, with no I/O.
        let app = crate::ui::windows::WindowRegistry::app_for(cx, workspace)
            .and_then(|app| app.upgrade())
            .expect("validated attachment scope");
...
fn finish_reclaim(
    ...
    let reclaimed = {
        ...
        match &outcome {
            Ok(_) => {
                links.reclaiming.remove(&workspace).is_some()
                    | links.rebuild_after_attach.remove(&workspace)
            }
...
fn launch_attempt(cx: &mut gpui::App, host: HostId, target: RemoteTarget) {
    ...
    cx.spawn(async move |cx| {
        ...
        let outcome = cx
            .background_executor()
            .spawn(
                async move { remote_connect::connect_blocking(&target, header, &label_for_task) },
            )
            .await;
...
mod tests {
    #[cfg(unix)]
    #[gpui::test]
    fn automatic_attachment_use_is_retained_or_retired_by_scope(cx: &mut gpui::TestAppContext) {
        use gpui::VisualContext as _;
        use std::sync::{atomic::AtomicBool, mpsc};
        use tty7_core::daemon::control::{AttachmentProof, ControlHello};
        ...
        .flat_map(|case| [(case, false), (case, true)])
        {
            ...
            let mut exchanges = vec![(
                ControlRequest::WorkspaceAttachIfFree { id: key.clone() },
            ...
            if explicit {
                exchanges = vec![
                    (
                        ControlRequest::WorkspaceAttach { id: key.clone() },
                        ReplyOk::Attached {
                            took_over_from: Some("other".into()),
                        },
                        false,
                    ),
                    (
                        ...
                        ReplyOk::AttachmentProof(proof.clone()),
                        false,
                    ),
                ];
            }
            if committed {
                exchanges.push((
                    ControlRequest::Shells,
                    ReplyOk::Shells(Default::default()),
                    false,
                ));
            }
            exchanges.push((
            ...
            let (socket, server) = crate::ui::test_control::control_socket_fixture(
                exchanges,
                Arc::new(AtomicBool::new(true)),
                move |req| {
                    if matches!(
                        req,
                        ControlRequest::Shells | ControlRequest::WorkspaceReleaseProof { .. }
                    ) {
                        events_tx.send(req.clone()).unwrap();
                    }
                },
            );
            let remote = RemoteHost::over_unix(
                socket,
            ...
            app.update(cx, |app, cx| {
                let entry = crate::core::session::WindowView {
                    id: app.workspace,
                    open: true,
                    host: Some(RemoteRef::new(target, proof.workspace)),
                    ..Default::default()
                };
                ...
                remote_connect::HostLinks::insert(cx, remote, "/fixture".into());
                RemoteLinks::mark(cx, host, |_| {});
            });
            let workspace = cx.update(|cx| app.read(cx).workspace);
            let scope = cx.update(|cx| {
                refresh_attachment_scopes(cx, host);
                AttachmentScope::capture(cx, workspace).unwrap()
            });
            let acquired = acquire_attachment(&client, &key, explicit).ok().unwrap();
            if case == "epoch" {
                app.update(cx, |app, cx| app.retire_workspace_use(cx));
            }
            cx.update(|cx| {
                if case == "unregistered" {
                    crate::ui::windows::WindowRegistry::unregister(cx, workspace);
                }
                if case == "connection" {
                    remote_connect::HostLinks::remove(cx, host);
                }
                if case == "replacement" {
                    let replacement = fake_connected(&target_key);
                ...
                assert_eq!(
                    app.read(cx).workspace_use.borrow().is_some(),
                    committed,
                    "{case}"
                );
                if committed {
            ...
            loop {
                ...
                match events_rx.try_recv() {
                    Ok(ControlRequest::Shells) => {
                        if case == "disconnect" {
                            cx.update(|cx| RemoteLinks::disconnect(cx, host));
                        } else {
                            app.update(cx, |app, cx| app.retire_workspace_use(cx));
                        }
                    }
                    Ok(ControlRequest::WorkspaceReleaseProof { proof: actual }) => {
                        assert_eq!(actual, proof);
                        break;
                    }
                    Ok(other) => panic!("unexpected event {other:?}"),
                    Err(mpsc::TryRecvError::Empty) => {
                        assert!(Instant::now() < deadline, "{case}: no retirement");
            ...
            assert!(cx.update(|cx| app.read(cx).workspace_use.borrow().is_none()));
            client.close();
            server.join().unwrap();
            cx.update(|cx| remote_connect::HostLinks::remove(cx, host));
        }
    }

    #[gpui::test]
    fn automatic_attachment_scope_expires_on_view_retirement(cx: &mut gpui::TestAppContext) {
        use gpui::VisualContext as _;
        let (app, vcx) = crate::ui::app::test_window::harness(cx);
        let handle = vcx.window_handle();
        let weak = app.downgrade();
        ...
        let workspace = cx.update(|cx| app.read(cx).workspace);
        let captured = cx.update(|cx| AttachmentScope::capture(cx, workspace).unwrap());
        assert!(cx.update(|cx| captured.is_current(cx)));
        app.update(cx, |app, cx| app.retire_workspace_use(cx));
        assert!(
            !cx.update(|cx| captured.is_current(cx)),
            "same workspace ID revived an old epoch"
        );
        let reopened = cx.update(|cx| AttachmentScope::capture(cx, workspace).unwrap());
        assert_ne!(captured, reopened);
        assert!(cx.update(|cx| reopened.is_current(cx)));
        cx.update(|cx| crate::ui::windows::WindowRegistry::unregister(cx, workspace));
        assert!(
            !cx.update(|cx| reopened.is_current(cx)),
            "unregistered view accepted completion"
        );
        let (replacement, replacement_vcx) = crate::ui::app::test_window::harness(cx);
        let replacement_handle = replacement_vcx.window_handle();
        let replacement_weak = replacement.downgrade();
        replacement.update(cx, |app, cx| {
            app.workspace = workspace;
            // Hold the epoch fixed to isolate the entity identity check.
            app.workspace_view_epoch.set(reopened.epoch);
            crate::ui::windows::WindowRegistry::init(cx);
        ...
        assert!(
            !cx.update(|cx| reopened.is_current(cx)),
            "replacement entity inherited an old scope"
        );
    }

    #[gpui::test]
    fn automatic_attachment_scope_refresh_reopens_eligibility(cx: &mut gpui::TestAppContext) {
        use gpui::VisualContext as _;
        let (app, vcx) = crate::ui::app::test_window::harness(cx);
        let handle = vcx.window_handle();
        let weak = app.downgrade();
        let (host, target) = machine("tty7-test-scope");
        #[cfg(unix)]
        ...
        cx.update(|cx| {
            ...
            assert!(
                ...
                "unchanged scope duplicated its request"
            );
        });
        app.update(cx, |app, cx| app.retire_workspace_use(cx));
        cx.update(|cx| {
            ...
            assert_eq!(
                ...
                1,
                "reopened view stayed suppressed"
            );
            #[cfg(unix)]
            {
                let client = connected.host.client().clone();
                remote_connect::HostLinks::insert(cx, connected.host, connected.home);
                let key = workspaces_on(cx, host)[0].1.clone();
                finish_attachment(
                    cx,
                    host,
                    workspace,
                    ...
                    &client,
                    Some(captured),
                    Err("old scope failure".into()),
                );
                let links = cx.default_global::<RemoteLinks>();
                assert!(
                    links.attaching.contains(&workspace),
                    "old reply ended the current attempt"
                );
                assert!(!links.machines[&host].attach_sent.contains(&workspace));
                assert!(links.machines[&host].last_error.is_none());
                let current_scope = AttachmentScope::capture(cx, workspace).unwrap();
                finish_attachment(
                    cx,
                    host,
                    workspace,
                    ...
                    &client,
                    Some(current_scope),
                    Err("current scope failure".into()),
                );
                let links = cx.default_global::<RemoteLinks>();
                assert!(!links.attaching.contains(&workspace));
                assert_eq!(
                    links.machines[&host].last_error.as_deref(),
                    Some("current scope failure")
                );
                client.close();
                remote_connect::HostLinks::remove(cx, host);
            }
            cx.default_global::<RemoteLinks>()
                .reclaiming
                .insert(workspace, "other client".into());
        });
        app.update(cx, |app, cx| app.retire_workspace_use(cx));
        cx.update(|cx| {
            refresh_attachment_scopes(cx, host);
            let links = cx.default_global::<RemoteLinks>();
            assert!(!links.reclaiming.contains_key(&workspace));
            assert_eq!(
            ...
            assert!(
                reclaims_due(cx, host).is_empty(),
                "old view's takeover became automatic"
            );
        });
    }
    ...
    #[test]
    #[cfg(unix)]
    fn automatic_attachment_reply_requires_exact_workspace() {
        use crate::ui::test_control::control_fixture;
        use std::sync::atomic::AtomicBool;
        use tty7_core::daemon::control::AttachmentProof;
        for case in [
            "fresh", "reused", "foreign", "unit", "pong", "attached", "invalid",
        ] {
            let workspace = WorkspaceId::new();
            let proof = AttachmentProof {
                workspace: if case == "foreign" {
                    WorkspaceId::new()
                } else {
                    workspace
                },
            ...
            let reply = match case {
                "unit" => ReplyOk::Unit,
                "pong" => ReplyOk::Pong,
                "attached" => ReplyOk::Attached {
                    took_over_from: None,
                },
                _ => ReplyOk::WorkspaceAcquired {
                    proof: proof.clone(),
                    newly_acquired: case != "reused",
                },
            };
            ...
            let key = if case == "invalid" {
                "invalid".into()
            } else {
                workspace.to_string()
            };
            let outcome = acquire_attachment(&client, &key, false);
            if matches!(case, "fresh" | "reused") {
                let AttachmentAcquired::Tracked(usage) =
                    outcome.ok().expect("valid tracked acquisition")
                else {
                    panic!("automatic takeover");
                };
                assert_eq!(usage.proof(), &proof);
                assert_eq!(client.finish_workspace_use(usage).unwrap(), case == "fresh");
            } else {
                assert_eq!(
                    outcome.err().unwrap(),
                    t(L10nKey::RemoteWorkspaceAttachmentUnexpectedReply)
                );
            }
            client.close();
            server.join().unwrap();
    ...
    #[test]
    #[cfg(unix)]
    fn automatic_attachment_never_requests_takeover() {
        use crate::ui::test_control::control_fixture;
        use std::sync::atomic::AtomicBool;
        use tty7_core::daemon::control::AttachmentProof;
        // Only explicit Take Back sends Attach; malformed readback cannot arm cleanup.
        for case in [
            "attached",
            "pong",
            "fresh",
            ...
            let uncertain = case.starts_with("proof_");
            let mut exchanges = Vec::new();
            if uncertain {
                exchanges.push((
                    ControlRequest::WorkspaceAttachIfFree {
            ...
            if case != "invalid" {
                let reply = match case {
                    "attached" | "proof_foreign" | "proof_pong" => ReplyOk::Attached {
                        took_over_from: Some("other client".into()),
                    },
                    "pong" => ReplyOk::Pong,
                    _ => ReplyOk::WorkspaceAcquired {
            ...
            if case == "attached" || uncertain {
                let reply = match case {
                    "proof_pong" => ReplyOk::Pong,
                    "proof_foreign" => ReplyOk::AttachmentProof(AttachmentProof {
                        workspace: WorkspaceId::new(),
                        nonce: proof.nonce,
                    }),
                    _ => ReplyOk::AttachmentProof(proof.clone()),
                };
                exchanges.push((
            ...
            let (client, server) =
                control_fixture(exchanges, Arc::new(AtomicBool::new(true)), |_| {});
            let prior = uncertain.then(|| client.acquire_workspace_use(workspace).unwrap());
            let key = if case == "invalid" {
                "invalid".into()
            } else {
                workspace.to_string()
            };
            let outcome = acquire_attachment(&client, &key, true);
            if case == "attached" {
                let AttachmentAcquired::TakenBack(usage, who) = outcome.ok().unwrap() else {
                    panic!("expected tracked takeover");
                };
                assert_eq!(who.as_deref(), Some("other client"));
                assert_eq!(usage.proof(), &proof);
                assert!(client.finish_workspace_use(usage).unwrap());
            } else {
                assert_eq!(
                    outcome.err().unwrap(),
                    t(L10nKey::RemoteWorkspaceAttachmentUnexpectedReply)
            ...
            if let Some(prior) = prior {
                assert!(
                    !client.finish_workspace_use(prior).unwrap(),
                    "uncertain takeover must pin the old claim"
                );
            }
            assert!(matches!(
                client.call(ControlRequest::Ping).unwrap(),
    ...
    #[test]
    #[cfg(unix)]
    fn automatic_attachment_acquires_tracked_use_without_takeover() {
        use crate::ui::test_control::control_fixture;
        use std::sync::atomic::AtomicBool;
        use tty7_core::daemon::control::AttachmentProof;
        let proof = AttachmentProof {
        ...
        let AttachmentAcquired::Tracked(first) =
            acquire_attachment(&client, &key, false).ok().unwrap()
        else {
            panic!("automatic takeover");
        };
        let AttachmentAcquired::Tracked(second) =
            acquire_attachment(&client, &key, false).ok().unwrap()
        else {
            panic!("automatic takeover");
        };
    ...
    #[cfg(unix)]
    #[gpui::test]
    fn late_attachment_reply_cannot_mark_replacement_connection(cx: &mut gpui::TestAppContext) {
        cx.update(|cx| {
            crate::core::config::pin_test_config_dir();
            cx.set_global(crate::core::config::Config::default());
            crate::ui::windows::WindowRegistry::init(cx);
            let (host, target) = machine("tty7-test-late-attachment");
            ...
            entry.open = true;
            let id = entry.id;
            let key = entry.host.as_ref().unwrap().store_key();
            WorkspaceStore::install_for_test(
                cx,
                crate::core::session::WindowViews {
            ...
            let old = fake_connected(&target.connection_key());
            let replacement = fake_connected(&target.connection_key());
            let old_client = old.host.client().clone();
            let current = replacement.host.client().clone();
            remote_connect::HostLinks::insert(cx, replacement.host, replacement.home);
            RemoteLinks::mark(cx, host, |link| {
                link.last_error = Some("current failure".into())
            });
            cx.default_global::<RemoteLinks>().attaching.insert(id);
            finish_attachment(
                cx,
                host,
                id,
                ...
                &old_client,
                None,
                Err("old acquisition failure".into()),
            );
            let links = cx.default_global::<RemoteLinks>();
            assert!(
                links.attaching.contains(&id),
                "old reply ended the new acquisition"
            );
            assert!(!links.machines[&host].attach_sent.contains(&id));
            assert_eq!(
                links.machines[&host].last_error.as_deref(),
            ...
            finish_attachment(
                ...
                &current,
                None,
                Err("closed window's failure".into()),
            );
            let links = cx.default_global::<RemoteLinks>();
    ...
    #[cfg(unix)]
    #[gpui::test]
    fn reconnect_leaves_current_workspace_acquisition_to_the_pump(cx: &mut gpui::TestAppContext) {
        cx.update(|cx| {
            crate::core::config::pin_test_config_dir();
            cx.set_global(crate::core::config::Config::default());
            crate::ui::windows::WindowRegistry::init(cx);
            let (host, target) = machine("tty7-test-reconnect-entry");
            let mut open = crate::core::session::WindowView::on_remote(RemoteRef::new(
                target.clone(),
                WorkspaceId::new(),
            ));
            open.open = true;
            let mut closed = crate::core::session::WindowView::on_remote(RemoteRef::new(
                target.clone(),
                WorkspaceId::new(),
            ));
            closed.open = false;
            let expected = (open.id, open.host.as_ref().unwrap().store_key());
            let mut connected = fake_connected(&target.connection_key());
            connected.rows = [&open, &closed]
                .into_iter()
                .map(|view| RemoteWorkspaceRow {
                    id: view.host.as_ref().unwrap().workspace,
                    name: "fixture".into(),
                    panes: 0,
                    last_active: 0,
                })
            ...
            WorkspaceStore::install_for_test(
                cx,
                crate::core::session::WindowViews {
                    views: vec![open, closed],
                    active: None,
                },
            );
            RemoteLinks::mark(cx, host, |link| {
                link.attach_sent.insert(expected.0);
            });
            cx.default_global::<RemoteLinks>()
                .attaching
                .insert(expected.0);

            // This peer has no machine service. Reconnect must install its link
            // without attempting workspace acquisition or claiming success.
            finish_attempt(cx, host, &target, Ok(connected));
            let links = cx.default_global::<RemoteLinks>();
            assert!(!links.machines[&host].attach_sent.contains(&expected.0));
            assert!(!links.attaching.contains(&expected.0));
            assert_eq!(reclaims_due(cx, host), vec![expected]);
            assert!(
                reclaims_due(cx, host).is_empty(),
                "only one pending acquisition"
            );
            client.close();
            remote_connect::HostLinks::remove(cx, host);
        });
    ...
    #[cfg(unix)]
    #[gpui::test]
    fn machine_connection_after_refusal_does_not_create_workspace(cx: &mut gpui::TestAppContext) {
        use gpui::VisualContext as _;

        let (app, mut vcx) = crate::ui::app::test_window::harness(cx);
        // Register the live window so any accidental layout transition would
        // exercise the actual window path. Connection alone must not do so.
        let handle = vcx.window_handle();
        let weak = app.downgrade();
        app.update(cx, |app, cx| {
            crate::core::session::WorkspaceStore::install_for_test(
        ...
        app.update(cx, |app, cx| {
            assert!(WorkspaceStore::remote_ref(cx, app.workspace).is_none());
            assert!(
                WorkspaceStore::all(cx).views.is_empty(),
                "connection must not create a layout"
            );
            assert!(!crate::ui::tree_sync::tracks_workspace_for_test(
                cx,
                app.workspace
            ));
            assert!(app.connect.is_none(), "the machine connection completed");
            assert!(app.host_snapshots.contains_key(&target.host_id()));
        });
    }

    /// An approved replacement retries the existing machine connection intent;
    /// no deferred workspace creation is part of that intent.
    #[gpui::test]
    fn machine_server_replacement_retries_connection_without_create(cx: &mut gpui::TestAppContext) {
        let (app, mut vcx) = crate::ui::app::test_window::harness(cx);
        let target = RemoteTarget::Alias {
            alias: "tty7-test-refused-box".into(),
        };
    ...
    #[gpui::test]
    fn machine_server_replacement_preserves_absent_or_foreign_intent(
        cx: &mut gpui::TestAppContext,
    ) {
        let (app, _vcx) = crate::ui::app::test_window::harness(cx);
        let replaced = RemoteTarget::Alias {
            alias: "replacement-target".into(),
        };
        let foreign = HostChoice {
            target: RemoteTarget::Alias {
                alias: "other-connection".into(),
            },
            label: "Other".into(),
            detail: String::new(),
        };
        app.update(cx, |app, cx| {
            ...
            let tabs: Vec<_> = app.tabs.iter().map(|tab| tab.tree_id.get()).collect();
            app.connect = None;
            app.server_replaced(&replaced, "Replaced", "no-supervised-origin", cx);
            assert!(app.connect.is_none());
            app.connect = Some(ConnectFlow::Failed { choice: foreign.clone(), error: "keep-this-failure".into() });
            app.server_replaced(&replaced, "Replaced", "no-supervised-origin", cx);
            assert!(matches!(&app.connect, Some(ConnectFlow::Failed { choice, error }) if choice.target == foreign.target && error == "keep-this-failure"));
            assert_eq!(app.workspace, original);
            assert_eq!(app.tabs.iter().map(|tab| tab.tree_id.get()).collect::<Vec<_>>(), tabs);
        });
    }
    ...
    fn a_reclaim_that_lands_leaves_nothing_behind(cx: &mut gpui::TestAppContext) {
        cx.update(|cx| {
            ...
            finish_reclaim(cx, host, id, Ok(Some("desktop".into())));
    ...
    fn a_workspace_never_attached_on_this_link_is_due(cx: &mut gpui::TestAppContext) {
        cx.update(|cx| {
            ...
            finish_reclaim(cx, host, id, Ok(None));
            assert!(
                reclaims_due(cx, host).is_empty(),
