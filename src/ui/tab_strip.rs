...
///
/// Shared ceiling for the launcher and its alternate-shell submenu, capped
/// again against the viewport so short windows keep every action reachable.
...
impl NewTabMenu {
    fn alternate_shells(
        shells: &[(SharedString, ShellSpec)],
        default_shell: &SharedString,
    ) -> Vec<(SharedString, ShellSpec)> {
        shells
            .iter()
            .filter(|(label, _)| label != default_shell)
            .cloned()
            .collect()
    }

    fn build(self, menu: PopupMenu, window: &mut Window, cx: &mut Context<PopupMenu>) -> PopupMenu {
        ...
        let mut menu = menu
            ...
            .max_w(MENU_W)
            // Keep environment actions reachable in short windows.
            .scrollable(true)
            .max_h(ceiling);
        // None delegates default selection to the canonical spawn path, even
        // before the selected host's shell inventory has arrived.
        ...
        let alternatives = Self::alternate_shells(&self.shells, &self.default_shell);
        if !alternatives.is_empty() {
            let app = self.app.clone();
            menu = menu.submenu(
                t(L10nKey::TabMenuOtherTerminals),
                window,
                cx,
                move |mut menu, _, _| {
                    for (label, spec) in &alternatives {
                        let app = app.clone();
                        let spec = spec.clone();
                    ...
                    menu.scrollable(true).max_h(ceiling)
                },
            );
        }
        ...
        if !empty {
            ...
            let app = self.app.clone();
            menu = menu.submenu(
                t(L10nKey::MachineSavedConnections),
                window,
                cx,
                move |mut menu, _, _| {
...
impl Tty7App {
    ...
    pub(crate) fn new_tab_button(
        ...
        chrome_tile(Button::new(id).icon(Icon::new(IconName::Plus)), false, cx)
            ...
            .dropdown_menu(move |menu, window, cx| {
                ...
                let rows = this.update(cx, |this, cx| this.new_tab_menu_rows(app.clone(), cx));
                rows.build(menu, window, cx)
            })
    }
    ...
    fn new_tab_menu_rows(&self, app: gpui::WeakEntity<Self>, cx: &mut App) -> NewTabMenu {
        NewTabMenu {
            machines: self.machine_launcher_items(app.clone(), cx),
            app,
            shells: self
                .shells
                .shells
...
mod tests {
    ...
    #[test]
    fn the_new_tab_menu_nests_alternatives_without_repeating_default() {
        let shell = |label: &str| {
            (
        ...
        let default = SharedString::from("zsh");
        assert_eq!(NewTabMenu::alternate_shells(&shells, &default), shells[1..]);
        assert!(NewTabMenu::alternate_shells(&shells[..1], &default).is_empty());
        assert!(NewTabMenu::alternate_shells(&[], &default).is_empty());
        assert_eq!(
            NewTabMenu::alternate_shells(&shells, &SharedString::from("custom")),
            shells
        );
    }
    ...
    #[gpui::test]
    fn live_title_reaches_active_tab_without_overwriting_user_name(cx: &mut TestAppContext) {
        use crate::core::cli_agent::{AgentSessionState, AgentStatus, CLIAgent};
        use crate::daemon::protocol::DaemonMsg;
        use crate::ui::pane::{Pane, PaneSlot};
        use tty7_core::core::machine::{
            AgentFacts, AgentRecoveryBinding, Machine, PaneRecord, ProviderTitle,
        };
        let (app, mut vcx) = crate::ui::app::test_window::harness(cx);
        let (leaf, mut stream) = app.update_in(&mut vcx, |app, window, cx| {
            let (leaf, stream) = crate::terminal::view::quiet_test_pane(71, window, cx);
            app.tabs
                .push(crate::ui::app::Tab::new(Pane::leaf(PaneSlot::Ready(
                    leaf.clone(),
                ))));
            app.active = app.tabs.len() - 1;
            (leaf, stream)
        ...
        .unwrap();
        for _ in 0..200 {
            if leaf.read_with(&vcx, |leaf, _| {
                leaf.agent() == Some(CLIAgent::Codex) && leaf.agent_session().is_some()
            }) {
                break;
            }
        ...
        app.update_in(&mut vcx, |app, window, cx| {
            let record = PaneRecord {
                ...
                provider_title: Some(ProviderTitle {
                    agent: CLIAgent::Codex,
                    session_id: "session-a".into(),
                    title: "官方标题".into(),
                }),
            ...
            let host = leaf.read(cx).host_id();
            crate::ui::machine_mirror::MachineMirrors::install(
                cx,
                host,
                Machine {
                    panes: vec![record.clone()],
                    ..Machine::default()
                },
            );
            let index = app.active;
            assert_eq!(
                app.tab_label(&app.tabs[index], index, Some(window), cx),
                "官方标题"
            );
            app.tabs[index].name = Some("我的名字".into());
            assert_eq!(
                app.tab_label(&app.tabs[index], index, Some(window), cx),
                "我的名字"
            );
            app.tabs[index].name = None;
            let mut replaced = record;
            replaced.agent.as_mut().unwrap().session_id = Some("session-b".into());
            crate::ui::machine_mirror::MachineMirrors::install(
                cx,
                host,
                Machine {
                    panes: vec![replaced.clone()],
                    ..Machine::default()
                },
            );
            assert_ne!(
            ...
            replaced.agent.as_mut().unwrap().session_id = Some("session-a".into());
            crate::ui::machine_mirror::MachineMirrors::install(
                cx,
                host,
                Machine {
            ...
            leaf.update(cx, |leaf, _| {
                leaf.set_workspace(Some(crate::terminal::PaneWorkspace {
                    workspace: crate::core::session::WorkspaceId::new(),
                    target: crate::core::session::RemoteTarget::direct("me", "title.test", 22),
                    spec: None,
                    label: None,
                    resize_echo: false,
                }))
            ...
            assert_ne!(
                app.tab_label(&app.tabs[index], index, Some(window), cx),
                "官方标题",
                "remote pane cannot read a same-ID local mirror title"
            );
        });
    }

    #[gpui::test]
    fn live_title_reaches_exited_tab_only_after_runtime_agrees(cx: &mut TestAppContext) {
        ...
        let (app, mut vcx) = crate::ui::app::test_window::harness(cx);
        let (leaf, mut stream) = app.update_in(&mut vcx, |app, window, cx| {
            let (leaf, stream) = crate::terminal::view::quiet_test_pane(72, window, cx);
            app.tabs
                .push(crate::ui::app::Tab::new(Pane::leaf(PaneSlot::Ready(
                    leaf.clone(),
                ))));
        ...
        app.update_in(&mut vcx, |app, window, cx| {
            assert!(leaf.read(cx).agent().is_none());
            assert!(leaf.read(cx).agent_session().is_none());
            let mut record = PaneRecord {
                agent: Some(AgentFacts {
                    agent: CLIAgent::Codex,
                    session_id: Some("exited-session".into()),
                    launch_argv: None,
                    status: None,
                }),
                recovery_binding: Some(AgentRecoveryBinding {
                    agent: CLIAgent::Codex,
                    session_id: "exited-session".into(),
                    cwd: "/work".into(),
                    launch_argv: None,
                }),
                provider_title: Some(ProviderTitle {
            ...
            app.tabs[index].name = Some("用户命名".into());
            assert_eq!(
                app.tab_label(&app.tabs[index], index, Some(window), cx),
                "用户命名"
            );
        ...
        DaemonMsg::Agent(Some(CLIAgent::Claude))
            ...
            .unwrap();
        for _ in 0..200 {
            if leaf.read_with(&vcx, |leaf, _| leaf.agent() == Some(CLIAgent::Claude)) {
                break;
            }
            std::thread::sleep(std::time::Duration::from_millis(5));
        }
        app.update_in(&mut vcx, |app, window, cx| {
            assert_eq!(leaf.read(cx).agent(), Some(CLIAgent::Claude));
            let index = app.active;
            assert_ne!(
                app.tab_label(&app.tabs[index], index, Some(window), cx),
                "退出后的官方标题",
                "a new foreground provider cannot inherit the old exited name"
            );
        });
    }
    ...
    fn a_pane_with_nothing_to_say_falls_back_the_way_it_always_did() {
        ...
        let tab = strip_tab();
        assert_eq!(label_of(&tab, 0, Some(home())), "agentty");
