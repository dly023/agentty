...
pub(crate) fn set_menus(cx: &mut App) {
    cx.set_menus(application_menus(cx));
}

fn application_menus(cx: &App) -> Vec<Menu> {
    vec![
...
fn window_menu_items(cx: &App) -> Vec<MenuItem> {
    ...
    if items.len() == workspace_start {
        items.push(MenuItem::action(t(L10nKey::AppMenuNewTab), NewTab));
    }
    items
}
...
mod tests {
    ...
    #[gpui::test]
    fn machine_native_menus_exclude_legacy_workspace_mutations(cx: &mut TestAppContext) {
        ...
        cx.update(|cx| {
            ...
            crate::core::session::WorkspaceStore::install_for_test(cx, Default::default());
            let mut names = Vec::new();
            for menu in application_menus(cx) {
                collect(&menu.items, &mut names);
            }
            assert!(names.iter().any(|name| name.ends_with("::NewTab")));
            assert!(names.iter().any(|name| name.ends_with("::OpenSshProfiles")));
            for name in names {
                assert!(
                    ![
                        "NewWorkspace",
                    ...
                    .iter()
                    .any(|retired| name.ends_with(&format!("::{retired}"))),
                    "obsolete native menu action: {name}"
                );
            }
        });
    }
