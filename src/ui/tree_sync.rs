...
pub(crate) fn sync_window(app: &Tty7App, cx: &mut App) {
    // A retained pre-Resume GUI leaf is presentation, not an intent to write
    // the consumed predecessor back over a committed target replacement.
    if app.has_resume_pending(cx) {
        return;
    }
...
#[cfg(test)]
pub(crate) fn tracks_workspace_for_test(cx: &App, client_ws: WorkspaceId) -> bool {
    cx.try_global::<TreeSync>()
        .is_some_and(|tree| tree.windows.contains_key(&client_ws))
}

/// New side effects must not race a layout that is still being hydrated.
pub(crate) fn workspace_is_ready(cx: &App, client_ws: WorkspaceId) -> bool {
    cx.try_global::<TreeSync>()
        .and_then(|t| t.windows.get(&client_ws))
        .is_some_and(|state| state.informed && matches!(&state.sync, SyncPhase::Primed(_)))
}
...
fn start_prime(cx: &mut App, client_ws: WorkspaceId) {
    ...
    cx.spawn(async move |cx| {
        // Generate against the latest machine mirror on the main thread.
        let fresh = cx.update(|cx| fresh_workspace_name(cx, host));
        let outcome = cx
            .background_executor()
...
fn session_pane_from_node(node: &PaneNode, panes: &[PaneRecord]) -> SessionPane {
    match node {
        PaneNode::Leaf { pane } => {
            ...
            if let Some(record) = record.filter(|record| record.exit.is_some()) {
                return SessionPane::Stopped {
                    pane_id: *pane,
                    cwd: record.cwd.clone().map(std::path::PathBuf::from),
...
/// Install a freshly revalidated target snapshot before rendering already
/// attached panes. No pull/create is needed and no empty layout is published.
pub(crate) fn install_prepared_snapshot(
    cx: &mut App,
    client_ws: WorkspaceId,
    machine: Machine,
    target_ws: WorkspaceId,
) -> Session {
    let (machine, mirror, session) =
        layout_of(machine, target_ws).expect("prepared workspace was validated before UI commit");
    let state = cx
        .default_global::<TreeSync>()
        .windows
        .entry(client_ws)
...
fn settle_hydration(
    ...
    let host = WorkspaceStore::host_of(cx, client_ws);
    // The complete target snapshot already carries its authoritative names.
    crate::ui::machine_mirror::MachineMirrors::install(cx, host, machine);
...
/// A verified recovery RPC snapshot may beat the independent Layout event.
/// Update the same projections that event uses, without reentering the window
/// or enqueueing client writes. The caller has checked its proof and generation.
pub(crate) fn acknowledge_recovery(
    ...
    if !WorkspaceStore::all(cx).get(client_ws).is_some_and(|view| {
        view.host_id() == host
            && view.host.as_ref().map_or(client_ws, |r| r.workspace) == machine_ws
    }) || tab
        .root
        .pane_ids()
        .iter()
    ...
    if !state.informed || state.inflight || !state.queue.is_empty() {
        return false;
    }
    ...
    if !apply_to_mirror(mirror, &delta) {
        return false;
    }
    crate::ui::machine_mirror::MachineMirrors::apply_delta(
        cx,
        host,
        &machine_ws.to_string(),
        &delta,
    );
    true
}
...
impl Tty7App {
    pub(crate) fn apply_layout_delta(
        ...
        let applied = match delta {
            ...
            LayoutDelta::TabRestructured { tab, .. } if self.hold_resume_tab(tab.id, cx) => true,
    ...
    pub(super) fn rebuild_tab_from_tree(
        &mut self,
        index: usize,
        tab: &TreeTab,
        window: &mut gpui::Window,
        ...
        *gui.sidebar_group.borrow_mut() = tab.sidebar_group.as_deref().and_then(GroupKey::decode);
        if self.maximized.as_ref().is_some_and(|view| {
            self.tabs.get(self.active).is_none_or(|active| {
                !active
                    .pane
                    .leaves()
                    .iter()
                    .any(|slot| slot.presents_terminal(view.entity_id(), cx))
            })
        }) {
            self.maximized = None;
        }
    ...
    fn build_pane_from_tree(
        ...
        match node {
            PaneNode::Leaf { pane } => {
                ...
                // A missing mapping or remote mirror is not permission to
                // project a same-numbered pane from the Local machine.
                let records = WorkspaceStore::all(cx)
                    .get(self.workspace)
                    .and_then(|entry| {
                        crate::ui::machine_mirror::MachineMirrors::machine(cx, entry.host_id())
                    })
                    .map_or(&[][..], |machine| machine.panes.as_slice());
                let projected = session_pane_from_node(node, records);
                let workspace = self.window_workspace(cx);
                crate::ui::app::session_to_pane(
                    workspace.as_ref(),
                    self.workspace,
                    &projected,
                    None,
...
mod tests {
    ...
    #[cfg(unix)]
    #[gpui::test]
    fn stopped_resume_click_reconciles_exited_successor_over_control(
        cx: &mut gpui::TestAppContext,
    ) {
        resume_click_over_control(cx, false, false, false, false);
    }

    #[cfg(unix)]
    #[gpui::test]
    fn retained_resume_close_window_discards_late_reply(cx: &mut gpui::TestAppContext) {
        resume_click_over_control(cx, false, true, true, false);
    }

    #[cfg(unix)]
    #[gpui::test]
    fn retained_resume_dropped_view_retires_use(cx: &mut gpui::TestAppContext) {
        resume_click_over_control(cx, false, true, true, true);
    }

    #[gpui::test]
    fn rebuild_preserves_only_present_maximized_view(cx: &mut gpui::TestAppContext) {
        let (app, mut vcx, _streams) = crate::ui::app::test_window::harness_with_tabs(cx, 2);
        app.update_in(&mut vcx, |app, window, cx| {
            let second = app.tabs.pop().unwrap().pane;
            let first = std::mem::replace(&mut app.tabs[0].pane, Pane::Empty);
            let maximized = first.leaves()[0].terminal().unwrap().clone();
            app.tabs[0].pane = Pane::split_node(gpui::Axis::Horizontal, 0.4, first, second);
            app.maximized = Some(maximized.clone());
            let mut tab = TreeTab::leaf(1);
            tab.root.split_leaf(1, 2, TreeAxis::Horizontal, 0.4, false);
            app.tabs[0].tree_id.set(tab.id);
            assert!(app.rebuild_tab_from_tree(0, &tab, window, cx));
            assert_eq!(app.maximized.as_ref(), Some(&maximized));
            tab.root = PaneNode::Leaf { pane: 2 };
            assert!(app.rebuild_tab_from_tree(0, &tab, window, cx));
            assert!(
                app.maximized.is_none(),
                "removed presentation must not remain maximized"
            );
        });
    }
    ...
    #[cfg(unix)]
    #[gpui::test]
    fn stopped_resume_click_attaches_running_successor_over_control(cx: &mut gpui::TestAppContext) {
        isolated_running_resume(
            cx,
            "ui::tree_sync::tests::stopped_resume_click_attaches_running_successor_over_control",
            false,
            false,
        );
    }
    ...
    #[cfg(unix)]
    #[gpui::test]
    fn retained_resume_click_attaches_over_control(cx: &mut gpui::TestAppContext) {
        isolated_running_resume(
            cx,
            "ui::tree_sync::tests::retained_resume_click_attaches_over_control",
            true,
            false,
        );
    }
    ...
    #[cfg(unix)]
    #[gpui::test]
    fn retained_resume_close_window_never_attaches_successor(cx: &mut gpui::TestAppContext) {
        isolated_running_resume(
            cx,
            "ui::tree_sync::tests::retained_resume_close_window_never_attaches_successor",
            true,
            true,
        );
    }
    ...
    #[cfg(unix)]
    fn isolated_running_resume(
        cx: &mut gpui::TestAppContext,
        name: &str,
        retained: bool,
        close_window: bool,
    ) {
        const CHILD: &str = "AGENTTY_RUNNING_RESUME_TEST_CHILD";
        ...
        resume_click_over_control(cx, true, retained, close_window, false);
    }

    #[cfg(unix)]
    fn resume_click_over_control(
        cx: &mut gpui::TestAppContext,
        running: bool,
        retained: bool,
        close_window: bool,
        ...
        use tty7_core::daemon::control::{
            AttachmentProof, CONTROL_VERSION, ControlClientMsg, ControlHello, ControlHelloOk,
            ControlReply, ControlServerMsg,
        };
        let initial = crate::ui::session_recovery::tests::stopped_machine();
        let workspace = initial.workspaces[0].id;
        let tab = initial.workspaces[0].tabs[0].clone();
        let mut successor = initial.clone();
        successor.workspaces[0].tabs[0].root.replace_leaf(71, 99);
        successor.panes[0].id = 99;
        successor.panes[0].exit =
            (!running).then_some(tty7_core::core::machine::PaneExit { code: Some(23) });
        successor.panes[0].live = running;
        ...
        if close_window {
            ...
            // Keep the GUI alive: this is window detachment, not application quit.
            let other = cx.add_window(|window, cx| {
                ...
                let other_workspace = other.read(cx).workspace;
                ...
                gpui_component::Root::new(other, window, cx)
            });
            let weak = app.downgrade();
            app.update_in(&mut vcx, |app, window, cx| {
                WindowRegistry::register(cx, app.workspace, window.window_handle(), weak);
            });
            assert!(other.update(cx, |_, _, _| ()).is_ok());
            assert_eq!(cx.update(|cx| WindowRegistry::count(cx)), 2);
        }
        let (pane_check_tx, pane_check_rx) = std::sync::mpsc::channel();
        let pane_server = running.then(|| {
            use tty7_core::daemon::{protocol::*, router::*, transport};
            // The child process has pinned this path before any transport call.
            let dir = std::env::temp_dir().join(format!("tty7-covtest-{}", std::process::id()));
            assert_eq!(crate::core::config::config_dir_path().unwrap(), dir);
            assert_eq!(transport::endpoint_display(), dir.join("daemon.sock").display().to_string());
            let listener = transport::bind().unwrap(); // Never remove a pre-existing endpoint.
            ...
            let guard = CheckedPaneAttachment { identity: PaneAttachExpectation::Owned, proof: proof.clone(), tab: tab.id };
            std::thread::spawn(move || {
                let endpoint = endpoint;
                if close_window {
                    pane_check_rx.recv_timeout(Duration::from_secs(10)).unwrap();
                    assert!(matches!(listener.accept(), Err(e) if e.kind() == std::io::ErrorKind::WouldBlock),
                        "closed window attempted a pane connection");
                    return (None, listener, endpoint);
                }
                ...
                let accept = || loop {
                    match listener.accept() {
                        ...
                        Err(e) if e.kind() == std::io::ErrorKind::WouldBlock => {
                            assert!(Instant::now() < deadline, "no routed pane connection");
                ...
                for probe in [true, false] {
                    ...
                    // Darwin can inherit the listener's O_NONBLOCK flag.
                    stream.set_nonblocking(false).unwrap();
                    stream.set_read_timeout(Some(Duration::from_secs(10))).unwrap();
                    stream.set_write_timeout(Some(Duration::from_secs(10))).unwrap();
                    let (kind, payload) = read_frame(&mut stream).unwrap();
                    assert_eq!(kind, ROUTE_KIND);
                    let route = RouteHeader::decode(&payload).unwrap();
                    assert_eq!(route.channel, RouteChannel::Pane);
                    assert_eq!(route.action, RouteAction::Forward);
                    assert!(matches!(route.target, RouteTarget::LocalStdio { ref program, ref args } if program == "/__agentty_resume_fixture__" && args == &["--pane"]));
                    write_frame(&mut stream, ROUTE_KIND, &serde_json::to_vec(&RouteAck { ok: true, link: Some("fixture".into()), action: None, error: None }).unwrap()).unwrap();
                    let message = ClientMsg::read(&mut stream).unwrap();
                    if probe {
                        assert_eq!(message, ClientMsg::Version);
                        DaemonMsg::Version(DaemonVersion::current()).encode(&mut stream).unwrap();
                    } else {
                        let ClientMsg::AttachChecked { pane_id, guard: actual, size, .. } = message else { panic!("expected checked attachment, got {message:?}") };
                        assert_eq!(pane_id, 99);
                        assert_eq!(actual, guard);
                        DaemonMsg::Size(size).encode(&mut stream).unwrap();
                        DaemonMsg::Output(b"resumed-fixture\r\n".to_vec()).encode(&mut stream).unwrap();
                        attached = Some(stream);
                    }
                }
                (Some(attached.unwrap()), listener, endpoint)
            })
        });
        ...
        let server = std::thread::spawn(move || {
            ...
            ControlServerMsg::HelloOk(ControlHelloOk {
                ...
                build: "isolated-resume-peer".into(),
                separator: '/',
                home: "/fixture".into(),
                features: vec![
                    tty7_core::daemon::protocol::FEATURE_CHECKED_ATTACH.into(),
                    tty7_core::daemon::control::feature::MACHINE_TREE.into(),
                ],
                instance: "resume-control-test".into(),
            })
            .encode(&mut peer)
            .unwrap();
            let mut resumed = false;
            let mut leased = false;
            while let Ok(message) = ControlClientMsg::read(&mut peer) {
                let ControlClientMsg::Request { req_id, req } = message else {
                    panic!("unexpected control message")
                };
                peer_calls.lock().unwrap().push(req.clone());
                let retiring = matches!(&req, ControlRequest::WorkspaceReleaseProof { .. });
                let querying_proof =
                    matches!(&req, ControlRequest::WorkspaceAttachmentProof { .. });
                let reply = match req {
                    ControlRequest::WorkspaceAttachIfFree { id } => {
                        assert_eq!(id, workspace.to_string());
                        assert!(!leased);
                        leased = true;
                        ReplyOk::WorkspaceAcquired {
                            proof: peer_proof.clone(),
                            newly_acquired: true,
                        }
                    }
                    ControlRequest::ResumeStoppedSession { request } => {
                        assert!(!resumed, "second launch");
                        assert_eq!(request.proof, peer_proof);
                        assert_eq!(request.tab, peer_tab);
                        assert_eq!(request.pane, 71);
                        assert_eq!(
                            request.identity.id,
                            successor.panes[0]
                                .recovery_binding
                        ...
                        assert_eq!(request.identity.cwd, "/retained");
                        resumed = true;
                        if retained {
                    ...
                    ControlRequest::MachineGet => {
                        assert!(resumed, "unexpected pre-launch source query");
                        ReplyOk::MachineTree(Box::new(successor.clone()))
                    }
                    ControlRequest::WorkspaceAttachmentProof {
                        workspace: requested,
                    } => {
                        assert_eq!(requested, workspace);
                        ReplyOk::AttachmentProof(peer_proof.clone())
                    }
                    ControlRequest::Ping => ReplyOk::Pong,
                    ControlRequest::WorkspaceReleaseProof { proof } => {
                        assert!(close_window && leased, "unexpected or duplicate retirement");
                        assert_eq!(proof, peer_proof);
                        leased = false;
                        ReplyOk::Unit
                    }
                    other => panic!("unexpected side effect: {other:?}"),
                };
                if (ControlServerMsg::Response {
                    req_id,
                    reply: if querying_proof && !leased {
                        ControlReply::Err(tty7_core::daemon::control::WireError::from_io(
                            &std::io::Error::new(
                                std::io::ErrorKind::PermissionDenied,
                                "released fixture claim",
                            ),
                        ))
                    } else {
                        ControlReply::Ok(reply)
                    },
                })
                .encode(&mut peer)
                .is_err()
            ...
            assert!(resumed, "button did not submit Resume");
            assert!(!close_window || !leased, "window left its tracked use held");
        });
        let target = if running {
            RemoteTarget::LocalStdio {
                program: "/__agentty_resume_fixture__".into(),
                args: vec![],
            }
        } else {
            RemoteTarget::direct("fixture", "isolated-resume.test", 22)
        };
        let remote = tty7_core::host::remote::RemoteHost::over_unix(
            socket,
            &target.connection_key(),
            &ControlHello::gui("resume-click", "fixture"),
        )
        .unwrap();
        let control = remote.client().clone();
        let host = target.host_id();
        let mut retained_peer = None;
        let mut retained_view = None;
        let mut pending = app.update_in(&mut vcx, |app, window, cx| {
            WorkspaceStore::install_for_test(
                cx,
                WindowViews {
                    views: vec![WindowView {
                        id: app.workspace,
                        ..WindowView::on_remote(RemoteRef::new(target, workspace))
                    }],
                    active: Some(app.workspace),
                },
            );
            crate::ui::remote_connect::HostLinks::insert(cx, remote, "/fixture".into());
            crate::ui::machine_mirror::MachineMirrors::install(cx, host, initial.clone());
            cx.default_global::<TreeSync>().windows.insert(
                app.workspace,
                WsState {
                    sync: SyncPhase::Primed(WsMirror {
                        tabs: vec![tab.clone()],
                        active: Some(tab.id),
                    }),
                    informed: true,
            ...
            app.tabs[0].tree_id.set(tab.id);
            if retained {
                let (mut parts, mut peer) = crate::terminal::view::quiet_test_shell_parts(71);
                parts.owner = Some(app.workspace);
                parts.workspace = app.window_workspace(cx);
                tty7_core::daemon::protocol::DaemonMsg::Output(b"retained-output\r\n".to_vec())
                    .encode(&mut peer)
                    .unwrap();
                tty7_core::daemon::protocol::DaemonMsg::Exited { code: Some(0) }
                    .encode(&mut peer)
                    .unwrap();
                let view = cx.new(|cx| {
                    crate::terminal::view::TerminalView::from_shell_parts(parts, window, cx)
                });
                app.tabs[0].pane = Pane::leaf(PaneSlot::Ready(view.clone()));
                app.maximized = Some(view.clone());
                window.focus(&view.read(cx).focus_handle.clone(), cx);
                retained_peer = Some(peer);
                retained_view = Some(view);
                cx.notify();
                return None;
            }
            let session = session_from_tree(&initial.workspaces[0], &initial.panes);
            let pane = crate::ui::app::session_to_pane(
                None,
                app.workspace,
                &session.tabs[0].pane,
                None,
        ...
        if let Some(view) = &retained_view {
            ...
            while !view.read_with(&vcx, |view, _| view.terminal.child_exited()) {
                assert!(Instant::now() < deadline, "fixture exit was not observed");
                std::thread::sleep(Duration::from_millis(10));
            }
        ...
        let button = vcx
            .debug_bounds(if retained {
                "retained-pane-resume"
            } else {
                "stopped-pane-resume"
            })
            .unwrap();
        ...
        if retained {
            ...
            loop {
                ...
                assert!(
                    ...
                    "retained click did not submit Resume"
                );
                std::thread::sleep(Duration::from_millis(10));
            }
            pending = Some(app.read_with(&vcx, |app, cx| {
                let PaneSlot::Connecting(pending) = &app.tabs[0].pane.leaves()[0] else {
                    panic!("reply not released yet");
                };
                assert_eq!(
                    pending.read(cx).retained_view.as_ref(),
                    retained_view.as_ref()
                );
                assert_eq!(app.maximized.as_ref(), retained_view.as_ref());
                let original = retained_view
                    .as_ref()
                    .unwrap()
                    .read(cx)
                    .terminal
                ...
                let output: String = (0..15)
                    .map(|col| {
                        original.grid()[alacritty_terminal::index::Line(0)]
                            [alacritty_terminal::index::Column(col)]
                        .c
                    })
                    .collect();
                assert_eq!(output, "retained-output");
                assert!(app.has_resume_pending(cx));
                pending.clone()
            }));
            if close_window {
                let client_workspace = app.read_with(&vcx, |app, _| app.workspace);
                let pending = pending.take().unwrap();
                let weak_pending = pending.downgrade();
                if drop_view {
                    // Simulate a caller removing the view without the close action.
                    // This case tests ownership retirement, not bookmark cleanup.
                    vcx.update(|window, _| window.remove_window());
                } else {
                    vcx.dispatch_action(crate::core::actions::CloseWindow);
                }
                cx.update(|cx| {
                    assert_eq!(cx.windows().len(), 1, "the other window must remain open");
                    if !drop_view {
                        assert_eq!(crate::ui::windows::WindowRegistry::count(cx), 1);
                        assert!(
                            !cx.global::<TreeSync>()
                                .windows
                                .contains_key(&client_workspace)
                        );
                    }
                });
                drop(pending);
                ...
                // The coordinator owns PendingPane until it processes the reply.
                // Its release is a completion barrier, not a guessed sleep.
                let deadline = Instant::now() + Duration::from_secs(10);
                loop {
                    vcx.run_until_parked();
                    // Flush deferred entity destruction at an App update boundary.
                    cx.update(|_| ());
                    ...
                    assert!(
                        Instant::now() < deadline,
                        "closed view retained by recovery; requests: {:?}",
                        calls.lock().unwrap()
                    );
                    std::thread::sleep(Duration::from_millis(10));
                }
                cx.update(|cx| {
                    ...
                    if !drop_view {
                        assert!(
                            !WorkspaceStore::all(cx).get(client_workspace).unwrap().open,
                            "closed view must remain saved for an explicit reopen"
                        );
                        assert!(
                            !cx.global::<TreeSync>()
                                .windows
                    ...
                    let machine =
                        crate::ui::machine_mirror::MachineMirrors::machine(cx, host).unwrap();
                    assert_eq!(
                        machine.workspaces[0].tabs[0].root,
                        PaneNode::Leaf { pane: 71 }
                    );
                });
                let deadline = Instant::now() + Duration::from_secs(5);
                loop {
                    vcx.run_until_parked();
                    cx.update(|_| ());
                    if retired_rx.try_recv().is_ok() {
                        break;
                    }
                    assert!(
                        Instant::now() < deadline,
                        "removed view did not retire its workspace use"
                    );
                    std::thread::sleep(Duration::from_millis(10));
                }
                control.close();
                server.join().unwrap();
                if let Some(server) = pane_server {
                    pane_check_tx.send(()).unwrap();
                    let (stream, listener, endpoint) = server.join().unwrap();
                ...
                let requests: Vec<_> = calls
                    ...
                    .unwrap()
                    .iter()
                    .filter(|r| !matches!(r, ControlRequest::Ping))
                    .cloned()
                    .collect();
                assert_eq!(
                    requests
                        .iter()
                        .filter(|r| matches!(r, ControlRequest::WorkspaceReleaseProof { .. }))
                        .count(),
                    1
                );
                ...
                assert!(
                    matches!(
                        ...
                        [
                            ...
                            ControlRequest::ResumeStoppedSession { .. },
                            ControlRequest::MachineGet,
                            ControlRequest::WorkspaceAttachmentProof { .. },
                        ]
                    ),
                    "unexpected close side effects: {requests:?}"
                );
                return;
            }
            release_tx.send(()).unwrap();
        ...
        loop {
            vcx.run_until_parked();
            let reconciled = pending.read_with(&vcx, |p, _| {
                p.spawn.restore_pane == Some(99) && p.resume.is_none()
            });
            let landed = !running
                || app.read_with(&vcx, |app, cx| {
                    let PaneSlot::Ready(view) = &app.tabs[0].pane.leaves()[0] else {
                        return false;
                    };
                    let view = view.read(cx);
                    ...
                    let output: String = (0..15)
                        ...
                        .collect();
                    view.pane_id == 99 && output == "resumed-fixture"
                });
            ...
            if Instant::now() >= deadline {
                control.close();
                server.join().unwrap();
                panic!("successful RPC did not reconcile original slot");
            }
            std::thread::sleep(Duration::from_millis(10));
        }
        app.update_in(&mut vcx, |app, window, cx| {
            assert_eq!(app.tabs.len(), 1);
            assert_eq!(app.tabs[0].tree_id.get(), tab.id);
            if !running {
                assert_eq!(
                ...
                assert!(matches!(
                    pending.read(cx).state,
                    crate::ui::pending_pane::PendingState::Stopped
                ));
            }
            let SyncPhase::Primed(mirror) = &cx.global::<TreeSync>().windows[&app.workspace].sync
            else {
                panic!("baseline missing")
            };
            assert_eq!(mirror.tabs[0].root, PaneNode::Leaf { pane: 99 });
            assert!(
                cx.global::<TreeSync>().windows[&app.workspace]
                    .queue
                    .is_empty()
            );
            assert!(!app.has_resume_pending(cx));
            if retained {
                let new = app.tabs[0].pane.leaves()[0].terminal().unwrap().clone();
                assert_eq!(app.maximized.as_ref(), Some(&new));
                assert!(new.read(cx).focus_handle.contains_focused(window, cx));
            }
        });
        assert_eq!(vcx.debug_bounds("stopped-pane-resume").is_some(), !running);
        if let Some(server) = pane_server {
            let (stream, listener, endpoint) = server.join().unwrap();
            let mut stream = stream.expect("running successor must attach");
            stream
                .set_read_timeout(Some(Duration::from_millis(50)))
                .unwrap();
            while let Ok(message) = tty7_core::daemon::protocol::ClientMsg::read(&mut stream) {
                assert!(
                    matches!(message, tty7_core::daemon::protocol::ClientMsg::Resize(_)),
                    "unexpected post-attach effect: {message:?}"
                );
            }
        ...
        assert_eq!(
            calls
                .iter()
                .filter(|r| matches!(r, ControlRequest::ResumeStoppedSession { .. }))
                .count(),
            1
        );
        assert!(matches!(
        ...
        assert!(matches!(calls[2], ControlRequest::MachineGet));
        assert!(matches!(
            calls[3],
            ControlRequest::WorkspaceAttachmentProof { .. }
        ));
    ...
    #[gpui::test]
    fn resume_snapshot_updates_sync_mirror_before_releasing_gui_hold(
        cx: &mut gpui::TestAppContext,
    ) {
        let (app, mut vcx, _streams) = crate::ui::app::test_window::harness_with_tabs(cx, 1);
        app.update_in(&mut vcx, |app, _, cx| {
            ...
            let old = TreeTab::leaf(71);
            let mut next = old.clone();
            next.root.replace_leaf(71, 99);
            ...
            assert!(!acknowledge_recovery(
                cx,
                ws,
                HostId::from_connection_key("foreign"),
                ws,
                &next,
                &record
            ));
            ...
            let SyncPhase::Primed(mirror) = &cx.global::<TreeSync>().windows[&ws].sync else {
                panic!("lost baseline")
            };
            assert_eq!(
                mirror.tabs[0], next,
                "next save still sees a consumed predecessor"
            );
            assert!(
                cx.global::<TreeSync>().windows[&ws].queue.is_empty(),
                "acknowledgment must not write to the target"
            );
            let facts =
                crate::ui::machine_mirror::MachineMirrors::machine(cx, HostId::LOCAL).unwrap();
            assert_eq!(facts.workspaces[0].tabs[0], next);
            assert!(facts.panes.iter().any(|p| p.id == 99));
            // The independently delivered event is idempotent afterward.
            let delta = LayoutDelta::TabRestructured {
                tab: next.clone(),
                pane: Some(record),
            };
            let SyncPhase::Primed(mirror) = &mut cx
                .default_global::<TreeSync>()
                .windows
                .get_mut(&ws)
                .unwrap()
            ...
            assert!(apply_to_mirror(mirror, &delta));
            assert_eq!(mirror.tabs[0], next);
        });
    }

    #[gpui::test]
    fn incremental_stopped_hydration_uses_target_facts_and_reuses_views(
        cx: &mut gpui::TestAppContext,
    ) {
        use crate::core::session::{RemoteRef, RemoteTarget, WindowView, WindowViews};
        use crate::ui::machine_mirror::MachineMirrors;
        use crate::ui::pending_pane::PendingState;
        use tty7_core::core::machine::PaneExit;
        let (app, mut vcx, _streams) = crate::ui::app::test_window::harness_with_tabs(cx, 3);
        app.update_in(&mut vcx, |app, window, cx| {
            let target = RemoteTarget::Alias {
                alias: "unroutable-delta-fixture".into(),
            };
            let host = target.host_id();
            let view = WindowView {
                id: app.workspace,
                ..WindowView::on_remote(RemoteRef::new(target, WorkspaceId::new()))
            };
            WorkspaceStore::install_for_test(
                cx,
                WindowViews {
            ...
            let tab = TreeTab {
                id: TabId::new(),
                name: Some("cold delta".into()),
                sidebar_group: None,
                root: PaneNode::Leaf { pane: 71 },
            };
            let active_id = app.tabs[app.active].tree_id.get();
            assert!(app.insert_tab_from_tree(0, &tab, window, cx));
            assert_eq!(app.tabs[0].tree_id.get(), tab.id);
            assert_eq!(app.tabs[app.active].tree_id.get(), active_id);
            let cold = app.tabs[0].pane.leaves()[0].clone();
            let PaneSlot::Connecting(pending) = &cold else {
                panic!("cold slot")
            };
            assert!(
                matches!(pending.read(cx).state, PendingState::Stopped),
                "delta discarded target exit"
            );
            assert_eq!(
                pending.read(cx).spawn.working_directory.as_deref(),
                Some(std::path::Path::new("/remote"))
            );
            assert!(app.insert_tab_from_tree(0, &tab, window, cx));
            assert_eq!(app.tabs.len(), 4, "duplicate delta is idempotent");
            let existing = app.tabs[1].pane.leaves()[0].clone();
            let existing_id = existing.terminal().unwrap().read(cx).pane_id;
            let node = PaneNode::Split {
                axis: TreeAxis::Horizontal,
                ratio: 0.4,
                a: Box::new(PaneNode::Leaf { pane: 71 }),
                b: Box::new(PaneNode::Leaf { pane: existing_id }),
            };
            let mut reused = [(existing_id, existing.clone())].into();
            let mixed = app
                .build_pane_from_tree(&node, &mut reused, window, cx)
                .unwrap();
            assert_eq!(mixed.leaves()[1].entity_id(), existing.entity_id());
            assert!(reused.is_empty());
            let Pane::Split { ratio, .. } = mixed else {
                panic!("split")
            };
            ...
            let rebuilt = TreeTab {
                root: PaneNode::Split {
                    ...
                    ratio: 0.6,
                    a: Box::new(PaneNode::Leaf { pane: 71 }),
                    b: Box::new(PaneNode::Leaf { pane: 72 }),
                },
                ..tab.clone()
            };
            assert!(app.rebuild_tab_from_tree(0, &rebuilt, window, cx));
            assert_eq!(app.tabs[0].pane.leaves()[0].entity_id(), cold.entity_id());
            let retained: Vec<_> = app.tabs[0]
                .pane
                .leaves()
                .iter()
                .map(PaneSlot::entity_id)
                .collect();
            assert!(app.rebuild_tab_from_tree(0, &rebuilt, window, cx));
            assert_eq!(
                app.tabs[0]
            ...
            assert_eq!(app.tabs[app.active].tree_id.get(), active_id);
            // A Local exit with the same numeric ID cannot fill missing remote facts.
            MachineMirrors::install(cx, host, Machine::default());
            assert!(app.rebuild_tab_from_tree(0, &rebuilt, window, cx));
            assert_eq!(
                app.tabs[0]
                    .pane
                    ...
                    .collect::<Vec<_>>(),
                retained,
                "missing fresh facts cannot discard retained views"
            );
            let unknown = app
                .build_pane_from_tree(&tab.root, &mut HashMap::new(), window, cx)
                .unwrap();
            let PaneSlot::Connecting(pending) = &unknown.leaves()[0] else {
                panic!("unroutable fixture")
            };
            assert!(!matches!(pending.read(cx).state, PendingState::Stopped));
            assert_eq!(pending.read(cx).spawn.working_directory, None);
            assert_eq!(
                pending.read(cx).spawn.restore_policy,
                crate::terminal::RestorePolicy::AttachOnly
            );
        });
    }
    ...
    #[test]
    fn confirmed_stopped_tree_leaf_is_not_a_spawn_candidate() {
        let mut record = PaneRecord::new(71);
        record.exit = Some(tty7_core::core::machine::PaneExit { code: None });
        record.cwd = Some("/retained".into());
        let projected = session_pane_from_node(&PaneNode::Leaf { pane: 71 }, &[record]);
        assert!(
            !matches!(projected, SessionPane::Leaf { .. }),
            "confirmed target exit must survive lowering as a no-spawn leaf"
        );
        let SessionPane::Stopped { pane_id, cwd, .. } = projected else {
            panic!("stopped leaf")
        };
        assert_eq!(pane_id, 71);
        assert_eq!(cwd.as_deref(), Some(std::path::Path::new("/retained")));
    }

    #[test]
    fn explicit_exit_is_not_auto_resumable() {
        use crate::core::cli_agent::CLIAgent;
        use crate::ui::pending_pane::PendingSpawn;
        use tty7_core::core::machine::AgentRecoveryBinding;
        let record = PaneRecord {
            agent: None,
        ...
        let mut decoded: PaneRecord = serde_json::from_slice(&encoded).unwrap();
        for live in [true, false] {
            decoded.live = live;
            let projected =
                session_pane_from_node(&PaneNode::Leaf { pane: 17 }, &[decoded.clone()]);
            ...
            assert_eq!(pane_id, Some(17), "keep the original attach candidate");
            ...
            assert_eq!(
                ...
                None,
                "even enabled auto restore cannot launch a persisted exited binding"
            );
            assert!(
                decoded.recovery_binding.is_some(),
                "manual recovery identity is retained"
            );
        }
    }
    ...
    #[gpui::test]
    fn machine_prime_names_follow_current_target(cx: &mut gpui::TestAppContext) {
        exercise_machine_name_replies(cx, false);
    }

    #[gpui::test]
    fn machine_hydration_names_follow_current_target(cx: &mut gpui::TestAppContext) {
        exercise_machine_name_replies(cx, true);
    }

    fn exercise_machine_name_replies(cx: &mut gpui::TestAppContext, hydrate: bool) {
        use crate::core::session::{RemoteRef, RemoteTarget, WindowView, WindowViews};
        use crate::ui::machine_mirror::MachineMirrors;

        for remote in [false, true] {
            for (current, failed, removed, answered) in [
                (true, false, false, Some("target-name")),
                (true, false, false, None),
                (false, false, false, Some("stale-name")),
                (false, true, false, None),
                (true, true, false, None),
                (true, false, true, Some("forgotten-name")),
            ] {
                cx.update(|cx| {
                    crate::ui::windows::WindowRegistry::init(cx);
                    let view = if remote {
                        WindowView::on_remote(RemoteRef::new(
                            RemoteTarget::Alias { alias: "name-fixture".into() },
                            WorkspaceId::new(),
                        ))
                    } else {
                        WindowView::default()
                    };
                    let client_ws = view.id;
                    let host = view.host_id();
                    WorkspaceStore::install_for_test(cx, WindowViews {
                        views: vec![view], active: Some(client_ws),
                    });
                    let machine_ws = tree_workspace_id(cx, client_ws);
                    let before = Machine {
                        workspaces: vec![Workspace {
                            id: machine_ws, name: Some("committed-name".into()),
                            ..Default::default()
                        }],
                        panes: Vec::new(),
                    };
                    MachineMirrors::install(cx, host, before.clone());
                    let state = cx.default_global::<TreeSync>().windows.entry(client_ws).or_default();
                    state.epoch = if current { 5 } else { 6 };
                    state.sync = SyncPhase::Unprimed { dirty: false, priming: true };
                    if removed {
                        cx.default_global::<TreeSync>().windows.remove(&client_ws);
                    }
                    if hydrate {
                        let outcome = if failed {
                            Err(io::Error::other("name fixture failure"))
                        } else {
                            let mut incoming = before.clone();
                            incoming.workspaces[0].name = answered.map(str::to_string);
                            Ok((incoming, WsMirror::default(), Session::default()))
                        };
                        settle_hydration(cx, client_ws, 5, Adopt::IfEmpty, outcome);
                    } else {
                        let outcome = if failed {
                            Err(io::Error::other("name fixture failure"))
                        } else {
                            Ok((WsMirror::default(), answered.map(str::to_string)))
                        };
                        finish_prime(cx, client_ws, 5, outcome);
                    }
                    let machine = MachineMirrors::machine(cx, host).unwrap();
                    let expected = if current && !failed && !removed {
                        answered
                    } else {
                        Some("committed-name")
                    };
                    assert_eq!(machine.workspaces.len(), 1, "no extra target layout");
                    assert_eq!(machine.workspaces[0].id, machine_ws, "target identity retained");
                    assert_eq!(machine.workspaces[0].name.as_deref(), expected,
                        "hydrate={hydrate} remote={remote} current={current} failed={failed} removed={removed}");
                    assert!(machine.panes.is_empty(), "name readback cannot create a pane");
                    if let Some(state) = cx.default_global::<TreeSync>().windows.get(&client_ws) {
                        assert!(state.queue.is_empty(), "readback cannot queue a rename");
                    } else {
                        assert!(removed, "a failed readback cannot discard a current carrier");
                    }
                });
            }
        }
    ...
    /// Target name readback survives retirement of the old form's override.
    #[gpui::test]
    fn a_window_owing_no_name_still_reads_the_machines(cx: &mut gpui::TestAppContext) {
        cx.update(|cx| {
            let (ws, view) = primed_window(cx);
    ...
    /// A window mid-prime with a target mirror ready to receive its name.
    fn primed_window(cx: &mut App) -> (WorkspaceId, crate::core::session::WindowView) {
        tty7_core::core::config::set_config_dir(
            std::env::temp_dir().join(format!("tty7-target-name-{}", std::process::id())),
        );
