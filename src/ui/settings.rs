...
impl Tty7App {
    ...
    fn delete_profile_confirmed(&mut self, id: Uuid, cx: &mut Context<Self>) {
        ...
        for path in mine.iter().filter(|p| !kept.contains(*p)) {
            ...
            let Ok(account) = crate::core::keychain::key_account_from_local_file(path) else {
                continue;
            };
...
mod tests {
    ...
    #[test]
    fn machine_settings_retire_directory_grouping() {
        for query in ["sidebar grouping", "by repo or folder"] {
            assert!(
                best_matching_section(query).is_none(),
                "obsolete setting: {query}"
            );
        }
        for key in [
