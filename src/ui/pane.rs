...
impl PaneSlot {
    /// Presentation identity, including an old screen held during recovery.
    /// This must never be used as proof of a live process or target ownership.
    pub(crate) fn presents_terminal(&self, id: gpui::EntityId, cx: &App) -> bool {
        match self {
            Self::Ready(view) => view.entity_id() == id,
            Self::Connecting(pending) => pending
                .read(cx)
                .retained_view
                .as_ref()
                .is_some_and(|view| view.entity_id() == id),
        }
    }
