...
use crate::core::desktop_files::{self, CopyError};
...
pub(crate) fn copy_into_dir(
    ...
    for src in sources {
        ...
        if crate::core::path_drag::same_local_tree(host.id(), src, dir) {
        ...
        // Remote internal drags have already been rejected by the consumer.
        if !desktop_files::source_exists(src) {
...
fn copy_over(host: &dyn Host, src: &Path, dir: &Path, dest: &Path, name: &str) -> io::Result<()> {
    ...
    if let Err(e) = copy_tree(host, src, &staged) {
...
fn copy_tree(host: &dyn Host, src: &Path, dest: &Path) -> io::Result<()> {
    desktop_files::copy_to_host(host, src, dest, MAX_DEPTH, REMOTE_FILE_MAX).map_err(|error| {
        match error {
            CopyError::Io(error) => error,
            CopyError::TooDeep => io::Error::other(t_fmt(
                L10nKey::FileDropTooDeep,
                &[("n", &MAX_DEPTH.to_string())],
            )),
            CopyError::TooLarge => io::Error::other(t_fmt(
