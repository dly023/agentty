...
use crate::core::path_drag::TreePathDrag;
...
impl Tty7App {
    ...
    fn file_tree_drop_tree_paths(
        ...
        let Some(paths) = drag.local_sources() else {
            window.push_notification(t(L10nKey::FileDropRemoteSource), cx);
            return;
        };
        self.file_tree_drop_paths(paths.to_vec(), dir, window, cx);
    }
...
impl Tty7App {
    pub(crate) fn render_file_tree_rows(
        ...
        let column = v_flex()
            ...
            .when_some(roots.first().cloned(), |d, root| {
                d.on_drop(cx.listener({
                    let root = root.clone();
                    move |this, paths: &TreePathDrag, window, cx| {
                        this.file_tree_drop_tree_paths(paths, root.clone(), window, cx);
                    }
                }))
    ...
    fn render_tree_row(
                ...
                h_flex()
                    ...
                    .when(!path.as_os_str().is_empty(), |d| {
                        d.on_drop(cx.listener({
                            let path = path.clone();
                            move |this, paths: &TreePathDrag, window, cx| {
                                this.file_tree_drop_tree_paths(paths, path.clone(), window, cx);
                            }
                        }))
                        .drag_over::<ExternalPaths>(|s, _, _, cx| {
        ...
        let row_el = h_flex()
            ...
            .drag_over::<TreePathDrag>(|s, _, _, cx| s.bg(cx.theme().drag_border.opacity(0.14)))
            .on_drop(cx.listener({
                let dir = if is_dir {
                    path.clone()
                } else {
                ...
                move |this, paths: &TreePathDrag, window, cx| {
                    this.file_tree_drop_tree_paths(paths, dir.clone(), window, cx);
                }
            }))
            .on_drop(cx.listener({
...
mod drop_gpui_tests {
    ...
    #[gpui::test]
    fn remote_tree_drag_never_copies_a_local_same_named_file(cx: &mut TestAppContext) {
        let _guard = serial();
        let root = tempfile::tempdir().unwrap();
        let from = tempfile::tempdir().unwrap();
        let source = from.path().join("note.txt");
        std::fs::write(&source, "local lookalike must stay local").unwrap();
        let (app, mut vcx, _streams) = files_panel_on(cx, root.path());
        app.update_in(&mut vcx, |app, window, cx| {
            let drag = TreePathDrag {
                host: HostId::from_connection_key("remote-source"),
                paths: vec![source],
            };
            app.file_tree_drop_tree_paths(&drag, root.path().to_path_buf(), window, cx);
        });
