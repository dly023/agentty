...
mod tests {
    ...
    fn sidebar_surface() -> Surface {
        Surface::SidebarRows {
            tabs: vec![TabId::new(), TabId::new()],
            visible: vec![0, 1],
        }
    }

    #[test]
    fn sidebar_drag_rejects_changed_membership() {
        let tabs = vec![TabId::new(), TabId::new(), TabId::new()];
        let original = Surface::SidebarRows {
            tabs: tabs.clone(),
            visible: vec![0, 2],
        };
        let mut drag = column(2, 30., 2., 0);
        drag.surface = original.clone();
        let state = Rc::new(RefCell::new(Some(drag)));
        let pointer = point(px(100.), px(90.));
        assert!(preview(&state, &original, 2, pointer).is_some());
        let mut replaced = tabs.clone();
        replaced[1] = TabId::new();
        let mut swapped = tabs.clone();
        swapped.swap(0, 2);
        for changed in [
            Surface::SidebarRows {
                tabs: tabs.clone(),
            ...
            clear_pending(&state);
            assert!(preview(&state, &changed, 2, pointer).is_none());
            set_pending(&state, &changed, vec![2, 1, 0]);
            assert!(state.borrow().as_ref().unwrap().pending.borrow().is_none());
        }
        set_pending(&state, &original, vec![1, 2, 0]);
        assert_eq!(take_landed(&state).order, Some(vec![1, 2, 0]));
    }
    ...
    fn unequal_sizes_swap_in_both_directions() {
        ...
        let short = Reorder::new(sidebar_surface(), 0, rects, Axis::Vertical, px(2.), grab);
    ...
    fn pending_only_survives_the_frame_that_recorded_it() {
        ...
        set_pending(&state, &sidebar_surface(), vec![2, 1, 0]);
