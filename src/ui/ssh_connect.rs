...
use crate::core::keychain::{CredentialStore, OsCredentialStore, key_account_from_local_file};
...
fn build_spec_inner(
    ...
    if matches!(profile.auth, AuthMode::Auto | AuthMode::PublicKey) {
        ...
        for path in &probed {
            let Ok(account) = key_account_from_local_file(path) else {
                continue;
            };
...
mod tests {
    ...
    fn a_tilde_and_an_absolute_identity_path_resolve_the_same_stored_passphrase() {
        ...
        let store = InMemoryCredentialStore::new();
        let account =
            crate::core::keychain::key_account_from_contents(&std::fs::read(&key).unwrap());
