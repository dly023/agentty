...
impl Command {
    ...
    pub fn base_commands(cx: &App, chrome: ChromeState) -> Vec<Command> {
        ...
        let workspaces = [Command::localized(
...
mod gpui_tests {
    ...
    #[gpui::test]
    fn machine_palette_excludes_legacy_workspace_mutations(cx: &mut TestAppContext) {
        crate::core::config::pin_test_config_dir();
        cx.update(|cx| {
            cx.set_global(Config::default());
            crate::ui::i18n::set_locale("en");
            let commands = Command::base_commands(
                cx,
                ChromeState {
            ...
            let ids: Vec<_> = commands.iter().filter_map(|cmd| cmd.kind.id()).collect();
            assert!(
                ids.contains(&"new-tab"),
                "session creation must remain available"
            );
            let obsolete: Vec<_> = ids
                .iter()
                .copied()
                .filter(|id| {
                    matches!(
            ...
            assert!(
                obsolete.is_empty(),
                "machine-only palette still exposes workspace mutations: {obsolete:?}"
            );
        });
    }
