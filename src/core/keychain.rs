...
/// Account identity for a client-owned SSH key. Keep raw key bytes out of GUI
/// callers; target workspace paths must never be passed to this local API.
pub fn key_account_from_local_file(path: impl AsRef<std::path::Path>) -> std::io::Result<String> {
    std::fs::read(path).map(|bytes| key_account_from_contents(&bytes))
}
...
mod tests {
    ...
    #[test]
    fn local_key_account_uses_contents_and_missing_file_has_no_fallback() {
        let dir = tempfile::tempdir().unwrap();
        let first = dir.path().join("first");
        let second = dir.path().join("second");
        let bytes = b"fixture-key-not-a-real-secret";
        std::fs::write(&first, bytes).unwrap();
        std::fs::write(&second, bytes).unwrap();
        let account = key_account_from_local_file(&first).unwrap();
        assert_eq!(account, key_account_from_contents(bytes));
        assert_eq!(key_account_from_local_file(&second).unwrap(), account);
        let store = InMemoryCredentialStore::new();
        store
            .set_key_passphrase(&account, "fixture-passphrase")
            .unwrap();
        assert_eq!(
            store
                .passphrase_for_key(&key_account_from_local_file(&second).unwrap())
                .unwrap(),
            Some("fixture-passphrase".into())
        );
        std::fs::write(&second, b"replacement-fixture").unwrap();
        let replacement = key_account_from_local_file(&second).unwrap();
        assert_ne!(replacement, account);
        assert_eq!(store.passphrase_for_key(&replacement).unwrap(), None);
        assert_eq!(
            key_account_from_local_file(dir.path().join("missing"))
                .unwrap_err()
                .kind(),
            std::io::ErrorKind::NotFound
        );
        assert!(key_account_from_local_file(dir.path()).is_err());
        assert_eq!(
            store.passphrase_for_key(&account).unwrap(),
            Some("fixture-passphrase".into())
        );
    }
