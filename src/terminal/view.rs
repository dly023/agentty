...
pub(crate) const DEFAULT_TITLE: &str = "agentty";
...
/// Product placeholders (including the legacy `tty7` title) say nothing
/// about the pane itself.
pub(crate) fn stated_title(title: &str) -> Option<&str> {
    match title.trim() {
        "" => None,
        t if t == DEFAULT_TITLE || t == "tty7" => None,
        t => Some(t),
    }
}
...
impl ShellParts {
    pub(crate) fn matches_checked_identity(
        &mut self,
        expected: &crate::daemon::protocol::PaneAttachExpectation,
    ) -> bool {
        self.terminal.poll_exited();
        if !self.restored || self.terminal.exited || self.terminal.child_exited() {
            return false;
        }
        match expected {
            ...
            crate::daemon::protocol::PaneAttachExpectation::Shell => {
                self.terminal.foreground_agent().is_none()
                    && self.terminal.agent_session().is_none()
            }
            crate::daemon::protocol::PaneAttachExpectation::Agent(identity) => {
                self.terminal.foreground_agent() == Some(identity.agent)
                    && self.terminal.agent_session().and_then(|s| s.session_id)
...
fn compose_notification_title(
    ...
    match (lead.or(host), workspace) {
        ...
        (None, None) => DEFAULT_TITLE.to_string(),
    }
}
...
impl TerminalView {
    pub fn spawn_shell_terminal_in(
        ...
        let attached = match restore_pane {
            Some(id) => crate::terminal::restoration_attachment(
                RemoteTerminal::attach_restoring_on(
                    &route,
                    TermSize::new(80, 24),
                    8,
                    17,
    ...
    fn show_jump_bottom(&self) -> bool {
        let term = self.terminal.term.lock();
        !term.mode().contains(TermMode::ALT_SCREEN) && term.grid().display_offset() > 0
    }

    fn render_jump_bottom(&self, cx: &mut Context<Self>) -> Option<impl IntoElement + use<>> {
        if !self.show_jump_bottom() {
            return None;
        }
        Some(
            div()
                .debug_selector(|| "terminal-jump-bottom".into())
                .absolute()
                .right(px(GRID_PAD_X + 12.))
                .bottom(self.line_height * 2. + px(GRID_PAD_Y))
                .occlude()
                .on_mouse_down(MouseButton::Left, |_, _, cx| cx.stop_propagation())
                .child(
                    Button::new("terminal-jump-bottom")
                        .small()
                        .icon(IconName::ChevronDown)
                        .label(t(L10nKey::TerminalJumpBottom))
...
impl Render for TerminalView {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        div()
            ...
            .drag_over::<crate::core::path_drag::TreePathDrag>(|s, _, _, cx| {
                s.bg(cx.theme().drag_border.opacity(0.12))
            })
            .on_drop(cx.listener(
                |this, paths: &crate::core::path_drag::TreePathDrag, window, cx| {
                    let Some(paths) = paths.paths_on(this.host_id()) else {
                        window.push_notification(t(L10nKey::FileDropForeignTerminal), cx);
                        return;
                    };
                    window.focus(&this.focus_handle, cx);
            ...
            .children(self.render_jump_bottom(cx))
            .children(search_bar)
            .children(input_bar)
...
mod tests {
    ...
    fn a_notification_title_keeps_at_most_two_segments() {
        ...
        assert_eq!(compose_notification_title(None, None, None), "agentty");
    }
...
pub(crate) fn quiet_test_shell_parts(
    ...
    let (client, peer) = test_stream_pair();
    (
        ShellParts {
            terminal: RemoteTerminal::from_stream(client, TermSize::new(80, 24)).unwrap(),
            pane_id,
            shell_spec: None,
            workspace: None,
            restored: true,
...
#[cfg(test)]
pub(crate) fn quiet_test_owned_pane(
    pane_id: u64,
    ...
    view.update(cx, |view, _| view.owner_workspace = Some(owner));
    (view, stream)
}
...
mod gpui_tests {
    ...
    fn title_events_drive_the_tab_title(cx: &mut TestAppContext) {
        ...
        settle(cx);
        window
            .update(cx, |view, _, _| assert_eq!(view.title, "agentty"))
            .unwrap();
    }
    ...
    #[gpui::test]
    fn jump_bottom_button_uses_existing_viewport_without_terminal_input(cx: &mut TestAppContext) {
        let (window, mut daemon) = harness(cx);
        window
            .update(cx, |view, w, cx| {
                assert!(!view.show_jump_bottom());
                scroll_into_history(view, 10);
                assert!(view.show_jump_bottom());
                view.queue_scroll_anim(5., w, cx);
                ...
                assert!(!view.scroll_anim_frame(epoch, cx));
                assert!(!view.show_jump_bottom());
                scroll_into_history(view, 3);
                let mut parser: alacritty_terminal::vte::ansi::Processor = Default::default();
                parser.advance(&mut *view.terminal.term.lock(), b"\x1b[?1049h");
                assert!(
                    !view.show_jump_bottom(),
                    "alternate screen owns its navigation"
                );
            })
            .unwrap();
        assert_eq!(
            next_input_until_timeout(&mut daemon),
            None,
            "viewport change must not write PTY input; ordinary focus/size frames are allowed"
        );
    }

    #[gpui::test]
    fn jump_bottom_button_click_returns_to_bottom_and_hides(cx: &mut TestAppContext) {
        let (window, view, mut daemon) = rooted_harness(cx);
        let mut vcx = gpui::VisualTestContext::from_window(window.into(), cx);
        vcx.run_until_parked();
        assert!(vcx.debug_bounds("terminal-jump-bottom").is_none());
        ...
        vcx.run_until_parked();
        let bounds = vcx
            .debug_bounds("terminal-jump-bottom")
            .expect("button visible while reading history");
        ...
        vcx.run_until_parked();
        view.update(&mut vcx, |view, _| {
            assert_eq!(display_offset(view), 0);
            assert!(!view.selecting, "button press must not select grid text");
        });
        assert!(vcx.debug_bounds("terminal-jump-bottom").is_none());
    ...
    #[gpui::test]
    fn child_exit_emits_lifecycle_event_but_disconnect_does_not(cx: &mut TestAppContext) {
        use std::cell::Cell;
        use std::rc::Rc;
