...
impl RemoteTerminal {
    ...
    pub fn attach_on(
        ...
        Self::attach_restoring_on(
            route,
            size,
            cell_w,
            cell_h,
            pane_id,
            &RestorePolicy::ReplaceMissing,
        )
    }

    pub(crate) fn attach_restoring_on(
        route: &PaneRoute,
        size: TermSize,
        cell_w: u16,
        cell_h: u16,
        ...
        let win = win_size(size, cell_w, cell_h);
        let mut stream = open_restore_stream(
            policy,
            pane_id,
            win,
            route.allow_remote_clipboard_write(),
            attach_reply_wait(route),
            || connect_routed(route),
        )?;
        let buffered = match attach_reply_prefix(&mut stream, pane_id, attach_reply_wait(route)) {
            ...
            Err(e) if route.is_local() && attach_unanswered(&e) => {
                ...
                // serving, but this says nothing about the requested pane.
                // Keep the unanswered error; no replacement is authorized.
                // If it does not answer there either, it
...
/// Classifies an ordinary restore attempt before its caller may spawn.
#[derive(Clone, Debug, PartialEq, Eq)]
pub enum RestorePolicy {
    ReplaceMissing,
    AttachOnly,
    Checked(Arc<std::collections::BTreeMap<u64, crate::daemon::protocol::CheckedPaneAttachment>>),
}

impl RestorePolicy {
    pub(crate) fn attach_only(&self) -> bool {
        !matches!(self, Self::ReplaceMissing)
    }
}

fn open_restore_stream(
    policy: &RestorePolicy,
    pane_id: u64,
    size: WinSize,
    allow_remote_clipboard_write: bool,
    wait: std::time::Duration,
    mut connect: impl FnMut() -> anyhow::Result<Stream>,
) -> anyhow::Result<Stream> {
    let request = match policy {
        RestorePolicy::Checked(guards) => {
            let guard = guards.get(&pane_id).ok_or_else(|| {
                anyhow::anyhow!(crate::ui::i18n::t(
                    crate::ui::i18n::L10nKey::HistoryRejoinUnavailable
                ))
            })?;
            let mut probe = connect()?;
            probe.set_read_timeout(Some(wait))?;
            ClientMsg::Version.encode(&mut probe)?;
            let supported = matches!(DaemonMsg::read(&mut probe),
                Ok(DaemonMsg::Version(version)) if version.has_feature(crate::daemon::protocol::FEATURE_CHECKED_ATTACH)
                    && (!matches!(&guard.identity, crate::daemon::protocol::PaneAttachExpectation::Stopped(_))
                        || version.has_feature(crate::daemon::protocol::FEATURE_STOPPED_OUTPUT)));
            anyhow::ensure!(
                supported,
                crate::ui::i18n::t(crate::ui::i18n::L10nKey::HistoryRejoinUnavailable)
            );
        ...
        RestorePolicy::ReplaceMissing | RestorePolicy::AttachOnly => ClientMsg::Attach {
            pane_id,
            size,
            allow_remote_clipboard_write,
        },
...
pub(crate) fn restoration_attachment<T>(
    result: anyhow::Result<T>,
    pane_id: u64,
    policy: RestorePolicy,
) -> anyhow::Result<Option<T>> {
    match result {
        Ok(attached) => Ok(Some(attached)),
        Err(error)
            if policy == RestorePolicy::ReplaceMissing
                && error.chain().any(|cause| {
                    cause
                        .downcast_ref::<AttachRefused>()
                        .is_some_and(|refused| refused.message == format!("no such pane {pane_id}"))
                }) =>
        {
            Ok(None)
        }
...
/// nothing (#673). An uncertain failure must remain retryable, not grant
/// permission to create another pane over a potentially live one.
fn attach_reply_prefix(
    ...
    let message = read_error_frame(stream, &mut buffered, wait)
        .ok_or_else(|| anyhow::anyhow!("incomplete or invalid Attach error for pane {pane_id}"))?;
...
mod tests {
    ...
    #[test]
    fn ordinary_restore_requires_exact_missing_verdict() {
        assert_eq!(
            restoration_attachment(Ok(42), 7, RestorePolicy::ReplaceMissing).unwrap(),
            Some(42)
        );
        for (message, missing) in [
            ("no such pane 7", true),
            ("no such pane 8", false),
            ("permission denied", false),
            ("no such pane 7; retry later", false),
        ] {
            ...
            DaemonMsg::Error(message.into())
                ...
                .unwrap();
            daemon.flush().unwrap();
            let result = attach_reply_prefix(&mut client, 7, std::time::Duration::from_millis(20));
            let outcome = restoration_attachment(result, 7, RestorePolicy::ReplaceMissing);
            if missing {
                assert_eq!(outcome.unwrap(), None);
            } else {
                assert!(outcome.is_err(), "must not replace on {message}");
            }
        }
        for kind in [
            ...
            let error = anyhow::Error::new(std::io::Error::new(kind, "no such pane 7"));
            assert!(
                restoration_attachment::<()>(Err(error), 7, RestorePolicy::ReplaceMissing).is_err()
            );
        }
        let error = anyhow::Error::new(AttachUnanswered {
            pane_id: 7,
            wait: std::time::Duration::from_millis(20),
        });
        assert!(
            restoration_attachment::<()>(
                Err(error.context("outer context")),
                7,
                RestorePolicy::ReplaceMissing
            )
            .is_err()
    ...
    #[test]
    fn checked_restore_rejects_missing_guard_and_unsupported_peer() {
        use crate::daemon::protocol::{
            CheckedPaneAttachment, DaemonVersion, PaneAttachExpectation,
        };
        ...
        let policy = RestorePolicy::Checked(Arc::new([(7, guard)].into()));
        let size = win_size(TermSize::new(80, 24), 8, 17);
        let wait = std::time::Duration::from_millis(30);
        assert!(
            open_restore_stream(&policy, 8, size, false, wait, || {
                panic!("missing guard opened transport")
            })
            .is_err()
        );
        let (client, mut peer) = UnixStream::pair().unwrap();
        let mut version = DaemonVersion::current();
        version.features.clear();
        DaemonMsg::Version(version).encode(&mut peer).unwrap();
        let mut client = Some(client);
        assert!(
            open_restore_stream(&policy, 7, size, false, wait, || {
                Ok(client
                    .take()
                    .expect("unsupported peer opened attachment channel"))
            })
            .is_err()
        );
        assert_eq!(ClientMsg::read(&mut peer).unwrap(), ClientMsg::Version);
        assert!(
            ClientMsg::read(&mut peer).is_err(),
            "unsupported peer received another request"
        );
        let (silent, mut peer) = UnixStream::pair().unwrap();
        let mut silent = Some(silent);
        assert!(
            open_restore_stream(&policy, 7, size, false, wait, || {
                Ok(silent
                    .take()
                    .expect("unanswered probe attempted attachment"))
            })
            .is_err()
        );
        assert_eq!(ClientMsg::read(&mut peer).unwrap(), ClientMsg::Version);

        let (probe, mut probe_peer) = UnixStream::pair().unwrap();
        let (attach, mut attach_peer) = UnixStream::pair().unwrap();
        DaemonMsg::Version(DaemonVersion::current())
            .encode(&mut probe_peer)
            .unwrap();
        DaemonMsg::Error("no such pane 7".into())
            .encode(&mut attach_peer)
            .unwrap();
        let mut streams = [probe, attach].into_iter();
        let mut stream = open_restore_stream(&policy, 7, size, false, wait, || {
            Ok(streams
                .next()
                .expect("checked attachment retried transport"))
        })
        .unwrap();
        assert_eq!(
            ClientMsg::read(&mut probe_peer).unwrap(),
            ClientMsg::Version
        );
        let RestorePolicy::Checked(guards) = &policy else {
            unreachable!()
        };
        assert_eq!(
            ClientMsg::read(&mut attach_peer).unwrap(),
            ClientMsg::AttachChecked {
                pane_id: 7,
                size,
                guard: guards[&7].clone(),
                allow_remote_clipboard_write: false,
            }
        );
        let refusal = attach_reply_prefix(&mut stream, 7, wait);
        assert!(restoration_attachment(refusal, 7, policy).is_err());
        drop(stream);
        assert!(
            ClientMsg::read(&mut attach_peer).is_err(),
            "refusal caused a downgrade or input"
        );
    }

    #[test]
    fn stopped_output_requires_its_own_peer_feature() {
        use crate::daemon::protocol::{
            CheckedPaneAttachment, DaemonVersion, PaneAttachExpectation,
        };
        let policy = RestorePolicy::Checked(Arc::new(
            [(
                7,
                CheckedPaneAttachment {
                    ...
                    tab: tty7_core::core::machine::TabId::new(),
                    identity: PaneAttachExpectation::Stopped(
                        tty7_core::agent_sessions::SessionIdentity {
                            provider: tty7_core::agent_sessions::Provider::Codex,
        ...
        version
            .features
            .retain(|f| f != crate::daemon::protocol::FEATURE_STOPPED_OUTPUT);
        DaemonMsg::Version(version).encode(&mut peer).unwrap();
        let mut client = Some(client);
        assert!(
            open_restore_stream(
                &policy,
                7,
                win_size(TermSize::new(80, 24), 8, 16),
                false,
                std::time::Duration::from_millis(30),
                || {
                    Ok(client
                        .take()
                        .expect("old peer must not receive a stopped output stream"))
                }
            )
            .is_err()
        );
        assert_eq!(ClientMsg::read(&mut peer).unwrap(), ClientMsg::Version);
        assert!(ClientMsg::read(&mut peer).is_err());
    }

    #[test]
    fn attach_only_never_authorizes_missing_replacement() {
        for policy in [RestorePolicy::ReplaceMissing, RestorePolicy::AttachOnly] {
            let (mut client, mut daemon) = UnixStream::pair().unwrap();
            DaemonMsg::Error("no such pane 7".into())
                .encode(&mut daemon)
                .unwrap();
            daemon.flush().unwrap();
            let result = attach_reply_prefix(&mut client, 7, std::time::Duration::from_millis(20));
            let decision = restoration_attachment(result, 7, policy.clone());
            match policy {
                RestorePolicy::ReplaceMissing => assert_eq!(decision.unwrap(), None),
                RestorePolicy::AttachOnly | RestorePolicy::Checked(_) => assert!(
                    decision.is_err(),
                    "Rejoin cannot authorize Spawn even after confirmed missing"
                ),
            }
        }
        assert_eq!(
            restoration_attachment(Ok(7), 7, RestorePolicy::AttachOnly).unwrap(),
            Some(7)
        );
    }

    #[test]
    fn incomplete_attach_error_cannot_invent_missing_pane() {
        let (mut client_side, mut daemon_side) = UnixStream::pair().unwrap();
        let mut frame = Vec::new();
        DaemonMsg::Error("permission denied".into())
            .encode(&mut frame)
            .unwrap();
        daemon_side.write_all(&frame[..frame.len() - 2]).unwrap();
        drop(daemon_side);
        let error = attach_reply_prefix(&mut client_side, 7, std::time::Duration::from_millis(20))
            .expect_err("incomplete refusal cannot attach");
        assert!(
            !format!("{error:#}").contains("no such pane"),
            "fabricated verdict: {error:#}"
        );
        assert!(
            !attach_refused(&error),
            "partial frame is not a complete refusal"
        );
    }
    ...
    fn child_exit_is_distinguished_from_daemon_disconnect() {
        ...
        assert!(
            !term.child_exited(),
            "a disconnect is not a confirmed child exit"
        );
    }
