use agentty_core::core::machine::{Axis, Machine, PaneSeed, Workspace};
use agentty_core::core::session::WorkspaceId;
use agentty_core::daemon::control::{CONTROL_VERSION, ControlEvent, ControlRequest, ReplyOk};
use agentty_core::daemon::protocol::PROTOCOL_VERSION;
...
pub fn execute(cli: Cli, ctx: &Context, backend: &mut dyn Backend) -> Result<Outcome> {
    ...
    match cli.command {
        None => match cli.path {
            ...
            // `agentty statu` helps nobody.
            Some(word) if !looks_like_a_path(&word) => bail!(
                "unknown subcommand '{word}' — run `agentty --help` for the list. \
...
fn local_server(
    ...
    if let Some(machine) = machine {
        bail!(
            "`agentty server {verb}` manages only the server on THIS machine — with -m {machine} \
...
fn run(args: RunArgs, ctx: &Context, backend: &mut dyn Backend) -> Result<Outcome> {
    ...
    if args.keep && workspace.is_none() {
        bail!(
            "`run --keep` keeps the pane alive, so it must be filed into a workspace — \
             pass --ws, or run inside a agentty shell where $AGENTTY_WS names one"
        );
    }
    ...
    let (code, exact) = match backend.run_wait()? {
        Some(code) => (code, true),
        None => {
            eprintln!("agentty: {EXIT_CODE_UNKNOWN}");
            (1, false)
        }
    };
...
/// interrupted `agentty run` leaves its pane running with nothing referencing it,
...
fn pane_ls_all(backend: &mut dyn Backend) -> Result<Outcome> {
    ...
    if orphans > 0 {
        human.push_str(&format!(
            "\n{orphans} pane(s) held by no workspace — `agentty pane close %<id>` stops one\n"
        ));
    }
...
fn doctor(ctx: &Context, backend: &mut dyn Backend) -> Result<Outcome> {
    ...
    if ctx.config_dir.is_none() && ctx.pane.is_none() {
        human.push_str(
            "\nnot inside a agentty shell — address commands need an explicit %pane/@tab/workspace\n",
        );
    }
    report(
        human,
...
mod tests {
    ...
    use agentty_core::core::machine::Tab;
    use clap::Parser;
    ...
    fn segment(bytes: &[u8]) -> crate::backend::CaptureSegment {
        crate::backend::CaptureSegment {
            size: agentty_core::daemon::protocol::WinSize {
                cols: 20,
                rows: 10,
                cell_w: 8,
                cell_h: 16,
    ...
    fn pane_info(pane_id: u64, owner: Option<&str>) -> agentty_core::daemon::protocol::PaneInfo {
        agentty_core::daemon::protocol::PaneInfo {
            pane_id,
            cwd: None,
            title: "sh".into(),
            alive: true,
    ...
    fn pane_ls_all_surfaces_the_panes_no_workspace_holds() {
        ...
        // interrupted `agentty run` leaves behind.
        backend.registry = vec![
            pane_info(1, None),
            pane_info(3, None),
            pane_info(77, Some("agentty-cli")),
        ];
    ...
    fn pane_ls_without_all_cannot_see_an_orphan() {
        let mut backend = mock();
        backend.registry = vec![pane_info(77, Some("agentty-cli"))];
    ...
    fn ls_and_ws_ls_are_the_same_request() {
        ...
        let mut a = mock();
        run_cli(&["agentty", "ls"], &ctx, &mut a);
        let mut b = mock();
        run_cli(&["agentty", "ws", "ls"], &ctx, &mut b);
    ...
    fn ws_rename_rm_attach_detach_build_their_requests() {
        ...
        run_cli(&["agentty", "ws", "rm", "web"], &ctx, &mut backend);
        assert_eq!(
            backend.control_calls[1],
        ...
        let out = run_cli(&["agentty", "ws", "attach", "api"], &ctx, &mut backend);
        assert_eq!(
            backend.control_calls[1],
        ...
        backend.control_calls.clear();
        run_cli(&["agentty", "ws", "detach", "api"], &ctx, &mut backend);
        assert_eq!(
            backend.control_calls[1],
    ...
    fn tab_verbs_resolve_machine_wide_ordinals_to_real_ids() {
        ...
        run_cli(&["agentty", "tab", "close", "@3"], &ctx, &mut backend);
        assert_eq!(
            backend.control_calls,
            vec![
                ControlRequest::MachineGet,
        ...
        backend.control_calls.clear();
        run_cli(&["agentty", "tab", "move", "@2", "0"], &ctx, &mut backend);
        assert_eq!(
            backend.control_calls[1],
    ...
    fn tab_new_uses_the_workspace_from_the_environment() {
        ...
        run_cli(
            &["agentty", "tab", "new", "--cwd", "C:\\elsewhere"],
            &ctx,
            &mut backend,
        );
        assert_eq!(
    ...
    fn pane_split_builds_the_split_and_spawns_the_new_shell() {
        ...
        let api = backend.machine.workspaces[0].id;
        let out = run_cli(
            &["agentty", "pane", "split", "%2", "--v", "--ratio", "0.3"],
            &Context::default(),
            &mut backend,
        );
        assert_eq!(
    ...
    fn split_without_an_address_uses_the_pane_from_the_environment() {
        ...
        run_cli(&["agentty", "split", "--h"], &ctx, &mut backend);
        let web = backend.machine.workspaces[1].id;
    ...
    fn send_reaches_the_pane_socket_seam_not_the_control_socket() {
        ...
        backend.sent.clear();
        run_cli(&["agentty", "send", "echo hi"], &ctx, &mut backend);
    ...
    fn run_passes_the_command_and_its_exit_code_through() {
        ...
        let out = execute(
            cli(&["agentty", "run", "--keep", "--", "cargo", "test"]),
            &ctx,
            &mut backend,
        )
        .unwrap();
    ...
    fn run_keep_spawns_first_then_files_the_pane_into_the_workspace() {
        ...
        let out = execute(
            cli(&[
                "agentty", "run", "--keep", "--ws", "api", "--cwd", "C:\\proj", "--", "cargo",
                "watch",
            ]),
            &Context::default(),
            &mut backend,
    ...
    fn run_answers_json_even_though_it_carries_an_exit_code() {
        ...
        let out = execute(
            cli(&["agentty", "run", "--json", "--", "cargo", "test"]),
            &ctx,
            &mut backend,
        )
        .unwrap();
    ...
    fn a_machine_flag_refuses_the_local_server_verbs() {
        for verb in ["start", "stop", "restart", "logs"] {
            let mut backend = mock();
            let err = execute(
                cli(&["agentty", "-m", "devbox", "server", verb]),
                &Context::default(),
                &mut backend,
            )
    ...
    fn a_mistyped_subcommand_is_named_as_one_not_offered_to_the_gui() {
        for typo in ["tree", "statu", "pnae", "workspace"] {
            let err = execute(cli(&["agentty", typo]), &Context::default(), &mut mock())
    ...
    fn agents_status_and_machine_ls_are_single_aggregate_requests() {
        use agentty_core::daemon::control::{RouteInfo, ServerStatus};

        let mut backend = mock();
        backend.replies.push_back(ReplyOk::AgentStates(Vec::new()));
        let out = run_cli(&["agentty", "agents"], &Context::default(), &mut backend);
        ...
        let out = run_cli(&["agentty", "status"], &Context::default(), &mut backend);
    ...
    fn doctor_backend() -> MockBackend {
        use agentty_core::daemon::control::ServerStatus;
    ...
    fn doctor_reports_the_injected_context_and_the_server_half() {
        ...
        let ctx = Context {
            pane: Some("7".into()),
            ws: None,
            config_dir: Some("/cfg/agentty".into()),
        };
        let out = human(run_cli(&["agentty", "doctor"], &ctx, &mut doctor_backend()));
        assert!(out.contains("set (/cfg/agentty)"), "{out}");
    }
}
