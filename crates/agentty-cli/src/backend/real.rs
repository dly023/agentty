...
use agentty_core::client::{ControlClient, PaneClient, PaneSession};
...
use agentty_core::daemon::protocol::{DaemonMsg, PaneInfo, PaneProcs, ShellSpec, WinSize};
use agentty_core::daemon::router::RouteTarget;
...
const NOT_RUNNING: &str =
    "could not reach the agentty server on this machine — `agentty server start` brings one up";
...
impl RealBackend {
    ...
    /// `AGENTTY_CONFIG_DIR`, which is what `agentty_core`'s own endpoint derivation
    ...
    fn hello_msg() -> ControlHello {
        ControlHello::host_rpc(format!("agentty-cli-{}", std::process::id()), hostname())
    }
        ...
        let code = loop {
            match session.recv() {
                Ok(DaemonMsg::Output(bytes)) | Ok(DaemonMsg::Snapshot(bytes)) => {
                    ...
                    // (`agentty run -- … | head`) must end the pipeline, not turn
...
fn hostname() -> String {
    std::env::var("COMPUTERNAME")
        ...
        .unwrap_or_else(|_| "agentty-cli".to_string())
}
