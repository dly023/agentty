...
#[command(
    ...
    long_about = "One name, three artifacts: this CLI, the agentty GUI (agentty-app), and\n\
                  ...
                  protocol of its own. Inside a agentty shell, $AGENTTY_CONFIG_DIR points at\n\
                  the server, and $AGENTTY_PANE / $AGENTTY_WS let the address-taking verbs run\n\
...
pub enum Command {
    #[command(about = "This machine on one screen (= agentty ws ls)")]
    Ls,
    ...
    #[command(about = "Split a pane (= agentty pane split)")]
    Split(SplitArgs),
    ...
    #[command(about = "Server health at a glance (= agentty server status)")]
    Status,
    ...
    #[command(subcommand, about = "The agentty-server process on this machine")]
    Server(ServerCmd),
}
...
pub struct RunArgs {
    ...
    #[arg(
        ...
        help = "Sets the pane's AGENTTY_WS and, with --keep, the workspace the kept pane is \
                filed into; defaults to $AGENTTY_WS inside a agentty shell"
    )]
    ...
    #[arg(
        ...
        required = true,
        value_name = "CMD",
        help = "The command, after `--`: agentty run -- cargo test"
    )]
    pub cmd: Vec<String>,
}
...
pub struct SplitArgs {
    #[arg(
        value_name = "%PANE",
        help = "Pane to split; defaults to $AGENTTY_PANE inside a agentty shell"
    )]
...
pub struct CaptureArgs {
    #[arg(
        value_name = "%PANE",
        help = "Pane to read; defaults to $AGENTTY_PANE inside a agentty shell"
    )]
    pub target: Option<String>,
...
mod tests {
    ...
    #[test]
    fn bare_agentty_is_the_gui_launcher() {
        let cli = parse(&["agentty"]);
    ...
    fn a_path_that_is_not_a_verb_still_launches_the_gui() {
        let cli = parse(&["agentty", "C:\\Users\\me\\proj"]);
    ...
    fn every_top_level_verb_parses() {
        assert!(matches!(
            parse(&["agentty", "ls"]).command,
            Some(Command::Ls)
        ));
        assert!(matches!(
            parse(&["agentty", "new"]).command,
            Some(Command::New { path: None })
        ));
        assert!(matches!(
            parse(&["agentty", "new", "C:\\proj"]).command,
            Some(Command::New { path: Some(p) }) if p == "C:\\proj"
        ));
        assert!(matches!(
            parse(&["agentty", "agents"]).command,
            Some(Command::Agents)
        ));
        assert!(matches!(
            parse(&["agentty", "events"]).command,
            Some(Command::Events)
        ));
        assert!(matches!(
            parse(&["agentty", "status"]).command,
            Some(Command::Status)
        ));
        assert!(matches!(
            parse(&["agentty", "doctor"]).command,
            Some(Command::Doctor)
        ));
        assert!(matches!(
            parse(&["agentty", "procs", "%3"]).command,
    ...
    fn run_takes_the_command_after_a_double_dash() {
        let cli = parse(&["agentty", "run", "--", "cargo", "test", "--keep"]);
        ...
        assert!(
            !args.keep,
            "--keep after -- belongs to the child, not to agentty"
        );

        let cli = parse(&[
            "agentty", "run", "--keep", "--cwd", "C:\\proj", "--", "make",
        ]);
        let Some(Command::Run(args)) = cli.command else {
            panic!("run did not parse");
        };
    ...
    fn run_without_a_command_is_a_usage_error() {
        let err = Cli::try_parse_from(["agentty", "run"]).unwrap_err();
        assert_eq!(err.kind(), ErrorKind::MissingRequiredArgument);
    }

    #[test]
    fn split_requires_exactly_one_axis() {
        let cli = parse(&["agentty", "split", "--v"]);
        let Some(Command::Split(args)) = cli.command else {
            panic!("split did not parse");
        };
        assert!(args.vertical && !args.horizontal);
        assert!(args.target.is_none(), "the pane comes from $AGENTTY_PANE");
        assert_eq!(args.ratio, 0.5);

        let cli = parse(&["agentty", "pane", "split", "%42", "--h", "--ratio", "0.3"]);
        ...
        let err = Cli::try_parse_from(["agentty", "split", "%1"]).unwrap_err();
        assert_eq!(
            err.kind(),
            ErrorKind::MissingRequiredArgument,
            "an axis must be chosen"
        );
        let err = Cli::try_parse_from(["agentty", "split", "--h", "--v"]).unwrap_err();
        assert_eq!(
            err.kind(),
    ...
    fn send_takes_an_optional_address_then_text() {
        let cli = parse(&["agentty", "send", "%42", "make -j8", "--enter"]);
        ...
        let cli = parse(&["agentty", "send", "make -j8"]);
        let Some(Command::Send(args)) = cli.command else {
            panic!("send did not parse");
        };
    ...
    fn every_ws_verb_parses() {
        assert!(matches!(
            parse(&["agentty", "ws", "ls"]).command,
            Some(Command::Ws(WsCmd::Ls))
        ));
        assert!(matches!(
            parse(&["agentty", "ws", "tree", "api"]).command,
            Some(Command::Ws(WsCmd::Tree { ws: Some(w) })) if w == "api"
        ));
        assert!(matches!(
            parse(&["agentty", "ws", "new", "api"]).command,
            Some(Command::Ws(WsCmd::New { name: Some(n) })) if n == "api"
        ));
        assert!(matches!(
            parse(&["agentty", "ws", "rename", "api", "web"]).command,
        ...
        assert!(matches!(
            parse(&["agentty", "ws", "attach", "api"]).command,
            Some(Command::Ws(WsCmd::Attach { ws })) if ws == "api"
        ));
        assert!(matches!(
            parse(&["agentty", "ws", "detach", "api"]).command,
    ...
    fn every_pane_verb_parses() {
        ...
        assert!(matches!(
            parse(&["agentty", "pane", "close", "%9"]).command,
    ...
    fn every_machine_and_server_verb_parses() {
        ...
        for (verb, want) in [
            ...
            let cli = parse(&["agentty", "server", verb]);
    ...
    fn global_flags_are_accepted_anywhere() {
        let cli = parse(&["agentty", "-m", "devbox", "ls", "--json"]);
        assert_eq!(cli.machine.as_deref(), Some("devbox"));
        assert!(cli.json);
        assert!(matches!(cli.command, Some(Command::Ls)));

        let cli = parse(&["agentty", "ws", "ls", "-q", "-m", "devbox"]);
        assert!(cli.quiet);
        assert_eq!(cli.machine.as_deref(), Some("devbox"));

        let cli = parse(&["agentty", "capture", "%3", "--scrollback", "--json"]);
        assert!(cli.json);
    ...
    fn capture_plain_is_its_own_flag_and_composes_with_scrollback() {
        ...
        let Some(Command::Capture(args)) = parse(&["agentty", "capture", "--plain"]).command else {
            panic!("capture did not parse");
        };
        ...
        let Some(Command::Capture(args)) =
            parse(&["agentty", "capture", "%3", "--plain", "--scrollback"]).command
        else {
            panic!("capture did not parse");
        };
        assert!(args.plain && args.scrollback);
    ...
    fn usage_errors_exit_with_code_2() {
        let err = Cli::try_parse_from(["agentty", "ws", "frobnicate"]).unwrap_err();
        assert_eq!(
            err.exit_code(),
            2,
        ...
        let err =
            Cli::try_parse_from(["agentty", "tab", "move", "@7", "not-a-number"]).unwrap_err();
