...
fn main() {
    if std::env::var(DAEMON_ENV).as_deref() == Ok("1") {
        if let Err(e) = agentty_core::daemon::server::run_daemon() {
...
impl Daemon {
    fn start() -> Daemon {
        ...
        let child = Command::new(own)
            ...
            .stderr(Stdio::null())
            .spawn()
            .expect("start the in-test agentty server");
        #[cfg(windows)]
    ...
    fn cli(&self, args: &[&str]) -> Command {
        let mut cmd = Command::new(env!("CARGO_BIN_EXE_agentty"));
        cmd.args(args)
            .env("AGENTTY_CONFIG_DIR", self.dir.path())
            .env("AGENTTY_DATA_DIR", self.dir.path())
            .env_remove("AGENTTY_PANE")
            .env_remove("AGENTTY_WS")
    ...
    fn run(&self, args: &[&str]) -> Output {
        self.cli(args)
            .output()
            .unwrap_or_else(|e| panic!("could not run agentty {args:?}: {e}"))
    }
    ...
    fn run_json(&self, args: &[&str]) -> serde_json::Value {
        ...
        serde_json::from_str(&out)
            .unwrap_or_else(|e| panic!("agentty {args:?} --json printed no JSON ({e}): {out}"))
    }
}
...
fn run_streams_output_and_passes_the_exit_code(daemon: &Daemon) {
    let echo = one_shot("echo agentty_e2e_run_marker");
    ...
    assert!(stdout.contains("agentty_e2e_run_marker"), "{stdout}");
...
fn run_keep_files_the_pane_so_ls_shows_it(daemon: &Daemon) {
    ...
    args.extend(one_shot("echo agentty_e2e_keep_marker"));
...
fn config_dir_alone_resolves_both_endpoints(daemon: &Daemon) {
    let out = Command::new(env!("CARGO_BIN_EXE_agentty"))
        .args(["status", "--json"])
        .env_remove("AGENTTY_DATA_DIR")
        .env_remove("AGENTTY_CONTROL_SOCK")
        .env_remove("AGENTTY_PANE")
        .env_remove("AGENTTY_WS")
        .env_remove("AGENTTY_SOCKET")
        .env("AGENTTY_CONFIG_DIR", daemon.dir.path())
        .output()
        .expect("run agentty status with only AGENTTY_CONFIG_DIR");
    assert!(
        out.status.success(),
    ...
    let out = Command::new(env!("CARGO_BIN_EXE_agentty"))
        ...
        .env("AGENTTY_CONFIG_DIR", daemon.dir.path())
        .output()
        .expect("run agentty run with only AGENTTY_CONFIG_DIR");
    assert_eq!(
        out.status.code(),
        Some(9),
...
fn send_then_capture_round_trip(daemon: &Daemon) {
    ...
    daemon.run_ok(&[
        "send",
        &address,
        "echo agentty_e2e_capture_marker",
        "--enter",
    ]);
    ...
    loop {
        ...
        if seen.contains("agentty_e2e_capture_marker") {
            return;
        }
        assert!(
            Instant::now() < deadline,
...
/// `agentty … | head -1` must end the way `cat … | head -1` ends. Rust ignores
...
fn a_reader_that_hung_up_ends_the_pipeline_quietly(daemon: &Daemon) {
    ...
    let printer = one_shot("echo agentty_e2e_pipe_marker");
    ...
    for args in [vec!["ls"], streaming] {
        let mut child = daemon
            ...
            .stderr(Stdio::piped())
            .spawn()
            .unwrap_or_else(|e| panic!("could not spawn agentty {args:?}: {e}"));
        ...
        child
            ...
            .expect("reading agentty's stderr");
        let status = child.wait().expect("waiting for agentty");

        assert!(
            !stderr.contains("panicked"),
            "agentty {args:?} panicked when its reader hung up: {stderr}"
        );
        assert!(
            !stderr.to_lowercase().contains("broken pipe"),
            "a hung-up reader is how a pipeline ends, not something to report: \
             agentty {args:?} said {stderr}"
        );
        ...
        assert_ne!(
            status.code(),
            Some(1),
            "agentty {args:?} treated a hung-up reader as a failure: {stderr}"
        );
    }
}
...
fn capture_plain_returns_text_not_escapes(daemon: &Daemon) {
    ...
    daemon.run_ok(&["send", &address, "echo agentty_e2e_plain_marker", "--enter"]);

    let deadline = Instant::now() + SETTLE_WITHIN;
    loop {
        let raw = daemon.run_ok(&["capture", &address, "--scrollback"]);
        let plain = daemon.run_ok(&["capture", &address, "--scrollback", "--plain"]);
        if plain.contains("agentty_e2e_plain_marker") {
            assert!(
                raw.contains('\r'),
