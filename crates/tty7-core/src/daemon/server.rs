...
struct Registry {
    ...
    next_id: AtomicU64,
    services: std::sync::OnceLock<crate::host::server::Services>,
}
...
impl Registry {
    ...
    fn install_services(&self, services: &crate::host::server::Services) {
        // Share authority, not the PaneDirectory back-reference (which would
        // make an Arc cycle). Install once, before either listener serves panes.
        let shared = crate::host::server::Services {
            provider_titles: services.provider_titles.clone(),
            machine: services.machine.clone(),
            attachments: services.attachments.clone(),
            panes: None,
        };
        assert!(
            self.services.set(shared).is_ok(),
            "pane services installed twice"
        );
    }
    ...
    fn retire_exited(&self, id: u64) {
        if let Some(pane) = self.get(id).filter(|pane| !pane.info().alive) {
...
impl crate::host::server::PaneDirectory for Registry {
    ...
    ) -> std::io::Result<crate::daemon::control::SleepTabReply> {
        recovery::sleep_tab(self, request, cancelled)
    }
    fn sleep_session(
        self: Arc<Self>,
        ...
        recovery::sleep(self, request, cancelled)
    }
    fn resume_stopped(
        self: Arc<Self>,
        request: &crate::daemon::control::StoppedResumeRequest,
        cancelled: &dyn Fn() -> bool,
    ) -> std::io::Result<u64> {
        recovery::resume(self, request, cancelled)
    }
        ...
        .spawn(move || {
            ...
            loop {
                ...
                for pane in registry.all() {
                    ...
                    let mark = pane.persist_scrollback();
...
pub fn run_daemon() -> anyhow::Result<()> {
    ...
    #[cfg(any(unix, windows))]
    {
        ...
        registry.install_services(&services);
...
pub fn control_services() -> crate::host::server::Services {
    ...
    match MachineStore::shared() {
        Ok(machine) => {
            ...
            let mut services = crate::host::server::Services::with_machine(machine.clone());
            match crate::agent_sessions::live_titles::start_current(machine) {
                Ok(watch) => services.provider_titles = Some(Arc::new(watch)),
                Err(error) => log::warn!("Codex title watch unavailable: {error}"),
            }
            services
        }
        Err(e) => {
...
fn handle_conn(stream: Stream, registry: Arc<Registry>) -> anyhow::Result<()> {
    ...
    match first {
        ...
        ClientMsg::AttachChecked { pane_id, guard, .. }
            if matches!(
                &guard.identity,
                crate::daemon::protocol::PaneAttachExpectation::Stopped(_)
            ) =>
        {
            let mut write_stream = write_stream;
            match recovery::stopped_output(&registry, &guard, pane_id) {
                Ok(frames) => {
                    for frame in frames {
                        frame.encode(&mut write_stream)?;
                    }
                }
                Err(error) => DaemonMsg::Error(error.to_string()).encode(&mut write_stream)?,
            }
            // One-way replay: no subscriber, controller, input loop or registry mutation.
            Ok(())
        }
...
fn stream_pane_with_attach(
    ...
    guard: Option<&crate::daemon::protocol::CheckedPaneAttachment>,
) -> anyhow::Result<()> {
    let (tx, rx) = mpsc::channel::<DaemonMsg>();
    let epoch = match guard {
        Some(guard) => match registry
            .services
            .get()
            .ok_or_else(|| std::io::Error::other("pane ownership authority is unavailable"))
            .and_then(|services| {
                services.with_pane_attachment(&guard.proof, guard.tab, id, || {
                    pane.attach_checked(tx, allow_remote_clipboard_write, &guard.identity)
            ...
            Err(reason) => {
                let mut w = write_stream;
                DaemonMsg::Error(reason.to_string()).encode(&mut w)?;
                return Ok(());
            }
        },
        None => pane.attach_with_permissions(tx, allow_remote_clipboard_write),
    };
...
mod tests {
    ...
    mod conn {
        ...
        #[test]
        fn checked_attach_missing_pane_cannot_spawn() {
            let (mut client, h) = serve();
            ClientMsg::AttachChecked {
                pane_id: 999,
                size: SIZE,
                guard: crate::daemon::protocol::CheckedPaneAttachment {
                    identity: crate::daemon::protocol::PaneAttachExpectation::Agent(
                        crate::daemon::protocol::PaneAttachIdentity {
