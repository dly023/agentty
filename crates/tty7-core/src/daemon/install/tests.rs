...
#[test]
fn source_build_identity_is_distinct_from_release_version() {
    let identity = RemoteProtocol::of_this_build();
    let prefix = format!("{}+source.", client_version());
    let hash = identity
        .build
        .strip_prefix(&prefix)
        .expect("source identity, not release version");
    ...
    let mut other_source = identity.clone();
    other_source.build = "another source".into();
    assert!(identity.serves(&other_source));
}
