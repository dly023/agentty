...
/// Non-preemptive attachment guarded by workspace proof, unique tab ownership,
/// and an explicit live Shell or Agent identity expectation.
pub const FEATURE_CHECKED_ATTACH: &str = "checked-attach-v4";
/// Checked, non-consuming output replay for an exactly identified stopped pane.
pub const FEATURE_STOPPED_OUTPUT: &str = "stopped-output-v1";
...
impl DaemonVersion {
    pub fn current() -> DaemonVersion {
        let mut features = vec![
            ...
            FEATURE_CHECKED_ATTACH.to_string(),
            FEATURE_STOPPED_OUTPUT.to_string(),
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct PaneAttachIdentity {
    pub agent: crate::core::cli_agent::CLIAgent,
    pub session_id: String,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub enum PaneAttachExpectation {
    /// Exact target-tree ownership, without asserting a provider runtime ID.
    /// The server still requires the proof, unique slot, liveness and no controller.
    Owned,
    Shell,
    Agent(PaneAttachIdentity),
    /// Read-only final output; never installs a live pane controller.
    Stopped(crate::agent_sessions::SessionIdentity),
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct CheckedPaneAttachment {
    pub identity: PaneAttachExpectation,
    pub proof: crate::daemon::control::AttachmentProof,
...
mod kind {
    ...
    pub const ATTACH_CHECKED: u8 = 57;
...
impl ClientMsg {
    pub fn encode<W: Write>(&self, w: &mut W) -> io::Result<()> {
        match self {
            ...
            ClientMsg::AttachChecked {
                ...
                w,
                kind::ATTACH_CHECKED,
                &to_json(&(pane_id, size, guard, allow_remote_clipboard_write))?,
            ),
    ...
    pub fn from_frame(k: u8, payload: Vec<u8>) -> io::Result<Self> {
        Ok(match k {
            ...
            kind::ATTACH_CHECKED => {
                let (pane_id, size, guard, allow_remote_clipboard_write) = from_json(&payload)?;
                ClientMsg::AttachChecked {
                    pane_id,
                    size,
                    guard,
...
mod tests {
    ...
    #[test]
    fn checked_attach_has_distinct_wire_kind() {
        let msg = ClientMsg::AttachChecked {
            pane_id: 42,
            size: SIZE,
            guard: CheckedPaneAttachment {
                identity: PaneAttachExpectation::Agent(PaneAttachIdentity {
        ...
        assert_eq!(kind, kind::ATTACH_CHECKED);
        assert_ne!(kind, kind::ATTACH);
        assert_eq!(ClientMsg::from_frame(kind, payload).unwrap(), msg);
        assert!(DaemonVersion::current().has_feature(FEATURE_CHECKED_ATTACH));
        let ClientMsg::AttachChecked { guard, .. } = &msg else {
            unreachable!()
        };
        let old_v2 = serde_json::to_vec(&(
            42u64,
            SIZE,
            serde_json::json!({
                "identity": {"agent": "Codex", "session_id": "session-a"},
                "proof": guard.proof,
                "tab": guard.tab,
            }),
            false,
        ...
        .unwrap();
        assert!(ClientMsg::from_frame(kind::ATTACH_CHECKED, old_v2).is_err());
        let mut shell = msg.clone();
        if let ClientMsg::AttachChecked { guard, .. } = &mut shell {
            guard.identity = PaneAttachExpectation::Shell;
        }
        let mut encoded = Vec::new();
        shell.encode(&mut encoded).unwrap();
        let (k, payload) = read_frame(&mut std::io::Cursor::new(encoded)).unwrap();
        assert_eq!(ClientMsg::from_frame(k, payload).unwrap(), shell);
        if let ClientMsg::AttachChecked { guard, .. } = &mut shell {
            guard.identity = PaneAttachExpectation::Owned;
        }
        let mut encoded = Vec::new();
        shell.encode(&mut encoded).unwrap();
        let (k, payload) = read_frame(&mut std::io::Cursor::new(encoded)).unwrap();
        assert_eq!(ClientMsg::from_frame(k, payload).unwrap(), shell);
        assert!(!DaemonVersion::current().has_feature("checked-attach-v3"));
        assert!(ClientMsg::from_frame(kind::ATTACH_CHECKED, b"[]".to_vec()).is_err());
        let old = to_json(&(
            42u64,
            SIZE,
            PaneAttachIdentity {
        ...
        .unwrap();
        assert!(
            ClientMsg::from_frame(kind::ATTACH_CHECKED, old).is_err(),
            "identity-only v1 cannot bypass proof"
        );
    }
