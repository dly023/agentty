...
/// v10 adds target-owned AgentSessions discovery and ValidateAgentSession.
/// v11 adds non-preemptive WorkspaceAttachIfFree for automatic GUI attachment.
/// v12 adds attachment-scoped ownership proofs and conditional release.
/// v13 returns atomic acquisition ownership and proof from WorkspaceAttachIfFree.
/// v14 adds target-owned retained-session identity resolution.
/// v15 adds explicit target-owned stopped-session Resume.
/// v16 adds explicit single-session Sleep.
/// v17 adds whole-tab Sleep with preflight and partial execution results.
/// v18 adds connection-owned history title subscriptions.
pub const CONTROL_VERSION: u32 = 18;
...
pub enum ControlRequest {
    ...
    WorkspaceAttachIfFree {
        id: String,
    },
    WorkspaceDetach {
        id: String,
    },
    WorkspaceAttachmentProof {
    ...
    SleepTab {
        request: SleepTabRequest,
    },
    ValidateAgentSession {
        session: crate::agent_sessions::HistorySession,
    },
    WorkspaceTree {
        workspace: WorkspaceId,
    },
...
impl ControlRequest {
    pub fn deadline(&self) -> Duration {
        ...
        match self {
            ...
            Shells | AgentSessions | TitleWatchOpen { .. } | ValidateAgentSession { .. } | ResolveAgentSession { .. }
            | ResumeStoppedSession { .. } | SleepSession { .. } | SleepTab { .. } => Duration::from_secs(20),
            WorkspaceAttach { .. } | WorkspaceAttachIfFree { .. } | WorkspaceDetach { .. }
            | WorkspaceAttachmentProof { .. } | WorkspaceReleaseProof { .. } => Duration::from_secs(10),
...
impl std::fmt::Debug for AttachmentProof {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("AttachmentProof")
...
pub enum ReplyOk {
    ...
    AgentSessions(crate::agent_sessions::HistorySnapshot),
    ResolvedAgentSession(crate::agent_sessions::HistorySession),
    StoppedSessionResumed {
        workspace: WorkspaceId,
        tab: crate::core::machine::TabId,
        predecessor: u64,
        pane: u64,
    },
    TabSleep(SleepTabReply),
...
pub struct StoppedResumeRequest {
    pub proof: AttachmentProof,
    pub tab: crate::core::machine::TabId,
    pub pane: u64,
    pub identity: crate::agent_sessions::SessionIdentity,
    pub size: super::protocol::WinSize,
}
...
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct SleepTabRequest {
    pub proof: AttachmentProof,
    pub tab: crate::core::machine::TabId,
    pub panes: Vec<SleepTabPane>,
}

/// Signal acknowledgements are not durable exit facts. A failure means later
/// live members were not attempted; the caller must refresh, never replay.
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct SleepTabReply {
    pub accepted: Vec<u64>,
    pub already_stopped: Vec<u64>,
    pub failed: Option<SleepTabFailure>,
}

#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct SleepTabFailure {
    pub pane: u64,
    pub error: WireError,
}
...
impl WireError {
    ...
    pub fn from_io(e: &io::Error) -> Self {
        ...
        let kind = match e.kind() {
            K::Unsupported => WireErrorKind::Unsupported,
...
impl WireErrorKind {
    pub fn to_io_kind(self) -> io::ErrorKind {
        ...
        match self {
            ...
            WireErrorKind::Unsupported => K::Unsupported,
...
pub enum ControlEvent {
    HistoryTitles {
        id: uuid::Uuid,
        event: crate::agent_sessions::live_titles::HistoryTitleEvent,
    },
    Watch {
        id: u64,
        paths: Vec<String>,
...
type WorkspaceClaimGate = Arc<Mutex<WorkspaceClaimState>>;

#[derive(Default)]
struct WorkspaceClaimState {
    proof: Option<AttachmentProof>,
    owned: bool,
    untracked: bool,
    uses: std::collections::HashSet<uuid::Uuid>,
}

impl WorkspaceClaimState {
    fn adopt(&mut self, proof: &AttachmentProof, newly: bool) {
        if self.proof.as_ref() != Some(proof) {
            *self = Self {
...
/// An operation/window use of a connection-owned claim; never persisted.
/// Clones identify the same use and finishing it is idempotent.
#[derive(Clone)]
#[must_use = "retain in the owning view or finish on the blocking I/O lane"]
pub struct WorkspaceUse {
    gate: WorkspaceClaimGate,
    proof: AttachmentProof,
    id: uuid::Uuid,
}
...
impl WorkspaceUse {
    fn register(
        gate: &WorkspaceClaimGate,
        state: &mut WorkspaceClaimState,
        proof: AttachmentProof,
        newly: bool,
    ) -> Self {
        state.adopt(&proof, newly);
        let id = uuid::Uuid::new_v4();
        state.uses.insert(id);
        Self {
            gate: gate.clone(),
            proof,
            id,
    ...
    pub fn proof(&self) -> &AttachmentProof {
        &self.proof
    }
}

struct ClientInner {
    workspace_claims: Mutex<HashMap<crate::core::session::WorkspaceId, WorkspaceClaimGate>>,
...
impl ControlClient {
        ...
        W: Write + Send + 'static,
    {
        ...
        let inner = Arc::new(ClientInner {
            workspace_claims: Mutex::new(HashMap::new()),
    ...
    pub fn call_with_deadline(
        ...
        let workspace = match &req {
            ...
            | ControlRequest::WorkspaceDetach { id } => id.parse().ok(),
            ControlRequest::WorkspaceReleaseProof { proof } => Some(proof.workspace),
            _ => None,
        };
        let Some(workspace) = workspace else {
            return self.call_uncoordinated(req, blob, deadline);
        };
        let gate = self.workspace_claim_gate(workspace)?;
        let mut state = gate
            .lock()
            .map_err(|_| io::Error::other("workspace claim lock poisoned"))?;
        // Raw ownership calls have no tracked consumer lifetime, even when
        // their reply is lost or reuses the current nonce.
        state.untracked = true;
        let acquiring = matches!(req, ControlRequest::WorkspaceAttachIfFree { .. });
        let detaching = matches!(
            req,
            ControlRequest::WorkspaceDetach { .. } | ControlRequest::WorkspaceReleaseProof { .. }
        );
        let result = self.call_uncoordinated(req, blob, deadline);
        if acquiring
        ...
        {
            state.adopt(proof, *newly_acquired);
            state.untracked = true;
        } else if detaching
            && matches!(
                &result,
        ...
        {
            *state = WorkspaceClaimState::default();
        }
        result
    }

    fn workspace_claim_gate(
        &self,
        workspace: crate::core::session::WorkspaceId,
    ) -> io::Result<WorkspaceClaimGate> {
        Ok(self
            .inner
            .workspace_claims
            .lock()
            .map_err(|_| io::Error::other("workspace claim map poisoned"))?
            .entry(workspace)
            .or_default()
            .clone())
    }

    pub fn acquire_workspace_use(
        &self,
        workspace: crate::core::session::WorkspaceId,
    ) -> io::Result<WorkspaceUse> {
        let request = ControlRequest::WorkspaceAttachIfFree {
            id: workspace.to_string(),
        };
        self.acquire_workspace_use_with_deadline(workspace, request.deadline())
    }

    fn acquire_workspace_use_with_deadline(
        &self,
        workspace: crate::core::session::WorkspaceId,
        deadline: Duration,
    ) -> io::Result<WorkspaceUse> {
        ...
        let outcome = self.call_uncoordinated(request, &[], deadline);
        let (proof, newly) = match outcome {
            Ok(ControlResponse {
                reply:
                    ReplyOk::WorkspaceAcquired {
                        proof,
                ...
                ..
            }) if proof.workspace == workspace => (proof, newly_acquired),
            other => {
                state.untracked = true;
                return Err(other.err().unwrap_or_else(|| {
                    io::Error::new(
                        io::ErrorKind::InvalidData,
                        "unexpected workspace acquisition reply",
                    )
                }));
            }
        };
        Ok(WorkspaceUse::register(&gate, &mut state, proof, newly))
    }

    /// Explicit user takeover, followed by target-owned proof readback.
    /// Normal acquisition must never escalate to this operation on refusal.
    pub fn take_back_workspace_use(
        &self,
        workspace: crate::core::session::WorkspaceId,
    ) -> io::Result<(WorkspaceUse, Option<String>)> {
        let gate = self.workspace_claim_gate(workspace)?;
        let mut state = gate
            .lock()
            .map_err(|_| io::Error::other("workspace claim lock poisoned"))?;
        // Keep the gate across both RPCs so same-client acquire/release cannot
        // intervene between takeover and registering its verified use.
        let outcome = (|| {
            let request = ControlRequest::WorkspaceAttach {
                id: workspace.to_string(),
            };
            let deadline = request.deadline();
            let ReplyOk::Attached { took_over_from } =
                self.call_uncoordinated(request, &[], deadline)?.reply
            else {
                return Err(io::Error::new(
                    io::ErrorKind::InvalidData,
                    "unexpected workspace takeover reply",
                ));
            };
            let request = ControlRequest::WorkspaceAttachmentProof { workspace };
            let deadline = request.deadline();
            match self.call_uncoordinated(request, &[], deadline)?.reply {
                ReplyOk::AttachmentProof(proof) if proof.workspace == workspace => {
                    Ok((proof, took_over_from))
                }
                _ => Err(io::Error::new(
                    io::ErrorKind::InvalidData,
                    "unexpected workspace takeover proof",
                )),
            }
        })();
        let (proof, took_over_from) = match outcome {
        ...
        // A raw claim may not have a recorded proof. Same-owner reuse is not
        // evidence that its unknown consumers have gone away.
        let newly = !state.untracked
            || took_over_from.is_some()
            || state.proof.as_ref().is_some_and(|old| old != &proof);
        Ok((
            WorkspaceUse::register(&gate, &mut state, proof, newly),
            took_over_from,
        ))
    }

    pub fn finish_workspace_use(&self, usage: WorkspaceUse) -> io::Result<bool> {
        let gate = self.workspace_claim_gate(usage.proof.workspace)?;
        if !Arc::ptr_eq(&gate, &usage.gate) {
            return Ok(false);
        }
        let mut state = gate
            .lock()
            .map_err(|_| io::Error::other("workspace claim lock poisoned"))?;
        if state.proof.as_ref() != Some(&usage.proof) || !state.uses.remove(&usage.id) {
            return Ok(false);
        }
        if !state.uses.is_empty() || state.untracked || !state.owned {
            return Ok(false);
        }
        *state = WorkspaceClaimState::default();
        let request = ControlRequest::WorkspaceReleaseProof { proof: usage.proof };
        let deadline = request.deadline();
        match self.call_uncoordinated(request, &[], deadline)?.reply {
            ReplyOk::Unit => Ok(true),
            _ => Err(io::Error::other("unexpected workspace release reply")),
        }
    }

    fn call_uncoordinated(
        &self,
        req: ControlRequest,
        blob: &[u8],
        deadline: Duration,
...
mod tests {
    ...
    fn history_fixture() -> crate::agent_sessions::HistorySession {
        crate::agent_sessions::HistorySession {
            provider: crate::agent_sessions::Provider::Codex,
            id: "01900000-0000-7000-8000-000000000001".into(),
            title: "history".into(),
            cwd: "/project".into(),
            source_path: "/provider/session.jsonl".into(),
            modified_ms: 1,
        }
    }
    ...
    fn every_request() -> Vec<ControlRequest> {
        vec![
            ...
            ControlRequest::TitleWatchOpen {
                id: uuid::Uuid::nil(),
                snapshot: crate::agent_sessions::HistorySnapshot {
                    sessions: vec![history_fixture()],
                    missing: vec![],
                    limited: false,
                },
            },
            ControlRequest::TitleWatchClose {
                id: uuid::Uuid::nil(),
            },
            ControlRequest::ResumeStoppedSession {
                request: stopped_resume_fixture(),
            },
            ControlRequest::ResolveAgentSession {
                identity: crate::agent_sessions::SessionIdentity {
                    provider: history_fixture().provider,
                    id: history_fixture().id,
                    cwd: history_fixture().cwd,
                },
            },
            ControlRequest::ValidateAgentSession {
                session: history_fixture(),
            },
    ...
    fn stopped_resume_fixture() -> StoppedResumeRequest {
        StoppedResumeRequest {
            ...
            size: super::super::protocol::WinSize {
                cols: 80,
                rows: 24,
                cell_w: 8,
                cell_h: 16,
    ...
    #[test]
    fn sleep_wire_accepts_only_target_identity() {
        let resume = stopped_resume_fixture();
        ...
        let wire = ControlRequest::SleepSession {
            request: request.clone(),
        };
        assert_eq!(
            serde_json::from_value::<ControlRequest>(serde_json::to_value(&wire).unwrap()).unwrap(),
            wire
        );
        for field in ["program", "args", "roots", "successor", "kill_pid", "cwd"] {
            let mut value = serde_json::to_value(&request).unwrap();
            value[field] = serde_json::json!("untrusted");
            assert!(serde_json::from_value::<SleepSessionRequest>(value).is_err());
        }
    }

    #[test]
    fn sleep_tab_wire_roundtrips_exact_members() {
        let resume = stopped_resume_fixture();
        let value = serde_json::json!({
            "sleep_tab": { "request": {
                "proof": resume.proof,
                "tab": resume.tab,
                "panes": [{"pane": resume.pane, "identity": resume.identity}]
            } }
        });
        let decoded: ControlRequest = serde_json::from_value(value.clone()).unwrap();
        assert_eq!(serde_json::to_value(decoded).unwrap(), value);
        for field in ["program", "args", "roots", "kill_pid", "skip_unbound"] {
            let mut extra = value.clone();
            extra["sleep_tab"]["request"][field] = serde_json::json!(true);
            assert!(serde_json::from_value::<ControlRequest>(extra).is_err());
            let mut extra = value.clone();
            extra["sleep_tab"]["request"]["panes"][0][field] = serde_json::json!(true);
            assert!(serde_json::from_value::<ControlRequest>(extra).is_err());
        }
        let reply = ReplyOk::TabSleep(SleepTabReply {
            accepted: vec![1],
            already_stopped: vec![2],
            failed: Some(SleepTabFailure {
                pane: 3,
                error: WireError::new(WireErrorKind::Other, "interrupted"),
            }),
        });
        assert_eq!(
            serde_json::from_value::<ReplyOk>(serde_json::to_value(&reply).unwrap()).unwrap(),
            reply
        );
    }

    #[test]
    fn stopped_resume_wire_rejects_client_executables_and_roots() {
        for field in ["program", "args", "roots", "successor"] {
            let mut value = serde_json::to_value(stopped_resume_fixture()).unwrap();
            value[field] = serde_json::json!("untrusted");
            assert!(serde_json::from_value::<StoppedResumeRequest>(value).is_err());
        }
    }

    fn every_reply() -> Vec<ControlReply> {
        vec![
            ControlReply::Ok(ReplyOk::StoppedSessionResumed {
                workspace: WorkspaceId::new(),
                tab: crate::core::machine::TabId::new(),
                predecessor: 7,
                pane: 42,
            }),
            ControlReply::Ok(ReplyOk::ResolvedAgentSession(history_fixture())),
            ...
            ControlReply::Ok(ReplyOk::AgentSessions(
                crate::agent_sessions::HistorySnapshot {
                    sessions: vec![],
                    missing: vec![crate::agent_sessions::Provider::Claude],
                    limited: false,
                },
            )),
    ...
    fn every_event() -> Vec<ControlEvent> {
        vec![
            ControlEvent::HistoryTitles {
                id: uuid::Uuid::nil(),
                event: crate::agent_sessions::live_titles::HistoryTitleEvent::Unavailable,
            },
            ControlEvent::HistoryTitles {
                id: uuid::Uuid::nil(),
                event: crate::agent_sessions::live_titles::HistoryTitleEvent::Names(vec![
                    crate::agent_sessions::live_titles::HistoryTitle {
                        provider: history_fixture().provider,
                        id: history_fixture().id,
    ...
    fn deadlines_match_the_contract_table() {
        ...
        let cases: Vec<(R, Duration)> = vec![
            ...
            (
                R::ResumeStoppedSession {
                    request: stopped_resume_fixture(),
                },
                s(20),
            ),
            (
                R::ResolveAgentSession {
                    identity: crate::agent_sessions::SessionIdentity {
                        provider: history_fixture().provider,
                        id: history_fixture().id,
                        cwd: history_fixture().cwd,
    ...
    #[test]
    fn workspace_claims_progress_independently() {
        let a = AttachmentProof {
            workspace: crate::core::session::WorkspaceId::new(),
            nonce: uuid::Uuid::new_v4(),
        };
        let b = AttachmentProof {
            workspace: crate::core::session::WorkspaceId::new(),
            nonce: uuid::Uuid::new_v4(),
        };
        ...
        let client = Arc::new(client_with_peer(no_events(), move |mut sock| {
            sock.set_read_timeout(Some(Duration::from_secs(5))).unwrap();
            sock.set_write_timeout(Some(Duration::from_secs(5)))
                .unwrap();
            let read_acquire = |sock: &mut TcpStream, proof: &AttachmentProof| {
                let ControlClientMsg::Request {
                    req_id,
                    req: ControlRequest::WorkspaceAttachIfFree { id },
                } = ControlClientMsg::read(sock).unwrap()
                else {
                    panic!("expected acquisition")
                };
                assert_eq!(id, proof.workspace.to_string());
                req_id
            };
            let first = read_acquire(&mut sock, &pa);
            seen_tx.send(()).unwrap();
            let second = read_acquire(&mut sock, &pb);
            reply_to(
                &mut sock,
                second,
                ReplyOk::WorkspaceAcquired {
            ...
            for expected in [pb, pa] {
                let ControlClientMsg::Request {
                    req_id,
                    req: ControlRequest::WorkspaceReleaseProof { proof },
                } = ControlClientMsg::read(&mut sock).unwrap()
                else {
                    panic!("expected final tracked use release")
                };
                assert_eq!(proof, expected);
                reply_to(&mut sock, req_id, ReplyOk::Unit);
            }
            done_tx.send(()).unwrap();
        }));
        let worker = client.clone();
        let first_workspace = a.workspace;
        let first = thread::spawn(move || worker.acquire_workspace_use(first_workspace));
        seen_rx.recv_timeout(Duration::from_secs(5)).unwrap();
        let second = client.acquire_workspace_use(b.workspace).unwrap();
        assert_eq!(second.proof(), &b);
        release_tx.send(()).unwrap();
        let first = first.join().unwrap().unwrap();
        assert_eq!(first.proof(), &a);
    ...
    #[test]
    fn workspace_take_back_interrupted_readback_pins_uncertain_claim() {
        for after_attach in [false, true] {
            ...
            let client = client_with_peer(no_events(), move |mut sock| {
                ...
                assert_eq!(
                    ...
                    ControlRequest::WorkspaceAttachIfFree {
                        id: expected.workspace.to_string()
                    }
                );
                reply_to(
                    &mut sock,
                ...
                let ControlClientMsg::Request { req_id, req } =
                    ...
                    panic!("expected explicit takeover");
                };
                assert_eq!(
                    req,
                ...
                if after_attach {
                    ...
                    let ControlClientMsg::Request { req, .. } =
                        ControlClientMsg::read(&mut sock).unwrap()
                    else {
                        panic!("expected target proof readback");
                    };
                    assert_eq!(
                        req,
            ...
            let old = client.acquire_workspace_use(proof.workspace).unwrap();
            assert!(client.take_back_workspace_use(proof.workspace).is_err());
            // No RPC on the broken link: uncertainty disarms old cleanup first.
    ...
    #[test]
    fn workspace_claim_uncertainty_prevents_tracked_cleanup() {
        for case in ["timeout", "unexpected", "foreign"] {
            let proof = AttachmentProof {
                workspace: crate::core::session::WorkspaceId::new(),
                nonce: uuid::Uuid::new_v4(),
            };
            let foreign = AttachmentProof {
                workspace: crate::core::session::WorkspaceId::new(),
                nonce: uuid::Uuid::new_v4(),
            };
            let (peer_proof, peer_foreign) = (proof.clone(), foreign.clone());
            let (done_tx, done_rx) = mpsc::channel();
            let client = client_with_peer(no_events(), move |mut sock| {
                sock.set_read_timeout(Some(Duration::from_secs(5))).unwrap();
                sock.set_write_timeout(Some(Duration::from_secs(5)))
                ...
                for first in [true, false] {
                    ...
                    assert_eq!(id, peer_proof.workspace.to_string());
                    let reply = if first {
                        ReplyOk::WorkspaceAcquired {
                        ...
                        match case {
                            "timeout" => {
                                assert!(
                                    matches!(ControlClientMsg::read(&mut sock).unwrap(), ControlClientMsg::Cancel { req_id: canceled } if canceled == req_id)
                                );
                                // Even a late purported fresh claim must not rearm cleanup.
                                ReplyOk::WorkspaceAcquired {
                                    proof: peer_proof.clone(),
                                    newly_acquired: true,
                                }
                            }
                            "unexpected" => ReplyOk::Pong,
                            "foreign" => ReplyOk::WorkspaceAcquired {
                                proof: peer_foreign.clone(),
                                newly_acquired: true,
                            },
                            _ => unreachable!(),
                        }
                ...
                let ControlClientMsg::Request {
                    ...
                    panic!("uncertain acquisition triggered a release")
                };
                reply_to(&mut sock, req_id, ReplyOk::Pong);
                assert!(
                    ControlClientMsg::read(&mut sock).is_err(),
                    "late reply rearmed cleanup"
                );
                done_tx.send(()).unwrap();
            });
            let original = client.acquire_workspace_use(proof.workspace).unwrap();
            let result = client.acquire_workspace_use_with_deadline(
                proof.workspace,
                if case == "timeout" {
            ...
            assert_eq!(
                result
                    .err()
                    .expect("uncertain acquisition cannot return a use")
                    .kind(),
                if case == "timeout" {
                    io::ErrorKind::TimedOut
                } else {
            ...
            forged.proof = foreign;
            assert!(!client.finish_workspace_use(original.clone()).unwrap());
            assert!(!client.finish_workspace_use(forged.clone()).unwrap());
            assert!(matches!(
                client.call(ControlRequest::Ping).unwrap(),
                ReplyOk::Pong
            ));
            // The ordered Pong proves the late reply cannot rearm cleanup.
            assert!(!client.finish_workspace_use(original).unwrap());
            assert!(!client.finish_workspace_use(forged).unwrap());
            client.close();
            done_rx.recv_timeout(Duration::from_secs(5)).unwrap();
        }
    }
