...
fn resume_spawn_config(
    pane: u64,
    cwd: PathBuf,
    invocation: &crate::core::cli_agent::ResumeInvocation,
    workspace: &str,
) -> SpawnConfig {
    let initial_cwd = Some(cwd);
    let mut cmd = CommandBuilder::new(invocation.program());
    cmd.args(invocation.args());
    // No shell was launched: keep the inherited/configured SHELL and do not
    // attach shell history integration to the provider executable.
    apply_common_command_setup(&mut cmd, &initial_cwd, pane, Some(workspace), "");
    SpawnConfig {
        cmd,
        initial_cwd,
        integration_dir: None,
        remote: None,
        // An agent invocation is not a shell preference. Persisting it as a
        // ShellSpec would turn ordinary restoration into an implicit Resume.
        shell: None,
    }
}
...
pub struct DaemonPane {
    ...
    scrollback_write: ScrollbackWriter,
    gate: Arc<OutputGate>,
...
#[derive(Default)]
struct ScrollbackWriter(Mutex<bool>);

impl ScrollbackWriter {
    fn persist<T>(&self, final_frame: bool, write: impl FnOnce() -> T) -> Option<T> {
        let mut sealed = self.0.lock().unwrap();
        if *sealed {
            return None;
        }
        let value = write();
    ...
    fn forget(&self, remove: impl FnOnce()) {
        let mut sealed = self.0.lock().unwrap();
        *sealed = true;
        remove();
    }
...
impl DeathReporter {
    ...
    fn report(&self, state: &Mutex<PaneState>, shutting_down: &AtomicBool) {
        ...
        st.exit_code = code;
        let pane = st.id;
        if shutting_down.load(Ordering::SeqCst) && !self.explicit_stop.load(Ordering::SeqCst) {
            drop(st);
        ...
        drop(st);
        crate::core::machine::observe_pane(pane, |p| p.observe_terminal_exit(code));
        if subscribed {
            return;
        }
...
impl DaemonPane {
    ...
    pub(crate) fn spawn_resume(
        ...
        let spawn = resume_spawn_config(id, cwd, invocation, workspace);
        Self::spawn_config(
            id,
            size,
            spawn,
    ...
    fn over_pty(
        ...
        let pane = Arc::new(Self {
            ...
            explicit_stop: death.explicit_stop.clone(),
            scrollback_write: ScrollbackWriter::default(),
            gate: gate.clone(),
            state: state.clone(),
                        ...
                        Ok(n) => {
                                ...
                                && facts_changed(&before, &after)
                            {
                                crate::core::machine::observe_pane(pane, |p| {
                                    p.observe_agent_identity(after.agent, after.cwd.as_deref());
    ...
    pub fn attach_checked(
        ...
        subscriber: Sender<DaemonMsg>,
        allow_remote_clipboard_write: bool,
        identity: &crate::daemon::protocol::PaneAttachExpectation,
    ) -> Result<u64, &'static str> {
        let mut st = self.state.lock().unwrap();
        let epoch =
            attach_checked_subscriber(&mut st, subscriber, allow_remote_clipboard_write, identity)?;
        self.gate.reset();
        Ok(epoch)
    }
    ...
    /// Serialize capture as well as the atomic write: a periodic frame taken
    /// before EOF must never overwrite the final retirement snapshot.
    pub(super) fn persist_scrollback(&self) -> u64 {
        self.persist_scrollback_frame(false)
    }

    pub(super) fn finalize_scrollback(&self) {
        self.persist_scrollback_frame(true);
    }

    pub(super) fn forget_scrollback(&self) {
        self.scrollback_write
            .forget(|| crate::daemon::scrollback::forget(self.id));
    }

    fn persist_scrollback_frame(&self, final_frame: bool) -> u64 {
        self.scrollback_write
            .persist(final_frame, || {
                let (segments, title, mark) = self.scrollback_snapshot();
                crate::daemon::scrollback::save(self.id, &segments, title.as_deref());
                mark
            })
            .unwrap_or_else(|| self.scrollback_mark())
    }
    ...
    pub(crate) fn sleep_session(
        &self,
        identity: &crate::agent_sessions::SessionIdentity,
    ) -> std::io::Result<()> {
        let state = self.state.lock().unwrap();
        self.check_sleep_state(&state, identity)?;
        self.explicit_stop.store(true, Ordering::SeqCst);
        // The observer, not this acknowledgement, records actual termination.
        self.hangup();
        drop(state);
        Ok(())
    }

    pub(crate) fn check_sleep_session(
        &self,
        identity: &crate::agent_sessions::SessionIdentity,
    ) -> std::io::Result<()> {
        self.check_sleep_state(&self.state.lock().unwrap(), identity)
    }
    ...
    fn check_sleep_state(
        ...
        let expected = match identity.provider {
            crate::agent_sessions::Provider::Codex => crate::core::cli_agent::CLIAgent::Codex,
            crate::agent_sessions::Provider::Claude => crate::core::cli_agent::CLIAgent::Claude,
        };
        if !matches!(&self.backend, PaneBackend::Pty(_))
            || !state.alive
            || state.agent.is_some_and(|agent| agent != expected)
            || (state.agent.is_some()
                && state
                    .agent_session
                    .as_ref()
                    ...
                    != Some(id.as_str()))
            || (state.agent.is_none() && state.agent_session.is_some())
        {
            return Err(std::io::Error::new(
                std::io::ErrorKind::PermissionDenied,
                "sleep runtime identity changed",
            ));
        }
        Ok(())
    }
...
fn attach_checked_subscriber(
    st: &mut PaneState,
    subscriber: Sender<DaemonMsg>,
    allow_remote_clipboard_write: bool,
    expectation: &crate::daemon::protocol::PaneAttachExpectation,
) -> Result<u64, &'static str> {
    if !st.alive {
        return Err("pane is no longer running");
    }
    if st.subscriber.is_some() {
        return Err("pane already has a controller");
    }
    let matches = match expectation {
        crate::daemon::protocol::PaneAttachExpectation::Stopped(_) => false,
        crate::daemon::protocol::PaneAttachExpectation::Owned => true,
        crate::daemon::protocol::PaneAttachExpectation::Shell => {
            st.agent.is_none() && st.agent_session.is_none()
        }
        crate::daemon::protocol::PaneAttachExpectation::Agent(identity) => {
            !identity.session_id.trim().is_empty()
                && st.agent == Some(identity.agent)
                && st
                    .agent_session
                    .as_ref()
    ...
    if !matches {
        return Err("pane agent identity changed");
    }
    Ok(attach_subscriber_with_permissions(
        st,
        subscriber,
        allow_remote_clipboard_write,
    ))
...
fn apply_agent_signals(
    ...
    for event in &events {
        if let Some(provider) = event.agent {
            if st.agent.is_some_and(|current| current != provider) {
                continue;
            }
            if st.agent.is_none() {
                apply_agent(st, Some((provider, Vec::new())));
            }
        }
        st.agent_session
...
fn apply_agent(
    ...
    if st.agent == agent && agent.is_some() {
        stamp_launch_argv(st, argv);
        return;
    }
    // A different provider cannot inherit the old provider's session or
    // launch flags. Publish the cleared status before the replacement identity.
    if st.agent_session.is_some() {
...
mod tests {
    #[test]
    fn stopped_resume_launch_uses_argv_not_shell_input_or_restore_preference() {
        let id = "01900000-0000-7000-8000-000000000001";
        for agent in [
            crate::core::cli_agent::CLIAgent::Codex,
            crate::core::cli_agent::CLIAgent::Claude,
        ] {
            let invocation = agent.resume_invocation(id).unwrap();
            let cwd = std::env::temp_dir();
            let spawn = resume_spawn_config(42, cwd.clone(), &invocation, "target-workspace");
            let argv: Vec<_> = spawn
                .cmd
                .get_argv()
                .iter()
                .map(|s| s.to_string_lossy().into_owned())
                .collect();
            let expected: Vec<_> = std::iter::once(invocation.program().to_string())
                .chain(invocation.args().iter().cloned())
                .collect();
            assert_eq!(argv, expected);
            assert_eq!(spawn.initial_cwd, Some(cwd));
            assert!(spawn.shell.is_none());
            assert!(spawn.integration_dir.is_none());
            assert!(spawn.remote.is_none());
        }
        let env = pane_environment(&std::collections::HashMap::new(), false, 42, None, "");
        assert!(
            !env.iter().any(|(key, _)| key == SHELL_ENV),
            "an agent is not the user's shell"
        );
    }
    ...
    #[test]
    fn scrollback_retirement_seals_capture_and_forget() {
        for finalize_first in [false, true] {
            let writer = ScrollbackWriter::default();
            assert_eq!(writer.persist(false, || "periodic"), Some("periodic"));
            if finalize_first {
                assert_eq!(writer.persist(true, || "final"), Some("final"));
                assert_eq!(
                    writer.persist(false, || panic!("late snapshot capture")),
                    None::<()>
                );
            }
            let removed = std::cell::Cell::new(false);
            writer.forget(|| removed.set(true));
            assert!(removed.get());
            assert_eq!(
                writer.persist(false, || panic!("deleted frame recreated")),
                None::<()>
            );
            assert_eq!(
                writer.persist(true, || panic!("reaper recreated deleted frame")),
                None::<()>
            );
        }
    }
    ...
    #[test]
    fn natural_pane_exit_clears_runtime_but_keeps_recovery() {
        for explicit in [false, true] {
            use crate::core::cli_agent::{AgentSessionState, CLIAgent};
            use crate::core::machine::{
                AgentFacts, MachineStore, OBSERVE_SLOT, PaneSeed, publish_observations,
            ...
            let tab = store
                .tab_create(ws.id, None, PaneSeed::bare(79), None, None)
                .unwrap();
            store.note_pane_facts(79, |p| {
                p.observe_agent_identity(
                    Some(AgentFacts {
                        agent: CLIAgent::Codex,
                        session_id: Some("observed-session".into()),
            ...
            let binding = store.pane(79).unwrap().recovery_binding;
            store.note_pane_facts(79, |p| {
                p.provider_title = Some(crate::core::machine::ProviderTitle {
                    agent: CLIAgent::Codex,
            ...
            st.agent_argv = Some(vec!["codex".into()]);
            st.agent_session = Some(AgentSessionState {
                session_id: Some("observed-session".into()),
                ..Default::default()
            });
            ...
            let reporter =
                DeathReporter::new(|| panic!("a subscribed pane must retain its carrier"));
            reporter.explicit_stop.store(explicit, Ordering::SeqCst);
            // Explicit stop remains a confirmed exit even if disposal sets teardown.
            reporter.report(&state, &AtomicBool::new(explicit));
            reporter.report(&state, &AtomicBool::new(explicit));
            let record = store.pane(79).unwrap();
            assert!(
                record.agent.is_none(),
                "natural exit must not retain automatic launch identity"
            );
            assert!(!record.live);
            assert_eq!(
                serde_json::to_value(&record).unwrap()["exit"],
                serde_json::json!({"code": null})
            );
            assert_eq!(record.recovery_binding, binding);
            assert_eq!(record.provider_title_text(), Some("kept title"));
            let st = state.lock().unwrap();
            ...
            assert_eq!(loaded.pane(79).unwrap().recovery_binding, binding);
            assert!(loaded.pane(79).unwrap().agent.is_none());
            assert_eq!(loaded.workspace(ws.id).unwrap().tabs[0].id, tab.id);
        }
    }

    #[test]
    fn natural_pane_exit_clears_status_without_detected_provider() {
        let mut st = test_state(true);
        st.agent_session = Some(crate::core::cli_agent::AgentSessionState::default());
        st.agent_argv = Some(vec!["stale".into()]);
        let (tx, rx) = mpsc::channel();
        st.subscriber = Some(tx);
        let state = Mutex::new(st);
        DeathReporter::new(|| {}).report(&state, &AtomicBool::new(false));
        let st = state.lock().unwrap();
        assert!(st.agent.is_none());
        assert!(st.agent_session.is_none());
        assert!(st.agent_argv.is_none());
        ...
        assert!(matches!(
            events.as_slice(),
            [DaemonMsg::AgentStatus(None), DaemonMsg::Exited { .. }]
        ));
    }
    ...
    #[test]
    fn foreground_provider_replacement_clears_old_session_before_publishing_identity() {
        use crate::core::cli_agent::{AgentSessionState, AgentStatus, CLIAgent};
        let mut st = test_state(true);
        let (tx, rx) = mpsc::channel();
        st.subscriber = Some(tx);
        st.agent = Some(CLIAgent::Claude);
        st.agent_argv = Some(vec![
            "claude".into(),
        ...
        apply_agent(&mut st, Some((CLIAgent::Codex, vec![])));
        assert_eq!(st.agent, Some(CLIAgent::Codex));
        assert!(
            st.agent_session.is_none(),
            "new provider cannot inherit old session ID"
        );
        assert!(
            st.agent_argv.is_none(),
            "empty argv cannot inherit previous provider flags"
        );
    ...
    #[test]
    fn foreign_provider_hook_cannot_replace_current_session() {
        use crate::core::cli_agent::{AgentSessionState, CLIAgent};
        let mut st = test_state(true);
        st.agent = Some(CLIAgent::Codex);
        st.agent_session = Some(AgentSessionState {
        ...
        apply_signals(
            ...
            sniffer.feed(
                concat!(
                    ...
                    r#"{"v":1,"agent":"claude","event":"session-start","session_id":"old-claude"}"#,
                    "\x07",
                )
                .as_bytes(),
            ),
        ...
        assert_eq!(st.agent, Some(CLIAgent::Codex));
        assert_eq!(
            st.agent_session, before,
            "a foreign hook cannot relabel current session state"
        );
    }
    ...
    #[test]
    fn checked_owned_attachment_is_live_and_non_preemptive() {
        use crate::daemon::protocol::PaneAttachExpectation;
        for case in ["shell", "provisional", "dead", "busy"] {
            let mut st = test_state(true);
            let (owner, _owner_rx) = mpsc::channel();
            match case {
                "provisional" => st.agent = Some(crate::core::cli_agent::CLIAgent::Codex),
                "dead" => st.alive = false,
                "busy" => st.subscriber = Some(owner),
                _ => {}
            }
            let epoch = st.subscriber_epoch;
            let (tx, rx) = mpsc::channel();
            let result =
                attach_checked_subscriber(&mut st, tx, false, &PaneAttachExpectation::Owned);
            assert_eq!(result.is_ok(), matches!(case, "shell" | "provisional"));
            if result.is_err() {
                assert_eq!(st.subscriber_epoch, epoch);
                assert!(rx.try_recv().is_err());
            }
        }
    }
    ...
    #[test]
    fn checked_attachment_shell_requires_empty_agent_facts() {
        use crate::core::cli_agent::{AgentSessionState, CLIAgent};
        use crate::daemon::protocol::PaneAttachExpectation;
        for case in ["shell", "agent", "residual", "dead", "busy"] {
            let mut st = test_state(true);
            let (owner, _owner_rx) = mpsc::channel();
            match case {
                "agent" => st.agent = Some(CLIAgent::Codex),
                "residual" => st.agent_session = Some(AgentSessionState::default()),
                "dead" => st.alive = false,
                "busy" => st.subscriber = Some(owner),
                _ => {}
            }
            st.ring.append(b"shell screen");
            let epoch = st.subscriber_epoch;
            let (tx, rx) = mpsc::channel();
            let result =
                attach_checked_subscriber(&mut st, tx, true, &PaneAttachExpectation::Shell);
            assert_eq!(result.is_ok(), case == "shell", "{case}");
            if case == "shell" {
                assert_eq!(st.subscriber_epoch, epoch + 1);
                assert!(matches!(rx.try_recv(), Ok(DaemonMsg::Size(_))));
            } else {
                assert_eq!(st.subscriber_epoch, epoch);
                assert!(!st.allow_remote_clipboard_write);
                assert!(rx.try_recv().is_err(), "{case}: leaked replay");
                assert_eq!(st.subscriber.is_some(), case == "busy");
            }
        }
    }

    #[test]
    fn checked_attachment_rejects_changed_or_occupied_pane() {
        use crate::core::cli_agent::{AgentSessionState, CLIAgent};
        use crate::daemon::protocol::PaneAttachIdentity;
        let identity = PaneAttachIdentity {
            agent: CLIAgent::Codex,
        ...
        for case in [
            ...
            st.ring.append(b"private replay");
            let (owner, owner_rx) = mpsc::channel();
            let mut expected = identity.clone();
            match case {
                "dead" => st.alive = false,
                "busy" => st.subscriber = Some(owner),
                "foreign-provider" => st.agent = Some(CLIAgent::Claude),
                "foreign-session" => {
                    st.agent_session.as_mut().unwrap().session_id = Some("session-b".into())
                }
                "unknown" => st.agent_session = None,
                "empty" => {
                    expected.session_id.clear();
                    st.agent_session.as_mut().unwrap().session_id = Some(String::new());
                }
                _ => unreachable!(),
            }
            let (tx, rx) = mpsc::channel();
            let epoch = st.subscriber_epoch;
            assert!(
                attach_checked_subscriber(
                    &mut st,
                    tx,
                    true,
                    &crate::daemon::protocol::PaneAttachExpectation::Agent(expected)
                )
                .is_err(),
                "{case}"
            );
            assert_eq!(st.subscriber_epoch, epoch, "{case}");
            assert!(!st.allow_remote_clipboard_write, "{case}");
            assert!(rx.try_recv().is_err(), "{case}: leaked replay");
            assert_eq!(st.subscriber.is_some(), case == "busy");
            if case == "busy" {
                notify(&mut st, DaemonMsg::Output(b"still owned".to_vec()));
                assert!(matches!(owner_rx.try_recv(), Ok(DaemonMsg::Output(_))));
            }
        }
    }

    #[test]
    fn checked_attachment_installs_only_matching_live_identity() {
        use crate::core::cli_agent::{AgentSessionState, CLIAgent};
        use crate::daemon::protocol::PaneAttachIdentity;
        let mut st = test_state(true);
        let identity = PaneAttachIdentity {
        ...
        assert_eq!(
            attach_checked_subscriber(
                ...
                tx,
                true,
                &crate::daemon::protocol::PaneAttachExpectation::Agent(identity.clone())
            ),
            Ok(1)
        );
        assert!(st.allow_remote_clipboard_write);
        ...
        assert!(matches!(replay.first(), Some(DaemonMsg::Size(_))));
        assert!(
            replay
                .iter()
                .any(|msg| matches!(msg, DaemonMsg::Snapshot(bytes) if bytes == b"screen"))
        );
        let (next, rx) = mpsc::channel();
        assert!(
            attach_checked_subscriber(
                &mut st,
                next,
        ...
        assert_eq!(st.subscriber_epoch, 1);
        assert!(rx.try_recv().is_err());
    }
