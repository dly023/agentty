...
mod tests {
    ...
    fn pin_config_dir() -> std::sync::MutexGuard<'static, ()> {
        // sweep() intentionally removes every snapshot outside its keep set;
        // unique pane ids cannot isolate tests sharing the same snapshot dir.
        static FILE_TESTS: std::sync::Mutex<()> = std::sync::Mutex::new(());
        let guard = FILE_TESTS.lock().unwrap();
