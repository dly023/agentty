...
pub fn refresh_hooks_at_launch() -> usize {
    refresh_hooks_in_machine_scope(
...
/// A secondary workbench does not own the user's machine-wide provider hooks.
/// Check before constructing a Host or inspecting provider files.
fn refresh_hooks_in_machine_scope(
    debug: bool,
    invocation: Option<&Path>,
    machine: Option<&Path>,
    refresh: impl FnOnce() -> usize,
) -> usize {
    if debug || !matches!((invocation, machine), (Some(a), Some(b)) if a == b) {
        return 0;
    }
    refresh()
}
...
mod tests {
    ...
    #[test]
    fn automatic_hook_refresh_requires_machine_config_scope() {
        let machine = Some(Path::new("/machine/config"));
        let isolated = Some(Path::new("/isolated/config"));
        let lexical_alias = Some(Path::new("/machine/other/../config"));
        for (debug, invocation, owner, allowed) in [
            (false, machine, machine, true),
            (true, machine, machine, false),
            (false, isolated, machine, false),
            (false, lexical_alias, machine, false),
            (false, None, machine, false),
            (false, machine, None, false),
            (false, None, None, false),
            (true, isolated, machine, false),
        ] {
            let mut calls = 0;
            let result = refresh_hooks_in_machine_scope(debug, invocation, owner, || {
                calls += 1;
                7
            });
            assert_eq!(calls, usize::from(allowed));
            assert_eq!(result, if allowed { 7 } else { 0 });
        }
    }
