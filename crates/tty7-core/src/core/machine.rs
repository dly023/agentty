...
pub struct PaneRecord {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub provider_title: Option<ProviderTitle>,
    #[serde(default)]
    pub ssh_spec: Option<Box<NativeSshSpec>>,
    #[serde(default)]
    pub agent: Option<AgentFacts>,
    /// Last complete target-observed session, not evidence of a live agent.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub recovery_binding: Option<AgentRecoveryBinding>,
    /// Target-issued explicit launch, not observed runtime identity. Only the
    /// stopped Resume transaction sets it; provisional detection may not erase
    /// that transaction's requested recovery candidate.
    #[serde(default, skip_serializing_if = "std::ops::Not::not")]
    pub resume_pending: bool,
    /// Durable target confirmation, distinct from an absent live observation.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub exit: Option<PaneExit>,
...
impl PaneRecord {
    /// Only the target daemon's observed-facts path calls this transition.
    pub(crate) fn observe_agent_identity(&mut self, agent: Option<AgentFacts>, cwd: Option<&str>) {
        if self.exit.is_some() {
            return;
        }
        if let Some(facts) = &agent
            && !self.awaiting_resumed_identity(facts)
        {
            self.resume_pending = false;
            self.recovery_binding = facts
                .session_id
                .as_ref()
                .filter(|id| !id.trim().is_empty())
                .zip(cwd.filter(|cwd| Path::new(cwd).is_absolute()))
                .map(|(id, cwd)| AgentRecoveryBinding {
                    agent: facts.agent,
                    session_id: id.clone(),
                    cwd: cwd.to_string(),
                    launch_argv: facts.launch_argv.clone(),
                });
        }
        self.agent = agent;
        if self.provider_title_text().is_none() {
            self.provider_title = None;
        }
    }

    pub(crate) fn observe_terminal_exit(&mut self, code: Option<i32>) {
        if self.exit.is_none() {
            self.observe_agent_identity(None, None);
            self.exit = Some(PaneExit { code });
        }
        self.resume_pending = false;
        self.live = false;
    }

    fn awaiting_resumed_identity(&self, facts: &AgentFacts) -> bool {
        self.resume_pending
            && facts
                .session_id
                .as_deref()
            ...
            && self
                .recovery_binding
                .as_ref()
                .is_some_and(|b| b.agent == facts.agent)
    }

    /// A title belongs to the last target-observed session even after exit.
    /// This is metadata eligibility, never evidence that a process is live.
    pub(crate) fn provider_title_binding(&self) -> Option<&AgentRecoveryBinding> {
        let binding = self.recovery_binding.as_ref()?;
        if self.ssh_spec.is_some() {
            return None;
        }
        if let Some(agent) = &self.agent
            && !self.awaiting_resumed_identity(agent)
            && (agent.agent != binding.agent
                || agent.session_id.as_deref() != Some(&binding.session_id))
        {
            return None;
        }
        Some(binding)
    ...
    pub fn provider_title_text(&self) -> Option<&str> {
        let title = self.provider_title.as_ref()?;
        let binding = self.provider_title_binding()?;
        if binding.agent != title.agent || binding.session_id != title.session_id {
            return None;
        }
...
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
pub struct AgentRecoveryBinding {
    pub agent: CLIAgent,
    pub session_id: String,
    pub cwd: String,
...
impl MachineStore {
    ...
    /// Called with the attachment handover lock held. Keep layout stable until
    /// the caller installs the subscriber under the pane-state lock.
    pub(crate) fn with_pane_owner<T>(
        &self,
        ...
        if unique_pane_owner(&machine, pane)? != Some((workspace, tab)) {
            return Err(io::Error::new(
                io::ErrorKind::PermissionDenied,
                "pane ownership changed or is ambiguous",
            ));
        }
        let result = attach();
        drop(machine);
    ...
    pub fn pane_replace(
        ...
        let live = self.seed_is_live(new.pane);
        self.mutate(origin, |m| {
            let delta = replace_pane(m, workspace, old, new, live)?;
            Ok(((), vec![(workspace, delta)]))
        })
    }

    pub(crate) fn check_stopped_session(
        &self,
        workspace: WorkspaceId,
        tab: TabId,
        pane: u64,
        identity: &crate::agent_sessions::SessionIdentity,
    ) -> io::Result<()> {
        self.with_stopped_session(workspace, tab, pane, identity, |_| Ok(()))
    }

    pub(crate) fn with_stopped_session<T>(
        &self,
        workspace: WorkspaceId,
        tab: TabId,
        pane: u64,
        identity: &crate::agent_sessions::SessionIdentity,
        read: impl FnOnce(&PaneRecord) -> io::Result<T>,
    ) -> io::Result<T> {
        let machine = self.locked();
        read(stopped_session_record(
            &machine, workspace, tab, pane, identity,
        )?)
    }

    /// Hold exact layout/binding ownership through the bounded signal request.
    /// The callback must not join the process or publish machine facts here.
    pub(crate) fn with_live_session<T>(
        ...
        pane: u64,
        identity: &crate::agent_sessions::SessionIdentity,
        stop: impl FnOnce(&PaneRecord) -> io::Result<T>,
    ) -> io::Result<T> {
        let machine = self.locked();
        let record = bound_session_record(&machine, workspace, tab, pane, identity)?;
        require_live_session(record)?;
        stop(record)
    }

    /// Whole-tab preflight holds the same ownership lock as single-session
    /// Sleep. Every member is checked before the callback can signal any PTY.
    pub(crate) fn with_tab_sessions<T>(
        &self,
        request: &crate::daemon::control::SleepTabRequest,
        stop: impl FnOnce(&[&PaneRecord]) -> io::Result<T>,
    ) -> io::Result<T> {
        let machine = self.locked();
        let tabs: Vec<_> = machine
            .workspaces
            .iter()
            .filter(|w| w.id == request.proof.workspace)
            .flat_map(|w| &w.tabs)
            .filter(|t| t.id == request.tab)
            .collect();
        let [tab] = tabs.as_slice() else {
            return Err(refuse("sleep tab ownership changed"));
        };
        let members = tab.root.pane_ids();
        if members.is_empty()
            || !members
                .iter()
                .copied()
                .eq(request.panes.iter().map(|p| p.pane))
        {
            return Err(refuse("sleep tab membership changed"));
        }
        let records = request
            .panes
            .iter()
            .map(|member| {
                let record = bound_session_record(
                    &machine,
    ...
    /// The caller holds target attachment authority and has validated the
    /// provider source. Allocation happens only after the final layout/binding
    /// checks, under the same transaction that consumes the predecessor.
    /// Any allocated process must be retained by the caller until this returns
    /// so failure cleanup cannot join a reader while the layout lock is held.
    pub(crate) fn resume_stopped_with(
        &self,
        workspace: WorkspaceId,
        tab: TabId,
        old: u64,
        identity: &crate::agent_sessions::SessionIdentity,
        spawn: impl FnOnce(&PaneRecord) -> io::Result<PaneSeed>,
    ) -> io::Result<u64> {
        self.mutate(None, |m| {
            let record = stopped_session_record(m, workspace, tab, old, identity)?.clone();
            let binding = record
                .recovery_binding
                .as_ref()
                .expect("validated stopped binding");
            let seed = spawn(&record)?;
            let new_id = seed.pane;
            if seed.agent.is_some()
                || seed.ssh_spec.is_some()
                || seed.cwd.as_deref() != Some(binding.cwd.as_str())
            {
                return Err(refuse("invalid stopped-session successor"));
            }
            let mut delta = replace_pane(m, workspace, old, seed, true)?;
            let successor = m
                .panes
                .iter_mut()
                .find(|p| p.id == new_id)
                .expect("registered above");
            successor.recovery_binding = record.recovery_binding.clone();
            successor.resume_pending = true;
            if record.provider_title_text().is_some() {
                successor.provider_title = record.provider_title.clone();
            }
            if let LayoutDelta::TabRestructured { pane, .. } = &mut delta {
                *pane = Some(successor.clone());
            }
            Ok((new_id, vec![(workspace, delta)]))
        })
    }
    ...
    pub fn note_pane_facts(self: &Arc<Self>, pane: u64, update: impl FnOnce(&mut PaneRecord)) {
        ...
        let result: io::Result<()> = self.mutate_with(None, Persist::Soon, |m| {
            ...
            update(record);
            record.id = before.id;
            if before.exit.is_some() {
                record.exit = before.exit.clone();
                record.recovery_binding = before.recovery_binding.clone();
            }
            if record.exit.is_some() {
...
/// Count occurrences, not just containers: two leaves in one split are also
/// ambiguous. Callers retain the machine lock while acting on this result.
fn unique_pane_owner(m: &Machine, pane: u64) -> io::Result<Option<(WorkspaceId, TabId)>> {
    let mut owners = m.workspaces.iter().flat_map(|w| {
        w.tabs.iter().flat_map(move |t| {
            t.root
                .pane_ids()
                .into_iter()
                .filter(move |id| *id == pane)
                .map(move |_| (w.id, t.id))
        })
    });
...
fn require_live_session(record: &PaneRecord) -> io::Result<()> {
    if !record.live || record.exit.is_some() || record.ssh_spec.is_some() {
        return Err(refuse("session is not a live local carrier"));
    }
    Ok(())
}

fn stopped_session_record<'a>(
    m: &'a Machine,
    workspace: WorkspaceId,
    tab: TabId,
    pane: u64,
    identity: &crate::agent_sessions::SessionIdentity,
) -> io::Result<&'a PaneRecord> {
    let record = bound_session_record(m, workspace, tab, pane, identity)?;
    if record.exit.is_none() || record.live || record.agent.is_some() || record.ssh_spec.is_some() {
        return Err(refuse("stopped session identity or runtime changed"));
    }
    Ok(record)
}

fn bound_session_record<'a>(
    m: &'a Machine,
    workspace: WorkspaceId,
    tab: TabId,
    pane: u64,
    ...
    let session_id = identity.canonical_id()?;
    let agent = match identity.provider {
        crate::agent_sessions::Provider::Codex => CLIAgent::Codex,
        crate::agent_sessions::Provider::Claude => CLIAgent::Claude,
    };
    if unique_pane_owner(m, pane)? != Some((workspace, tab)) {
        return Err(refuse("stopped session slot changed"));
    }
    let record = m
        .panes
        .iter()
        .find(|p| p.id == pane)
        .ok_or_else(|| not_found("stopped session record is missing"))?;
    let binding = record
        .recovery_binding
        .as_ref()
        .ok_or_else(|| refuse("stopped session binding is missing"))?;
    if binding.agent != agent || binding.session_id != session_id || binding.cwd != identity.cwd {
        return Err(refuse("stopped session identity or runtime changed"));
    }
    Ok(record)
}
...
fn replace_pane(
    ...
    let (owner, _) = unique_pane_owner(m, old)?
...
mod tests {
    #[test]
    fn confirmed_exit_survives_reload_and_rejects_late_facts() {
        use super::*;
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("machine.json");
        let record: PaneRecord = serde_json::from_value(serde_json::json!({
            "id": 71, "exit": {"code": 0},
            "recovery_binding": {"agent": "Codex", "session_id": "kept", "cwd": "/work", "launch_argv": null}
        })).unwrap();
        // Model a target-owned file, including an explicit zero exit code.
        let mut record_json = serde_json::to_value(&record).unwrap();
        record_json["exit"] = serde_json::json!({"code": 0});
        std::fs::write(
            &path,
            serde_json::to_vec(&serde_json::json!({"panes": [record_json], "workspaces": []}))
                .unwrap(),
        )
        .unwrap();
        let store = MachineStore::open(path.clone());
        let before = store.pane(71).unwrap();
        assert_eq!(
            serde_json::to_value(&before).unwrap()["exit"],
            serde_json::json!({"code": 0})
        );
        store.note_pane_facts(71, |p| {
            p.exit = None;
        ...
        let after = store.pane(71).unwrap();
        assert!(!after.live);
        assert!(after.agent.is_none());
        assert_eq!(after.recovery_binding, before.recovery_binding);
        assert_eq!(
            serde_json::to_value(&after).unwrap()["exit"],
            serde_json::json!({"code": 0})
        );
        store.flush();
        let loaded = MachineStore::open(path);
        assert_eq!(loaded.pane(71).unwrap(), after);
    }

    #[test]
    fn confirmed_exit_is_not_inferred_from_legacy_or_seed() {
        use super::*;
        let legacy: PaneRecord =
            serde_json::from_value(serde_json::json!({"id": 9, "live": false})).unwrap();
        assert!(serde_json::to_value(legacy).unwrap()["exit"].is_null());
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("machine.json");
        std::fs::write(
            &path,
            br#"{"panes":[{"id":9,"live":true}],"workspaces":[]}"#,
        )
        .unwrap();
        let loaded = MachineStore::open(path);
        let unknown = loaded.pane(9).unwrap();
        assert!(!unknown.live, "restart invalidates only ephemeral liveness");
        assert!(
            unknown.exit.is_none(),
            "restart is not confirmed termination"
        );
        let seed: PaneSeed =
            serde_json::from_value(serde_json::json!({"pane": 10, "exit": {"code": 0}})).unwrap();
        for live in [false, true] {
            let record = seed.clone().into_record(live);
            assert!(serde_json::to_value(&record).unwrap()["exit"].is_null());
            assert_eq!(record.live, live);
        }
    }
    ...
    #[test]
    fn recovery_binding_survives_exit_and_target_store_reload() {
        let (store, _dir, _, tab) = store_with_tab();
        let id = tab.root.pane_ids()[0];
        let original = _dir.path().join("original").to_string_lossy().into_owned();
        store.note_pane_facts(id, |pane| {
            pane.observe_agent_identity(Some(binding_facts()), Some(&original))
        });
        let binding = store
            .pane(id)
            .unwrap()
            .recovery_binding
            .expect("observed identity is retained");
        store.note_pane_facts(id, |pane| {
            pane.observe_agent_identity(None, Some("/work/other"));
            pane.cwd = Some("/work/other".into());
        });
        let pane = store.pane(id).unwrap();
        assert!(pane.agent.is_none());
        assert_eq!(pane.recovery_binding.as_ref(), Some(&binding));
        assert_eq!(binding.cwd, original);
        store.flush();
        let reopened = MachineStore::open(store.path().to_path_buf());
        let persisted = reopened.pane(id).unwrap();
        assert!(!persisted.live);
        assert!(persisted.agent.is_none());
        assert_eq!(persisted.recovery_binding, Some(binding));
    }

    #[test]
    fn recovery_binding_does_not_inherit_unknown_identity_or_client_seed() {
        let mut seed = PaneSeed::bare(81);
        seed.agent = Some(binding_facts());
        seed.cwd = Some("/client/cache".into());
        let mut pane = seed.into_record(true);
        assert!(
            pane.recovery_binding.is_none(),
            "client seeds are not daemon observations"
        );
        let absolute = std::env::temp_dir().to_string_lossy().into_owned();
        pane.observe_agent_identity(Some(binding_facts()), Some(&absolute));
        assert!(pane.recovery_binding.is_some());
        let mut unknown = binding_facts();
        unknown.session_id = None;
        pane.observe_agent_identity(Some(unknown), Some("/new"));
        assert!(pane.recovery_binding.is_none());
        pane.observe_agent_identity(Some(binding_facts()), None);
        assert!(
            pane.recovery_binding.is_none(),
            "unknown cwd cannot use client cwd"
        );
        pane.observe_agent_identity(Some(binding_facts()), Some("relative"));
        assert!(pane.recovery_binding.is_none());
        let old: PaneRecord = serde_json::from_str(r#"{"id":81}"#).unwrap();
        assert!(old.recovery_binding.is_none());
    }
    ...
    #[test]
    fn checked_attachment_layout_lock_rejects_duplicate_leaves() {
        let (store, _dir, workspace, tab) = store_with_tab();
        assert!(
            store
                .with_pane_owner(workspace, tab.id, 1, || {
                    assert!(
                        store.state.try_lock().is_err(),
                        "layout unlocked before attach"
                    );
                    Ok(())
                })
                .is_ok()
        );
        // Invalid legacy snapshots can contain duplicated references even
        // though current mutation APIs reject creating them.
        store.locked().workspaces[0].tabs[0].root = PaneNode::Split {
        ...
        assert!(
            store
                .with_pane_owner(workspace, tab.id, 1, || -> io::Result<()> {
                    panic!("duplicate pane reached attach")
                })
                .is_err()
        );
        store.locked().workspaces[0].tabs[0].root = PaneNode::Leaf { pane: 1 };
        store.locked().workspaces.push(Workspace {
            tabs: vec![Tab::leaf(1)],
            ..Workspace::default()
        });
        assert!(
            store
                .with_pane_owner(workspace, tab.id, 1, || -> io::Result<()> {
                    panic!("multiply owned pane reached attach")
                })
                .is_err()
        );
    }
    ...
    #[test]
    fn stopped_resume_consumes_predecessor_and_preserves_binding() {
        let (store, _dir) = store();
        let ws = store.workspace_create(None, None, None).unwrap();
        let tab = store
            .tab_create(ws.id, None, seed(7, "/work"), None, None)
        ...
        store.note_pane_facts(7, |p| {
            ...
            p.provider_title = Some(ProviderTitle {
                agent: CLIAgent::Codex,
                session_id: identity.id.clone(),
                title: "official title".into(),
            });
            p.observe_terminal_exit(Some(0));
        });
        store.flush();
        let old = store.pane(7).unwrap();
        let mut expected = store.machine().workspaces;
        expected[0].tabs[0].root.replace_leaf(7, 42);
        let calls = std::cell::Cell::new(0);
        assert_eq!(
            store
                .resume_stopped_with(ws.id, tab.id, 7, &identity, |record| {
                    calls.set(calls.get() + 1);
                    assert_eq!(record, &old);
                    Ok(seed(42, &identity.cwd))
                })
                .unwrap(),
            42
        );
        assert_eq!(calls.get(), 1);
        assert_eq!(store.machine().workspaces, expected);
        let new = store.pane(42).unwrap();
        assert_eq!(new.recovery_binding, old.recovery_binding);
        assert_eq!(new.provider_title, old.provider_title);
        assert!(
            new.agent.is_none(),
            "intent is not observed runtime identity"
        );
        assert!(new.exit.is_none());
        store.note_pane_facts(42, |p| {
            p.observe_agent_identity(
                Some(AgentFacts {
                    session_id: None,
                    ..binding_facts()
        ...
        let provisional = store.pane(42).unwrap();
        assert_eq!(
            provisional.recovery_binding, old.recovery_binding,
            "provisional provider detection must not erase the explicitly resumed target identity"
        );
        assert_eq!(provisional.provider_title, old.provider_title);
        assert!(provisional.resume_pending);
        assert!(provisional.agent.as_ref().unwrap().session_id.is_none());
        let mut confirmed = provisional.clone();
        confirmed.observe_agent_identity(
            Some(AgentFacts {
                session_id: Some(identity.id.clone()),
                ..binding_facts()
            }),
        ...
        assert!(!confirmed.resume_pending);
        confirmed.observe_agent_identity(
            Some(AgentFacts {
                session_id: None,
                ..binding_facts()
        ...
        assert!(
            confirmed.recovery_binding.is_none(),
            "normal unknown identity rules resume after confirmation"
        );
        let mut foreign = provisional.clone();
        foreign.observe_agent_identity(
            Some(AgentFacts {
                agent: CLIAgent::Claude,
                session_id: None,
                ..binding_facts()
        ...
        assert!(!foreign.resume_pending);
        assert!(foreign.recovery_binding.is_none());
        let mut stopped = provisional;
        stopped.observe_terminal_exit(Some(1));
        assert!(!stopped.resume_pending);
        assert_eq!(stopped.recovery_binding, old.recovery_binding);
        let injected: PaneSeed =
            serde_json::from_value(serde_json::json!({"pane": 99, "resume_pending": true}))
                .unwrap();
        assert!(!injected.into_record(true).resume_pending);
        assert!(store.pane(7).is_none());
        assert!(
            store
                .resume_stopped_with(ws.id, tab.id, 7, &identity, |_| {
                    panic!("consumed predecessor must reject before another allocation")
                })
                .is_err()
        );
        store.flush();
        let reopened = MachineStore::open(store.path().to_path_buf());
        let pending = reopened.pane(42).unwrap();
        assert!(pending.resume_pending);
        assert!(
            !pending.live,
            "pending launch intent does not prove liveness after restart"
        );
        assert_eq!(pending.recovery_binding, old.recovery_binding);

        store.note_pane_facts(42, |p| p.observe_terminal_exit(Some(0)));
        ...
        let attempts: Vec<_> = (0..2)
            .map(|index| {
                ...
                std::thread::spawn(move || {
                    barrier.wait();
                    store.resume_stopped_with(ws.id, tab.id, 42, &identity, |_| {
                        allocated.fetch_add(1, Ordering::SeqCst);
                        Ok(seed(100 + index, &identity.cwd))
                    })
                })
            })
            .collect();
        ...
        assert_eq!(
            ...
            1,
            "concurrent requests share the same predecessor consumption guard"
        );
    }

    #[test]
    fn stopped_resume_rejects_before_allocation_and_rolls_back() {
        let (store, _dir) = store();
        let ws = store.workspace_create(None, None, None).unwrap();
        let tab = store
            .tab_create(ws.id, None, seed(7, "/work"), None, None)
        ...
        let reject = |target: &crate::agent_sessions::SessionIdentity, target_tab| {
            let before = store.machine();
            assert!(
                store
                    .resume_stopped_with(ws.id, target_tab, 7, target, |_| {
                        panic!("invalid target must not allocate")
                    })
                    .is_err()
            );
            assert_eq!(store.machine(), before);
        };
        reject(&identity, tab.id); // live shell, no retained binding
        store.note_pane_facts(7, |p| {
            p.observe_agent_identity(
                Some(AgentFacts {
                    session_id: Some(identity.id.clone()),
        ...
        reject(&identity, tab.id); // retained binding does not prove termination
        store.note_pane_facts(7, |p| p.observe_terminal_exit(Some(0)));
        store.flush();
        reject(&identity, TabId::new());
        for changed in [
            crate::agent_sessions::SessionIdentity {
            ...
            crate::agent_sessions::SessionIdentity {
                provider: crate::agent_sessions::Provider::Claude,
                ..identity.clone()
            },
        ] {
            reject(&changed, tab.id);
        }
        let before = store.machine();
        ...
        let backup = store.path().with_extension("before-resume");
        std::fs::rename(store.path(), &backup).unwrap();
        std::fs::create_dir(store.path()).unwrap();
        assert!(
            store
                .resume_stopped_with(ws.id, tab.id, 7, &identity, |_| Ok(seed(42, "/work")))
                .is_err()
        );
        assert_eq!(store.machine(), before);
        assert!(heard.lock().unwrap().is_empty());
    ...
    #[test]
    fn pane_replacement_rejects_ambiguous_ownership_atomically() {
        for duplicate in ["split", "tab", "workspace"] {
            let (store, _dir) = store();
            let ws = store.workspace_create(None, None, None).unwrap();
            store
                .tab_create(ws.id, None, seed(7, "/work"), None, None)
                .unwrap();
            let mut fixture = store.machine();
            match duplicate {
                "split" => {
                    fixture.workspaces[0].tabs[0].root = PaneNode::Split {
                        axis: Axis::Vertical,
                        ratio: 0.37,
                        a: Box::new(PaneNode::Leaf { pane: 7 }),
                        b: Box::new(PaneNode::Leaf { pane: 7 }),
                    };
                }
                "tab" => {
                    let mut tab = fixture.workspaces[0].tabs[0].clone();
                    tab.id = TabId::new();
                    fixture.workspaces[0].tabs.push(tab);
                }
                "workspace" => {
                    let mut other = fixture.workspaces[0].clone();
                    other.id = WorkspaceId::new();
                    other.tabs[0].id = TabId::new();
                    fixture.workspaces.push(other);
                }
                _ => unreachable!(),
            }
            // Exercise the actual load path: syntactically valid old snapshots
            // can contain duplicate references and are not repaired on load.
            store.persist(&fixture).unwrap();
            let loaded = MachineStore::open(store.path().to_path_buf());
            let before = loaded.machine();
            let disk = std::fs::read(loaded.path()).unwrap();
            let (_sub, heard) = recorded(&loaded);
            assert!(
                loaded
                    .pane_replace(ws.id, 7, seed(42, "/work"), None)
                    .is_err(),
                "{duplicate}: replacement must not choose the first owner"
            );
            assert_eq!(loaded.machine(), before, "{duplicate}: in-memory rollback");
            assert_eq!(
                std::fs::read(loaded.path()).unwrap(),
                disk,
                "{duplicate}: disk unchanged"
            );
            assert!(heard.lock().unwrap().is_empty(), "{duplicate}: no delta");
        }
    }

    #[test]
    fn pane_replacement_preserves_exact_slot_and_rejects_stale_targets() {
        let (store, _dir) = store();
        let ws = store
            .workspace_create(None, Some("machine".into()), None)
            .unwrap();
        let tab = store
            .tab_create(ws.id, None, seed(7, "/work"), None, None)
            .unwrap();
        store
            .tab_rename(ws.id, tab.id, Some("kept title".into()), None)
            .unwrap();
        store
            .pane_split(
                ws.id,
        ...
        store
            ...
            .unwrap();
        store
            .tab_create(ws.id, None, seed(10, "/other-tab"), None, None)
            .unwrap();
        let other = store.workspace_create(None, None, None).unwrap();
        store
            .tab_create(other.id, None, seed(11, "/other-machine"), None, None)
            .unwrap();
        let (_sub, heard) = recorded(&store);

        for (workspace, old, successor) in [
            (other.id, 7, 42),
            (ws.id, 999, 42),
            (ws.id, 7, 8),
            (ws.id, 7, 11),
            ...
            assert!(
                store
                    .pane_replace(workspace, old, seed(successor, "/work"), None)
                    .is_err()
            );
            assert_eq!(store.machine(), before);
            assert_eq!(std::fs::read(store.path()).unwrap(), disk);
        ...
        // An unwritable commit must not consume the old record or publish a
        // successor. Use an exact disposable path obstruction, not chmod
        // (which depends on the account running the test).
        let before = store.machine();
        let backup = store.path().with_extension("before-replacement");
        std::fs::rename(store.path(), &backup).unwrap();
        std::fs::create_dir(store.path()).unwrap();
        assert!(
            store
        ...
        assert_eq!(
            store.machine(),
            before,
            "failed persistence rolls back layout and records"
        );
        assert!(heard.lock().unwrap().is_empty());
        ...
        let mut expected = store.machine().workspaces;
        assert!(expected[0].tabs[0].root.replace_leaf(7, 42));
        store
            .pane_replace(ws.id, 7, seed(42, "/work"), None)
            .unwrap();
        assert_eq!(
            store.machine().workspaces,
            expected,
            "only the chosen leaf changes"
        );
        assert!(store.pane(7).is_none());
        assert!(store.pane(42).is_some());
        assert_eq!(heard.lock().unwrap().len(), 1);
        let before = store.machine();
        let disk = std::fs::read(store.path()).unwrap();
        assert!(
            store
                .pane_replace(ws.id, 7, seed(43, "/work"), None)
                .is_err()
        );
        assert_eq!(
            store.machine(),
            before,
            "late duplicate completion cannot replace another leaf"
        );
        assert_eq!(std::fs::read(store.path()).unwrap(), disk);
        assert_eq!(heard.lock().unwrap().len(), 1);
    }
