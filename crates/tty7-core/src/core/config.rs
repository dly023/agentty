...
/// Client-only navigation preferences. No connection or target-owned facts.
#[derive(Debug, Clone, Default, PartialEq, Eq, Deserialize, Serialize)]
#[serde(default)]
pub struct MachineRailPreferences {
    pub pinned: Vec<crate::core::session::RemoteTarget>,
    pub collapsed: Vec<String>,
}

impl MachineRailPreferences {
    pub fn set_pinned(&mut self, target: crate::core::session::RemoteTarget, pinned: bool) {
        if pinned {
            if !self.pinned.contains(&target) {
                self.pinned.push(target);
            }
        } else {
            self.pinned.retain(|t| t != &target);
        }
    }

    pub fn toggle_collapsed(&mut self, key: String) {
    ...
    pub fn expanded(&self, key: &str, searching: bool) -> bool {
        searching || !self.collapsed.iter().any(|k| k == key)
    }

    pub fn order_key(&self, key: &str) -> (u8, usize) {
        if key.is_empty() {
            (0, 0)
        } else if let Some(at) = self.pinned.iter().position(|t| t.to_string() == key) {
            (1, at)
        } else {
            (2, 0)
        }
...
pub struct Config {
    ...
    pub machine_rail: MachineRailPreferences,
    #[serde(default = "default_true")]
...
impl Default for Config {
    fn default() -> Self {
        Self {
            ...
            machine_rail: MachineRailPreferences::default(),
...
mod tests {
    #[test]
    fn machine_preferences_pin_is_idempotent_and_unpin_preserves_folding() {
        let target = crate::core::session::RemoteTarget::direct("dev", "example.test", 22);
        let mut prefs = super::MachineRailPreferences::default();
        prefs.set_pinned(target.clone(), true);
        prefs.set_pinned(target.clone(), true);
        assert_eq!(prefs.pinned, vec![target.clone()]);
        prefs.toggle_collapsed(target.to_string());
        prefs.set_pinned(target.clone(), false);
        assert!(prefs.pinned.is_empty());
        assert!(!prefs.expanded(&target.to_string(), false));
    }

    #[test]
    fn machine_preferences_roundtrip_and_legacy_defaults() {
        let legacy: super::Config = serde_json::from_str("{}").unwrap();
        assert_eq!(
            legacy.machine_rail,
            super::MachineRailPreferences::default()
        );
        let mut config = legacy;
        let target = crate::core::session::RemoteTarget::direct("dev", "example.test", 22);
        config.machine_rail.set_pinned(target.clone(), true);
        config.machine_rail.toggle_collapsed(target.to_string());
        let restored: super::Config =
            serde_json::from_str(&serde_json::to_string(&config).unwrap()).unwrap();
        assert_eq!(restored.machine_rail, config.machine_rail);
    }

    #[test]
    fn machine_preferences_search_reveals_without_unfolding() {
        let mut prefs = super::MachineRailPreferences::default();
        prefs.toggle_collapsed(String::new());
        assert!(!prefs.expanded("", false));
        assert!(prefs.expanded("", true));
        assert!(!prefs.expanded("", false));
        prefs.toggle_collapsed(String::new());
        assert!(prefs.expanded("", false));
    }
    ...
    #[test]
    fn machine_preferences_order_is_local_then_pin_order() {
        let first = crate::core::session::RemoteTarget::direct("dev", "z.test", 22);
        let second = crate::core::session::RemoteTarget::direct("dev", "a.test", 22);
        let mut prefs = super::MachineRailPreferences::default();
        prefs.set_pinned(first.clone(), true);
        prefs.set_pinned(second.clone(), true);
        let mut keys = vec![
        ...
        keys.sort_by_key(|key| prefs.order_key(key));
        assert_eq!(
            keys,
            vec![
                String::new(),
