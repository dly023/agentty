...
pub enum SessionPane {
    /// Target-tree projection only. Client files cannot assert termination.
    #[serde(skip)]
    Stopped {
        pane_id: u64,
        cwd: Option<PathBuf>,
...
impl SessionPane {
    pub fn is_fully_stopped(&self) -> bool {
        match self {
            Self::Stopped { .. } => true,
            Self::Leaf { .. } => false,
            Self::Split { a, b, .. } => a.is_fully_stopped() && b.is_fully_stopped(),
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn confirmed_stopped_projection_is_not_client_persistence() {
        let cold = SessionPane::Stopped {
            pane_id: 71,
            cwd: None,
            shell: None,
        };
        assert!(cold.is_fully_stopped());
        assert!(serde_json::to_value(&cold).is_err());
        assert!(
            serde_json::from_value::<SessionPane>(serde_json::json!({
                "Stopped": {"pane_id": 71, "cwd": null, "shell": null}
            }))
            .is_err()
        );
        let legacy: SessionPane = serde_json::from_value(serde_json::json!({
            "Leaf": {"pane_id": 71, "exit": {"code": 0}}
        }))
        .unwrap();
        assert!(matches!(
            legacy,
        ...
        assert!(!legacy.is_fully_stopped());
        let both = SessionPane::Split {
            axis: SessionAxis::Horizontal,
            ratio: 0.3,
            a: Box::new(cold.clone()),
            b: Box::new(cold.clone()),
        };
        assert!(both.is_fully_stopped());
        let mixed = SessionPane::Split {
            axis: SessionAxis::Vertical,
            ratio: 0.7,
            a: Box::new(cold),
            b: Box::new(legacy),
        };
        assert!(!mixed.is_fully_stopped());
    }
