...
/// Canonical, validated invocation. Only the provider constructs shell tokens.
...
impl CLIAgent {
    ...
    pub fn resume_invocation(self, session_id: &str) -> Option<ResumeInvocation> {
        self.session_command_flags(session_id, None)?;
        let (program, verb) = match self {
            Self::Codex => ("codex", "resume"),
            Self::Claude => ("claude", "--resume"),
            _ => return None,
        };
        Some(ResumeInvocation {
            program,
            args: vec![verb.into(), session_id.into()],
        })
    }
    ...
    pub fn resume_command(
        ...
        match self {
            CLIAgent::Claude => {
                let invocation = self.resume_invocation(session_id)?;
                Some(format!(
                    "{}{flags} {}",
                    invocation.program(),
            ...
            CLIAgent::Codex => Some(format!(
                "{}{flags}",
                self.resume_invocation(session_id)?.command_line()
            )),
...
mod tests {
    #[test]
    fn history_resume_invocation_is_typed_and_rejects_injection() {
        let id = "01900000-0000-7000-8000-000000000001";
        let invocation = super::CLIAgent::Codex.resume_invocation(id).unwrap();
        assert_eq!(invocation.program(), "codex");
        assert_eq!(invocation.args(), &["resume".to_owned(), id.to_owned()]);
        assert_eq!(
            invocation.command_line(),
            super::CLIAgent::Codex.resume_command(id, None).unwrap()
        );
        assert!(
            super::CLIAgent::Claude
                .resume_invocation("id; touch /tmp/not-authorized")
                .is_none()
        );
    }
    use super::*;
