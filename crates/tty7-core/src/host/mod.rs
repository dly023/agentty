...
pub trait Host: Send + Sync + 'static {
    ...
    fn agent_sessions(&self) -> io::Result<crate::agent_sessions::HistorySnapshot> {
        Err(io::Error::new(
            ...
            "agent history discovery unavailable on this host",
        ))
    }

    /// Blocking target validation and subscription setup. Drop the returned
    /// lifetime off the UI thread as well; it retires target-owned resources.
    fn watch_agent_session_titles(
        &self,
        _snapshot: &crate::agent_sessions::HistorySnapshot,
    ) -> io::Result<crate::agent_sessions::live_titles::HistoryTitleSubscription> {
        Err(io::Error::new(
            io::ErrorKind::Unsupported,
            "agent history title subscription unavailable on this host",
        ))
    }

    fn validate_agent_session(
        &self,
        _session: &crate::agent_sessions::HistorySession,
    ) -> io::Result<()> {
        Err(io::Error::new(
            io::ErrorKind::Unsupported,
            "agent history validation unavailable on this host",
        ))
    }

    fn resolve_agent_session(
        &self,
        _identity: &crate::agent_sessions::SessionIdentity,
    ) -> io::Result<crate::agent_sessions::HistorySession> {
        Err(io::Error::new(
            io::ErrorKind::Unsupported,
            "retained session resolution unavailable on this host",
        ))
    }
