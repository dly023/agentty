...
impl RemoteHost {
        ...
        W: Write + Send + 'static,
    {
        ...
        let titles = Arc::new(TitleRegistry::default());
        let sink_titles = Arc::clone(&titles);
        let id = HostId::from_connection_key(connection_key);
        let sink: EventSink = Box::new(move |event| match event {
            ControlEvent::HistoryTitles { id, event } => sink_titles.dispatch(id, event),
        ...
        let down_titles = Arc::clone(&titles);
        client.on_link_down(move || down_titles.close_all());
...
#[derive(Default)]
struct TitleRegistry(Mutex<TitleRegistryState>);

#[derive(Default)]
struct TitleRegistryState {
    closed: bool,
    entries: HashMap<uuid::Uuid, TitleMailbox>,
}

struct TitleMailbox {
    tx: smol::channel::Sender<HistoryTitleEvent>,
    rx: smol::channel::Receiver<HistoryTitleEvent>,
}

impl TitleRegistry {
    fn register(&self, id: uuid::Uuid) -> io::Result<smol::channel::Receiver<HistoryTitleEvent>> {
        ...
        if state.entries.len() >= 16 || state.entries.contains_key(&id) {
            return Err(io::Error::new(
                io::ErrorKind::WouldBlock,
                "title subscription capacity reached",
            ));
        }
        let (tx, rx) = smol::channel::bounded(1);
        state
            .entries
            .insert(id, TitleMailbox { tx, rx: rx.clone() });
        Ok(rx)
    }

    fn dispatch(&self, id: uuid::Uuid, event: HistoryTitleEvent) {
        let state = self.0.lock().unwrap_or_else(|e| e.into_inner());
        let Some(mailbox) = state.entries.get(&id) else {
            return;
        };
        if let Err(smol::channel::TrySendError::Full(event)) = mailbox.tx.try_send(event) {
            let _ = mailbox.rx.try_recv();
            let _ = mailbox.tx.try_send(event);
        }
    }
    ...
    fn close_all(&self) {
        let mut state = self.0.lock().unwrap_or_else(|e| e.into_inner());
        state.closed = true;
        for mailbox in state.entries.values() {
            mailbox.tx.close();
        }
        state.entries.clear();
    }
...
struct RemoteTitleGuard {
    id: uuid::Uuid,
    registry: Arc<TitleRegistry>,
    client: Arc<ControlClient>,
}

impl Drop for RemoteTitleGuard {
    fn drop(&mut self) {
        self.registry.remove(self.id);
        if self.client.is_connected() {
            let _ = self
                .client
                .call(ControlRequest::TitleWatchClose { id: self.id });
        }
    }
}
...
impl Host for RemoteHost {
    fn watch_agent_session_titles(
        ...
        crate::host::guard_off_ui();
        let id = uuid::Uuid::new_v4();
        let events = self.titles.register(id)?;
        let guard = RemoteTitleGuard {
            id,
            registry: Arc::clone(&self.titles),
            client: Arc::clone(&self.client),
        };
        match self.call(ControlRequest::TitleWatchOpen {
            id,
            snapshot: snapshot.clone(),
        })? {
            ReplyOk::Unit => Ok(HistoryTitleSubscription::new(events, Box::new(guard))),
            other => Err(wrong_shape("title subscription acknowledgement", &other)),
        }
    }
    ...
    fn agent_sessions(&self) -> io::Result<crate::agent_sessions::HistorySnapshot> {
        match self.call(ControlRequest::AgentSessions)? {
            ReplyOk::AgentSessions(snapshot) => Ok(snapshot),
            other => Err(wrong_shape("agent history snapshot", &other)),
        }
    }

    fn validate_agent_session(
        &self,
        session: &crate::agent_sessions::HistorySession,
    ) -> io::Result<()> {
        match self.call(ControlRequest::ValidateAgentSession {
            session: session.clone(),
        })? {
            ReplyOk::Unit => Ok(()),
            other => Err(wrong_shape("agent history validation", &other)),
        }
    }
    ...
    ) -> io::Result<crate::agent_sessions::HistorySession> {
        let id = identity.canonical_id()?;
        match self.call(ControlRequest::ResolveAgentSession {
            identity: identity.clone(),
        })? {
            ReplyOk::ResolvedAgentSession(session)
                if session.provider == identity.provider
                    && session.id == id
                    && session.cwd == identity.cwd
                    && !session.source_path.is_empty()
                    && !session.source_path.contains('\0') =>
            {
                Ok(session)
            }
            _ => Err(io::Error::new(
                io::ErrorKind::InvalidData,
                "target returned a mismatched retained session",
            )),
        }
    }
...
mod tests {
    ...
    #[test]
    fn history_resume_validation_is_target_owned() {
        let session = crate::agent_sessions::HistorySession {
            provider: crate::agent_sessions::Provider::Codex,
            id: "01900000-0000-7000-8000-000000000001".into(),
            title: "remote".into(),
            cwd: "/target-only".into(),
            source_path: "/target-only/session.jsonl".into(),
            modified_ms: 1,
        };
        for denied in [false, true] {
            let expected = ControlRequest::ValidateAgentSession {
                session: session.clone(),
            };
            let request = expected.clone();
            let (host, seen) = host_with_peer('/', move |req| {
                assert_eq!(req, &request);
                Some((
                    if denied {
                        ControlReply::Err(WireError::new(
                            WireErrorKind::PermissionDenied,
                            "target denied validation",
                        ))
                    } else {
            ...
            let result = host.validate_agent_session(&session);
            if denied {
    ...
    #[test]
    fn retained_session_resolution_is_target_owned() {
        use crate::agent_sessions::{HistorySession, Provider, SessionIdentity};
        ...
        for case in 0..6 {
            let expected = ControlRequest::ResolveAgentSession {
                identity: identity.clone(),
            };
            let request = expected.clone();
            ...
            let (host, seen) = host_with_peer('/', move |req| {
                ...
                let mut session = HistorySession {
                    ...
                    cwd: query.cwd.clone(),
                    title: "target".into(),
                    source_path: "/remote-only/session.jsonl".into(),
                    modified_ms: 1,
                };
                let reply = match case {
                    1 => ControlReply::Err(WireError::new(
                        WireErrorKind::PermissionDenied,
                        "target refused",
                    )),
                    2 => {
                        session.provider = Provider::Claude;
                        ControlReply::Ok(ReplyOk::ResolvedAgentSession(session))
                    }
                    3 => {
                        session.id = "01900000-0000-7000-8000-000000000002".into();
                        ControlReply::Ok(ReplyOk::ResolvedAgentSession(session))
                    }
                    4 => {
                        session.cwd = "/foreign".into();
                        ControlReply::Ok(ReplyOk::ResolvedAgentSession(session))
                    }
                    5 => ControlReply::Ok(ReplyOk::Unit),
                    _ => ControlReply::Ok(ReplyOk::ResolvedAgentSession(session)),
                };
                Some((reply, vec![]))
            });
            let result = host.resolve_agent_session(&identity);
            if case == 0 {
                assert_eq!(result.unwrap().source_path, "/remote-only/session.jsonl");
            } else if case == 1 {
                assert_eq!(result.unwrap_err().kind(), io::ErrorKind::PermissionDenied);
            } else {
    ...
    #[test]
    fn agent_sessions_are_returned_only_from_the_peer() {
        use crate::agent_sessions::{HistorySession, HistorySnapshot, Provider};
        let expected = HistorySnapshot {
            sessions: vec![HistorySession {
                ...
                source_path: "/srv/provider/rollout.jsonl".into(),
                modified_ms: 17,
            }],
            missing: vec![Provider::Claude],
            limited: false,
        };
        let reply = expected.clone();
        let (host, seen) = host_with_peer('/', move |req| {
            assert_eq!(req, &ControlRequest::AgentSessions);
            Some((
                ControlReply::Ok(ReplyOk::AgentSessions(reply.clone())),
                vec![],
            ))
        });
        assert_eq!(host.agent_sessions().unwrap(), expected);
        assert_eq!(seen.recv().unwrap(), ControlRequest::AgentSessions);
    }

    #[test]
    fn agent_sessions_remote_failure_has_no_local_fallback() {
        let (host, seen) = host_with_peer('/', |req| {
            assert_eq!(req, &ControlRequest::AgentSessions);
            Some((
                ControlReply::Err(WireError::new(
                    WireErrorKind::PermissionDenied,
                    "target denied history",
                )),
                vec![],
            ))
        });
        let error = host.agent_sessions().unwrap_err();
        assert_eq!(error.kind(), io::ErrorKind::PermissionDenied);
        assert_eq!(seen.recv().unwrap(), ControlRequest::AgentSessions);
    }

    #[test]
    fn history_title_unsupported_host_has_no_local_fallback() {
        let (host, seen) = host_with_peer('/', |req| match req {
            ControlRequest::TitleWatchOpen { .. } => Some((
                ControlReply::Err(WireError::new(
                    WireErrorKind::Unsupported,
                    "target does not provide titles",
                )),
                vec![],
            )),
        ...
        let snapshot = crate::agent_sessions::HistorySnapshot {
            sessions: Vec::new(),
            missing: Vec::new(),
            limited: false,
        };
        let error = host.watch_agent_session_titles(&snapshot).err().unwrap();
    ...
    #[test]
    fn history_title_remote_registry_coalesces_and_rejects_retired_ids() {
        use crate::agent_sessions::live_titles::{HistoryTitle, HistoryTitleEvent};
        let registry = TitleRegistry::default();
        let id = uuid::Uuid::new_v4();
        ...
        registry.dispatch(id, names("first"));
        registry.dispatch(id, names("latest"));
        registry.dispatch(uuid::Uuid::new_v4(), names("foreign"));
        assert_eq!(rx.capacity(), Some(1));
        assert_eq!(rx.try_recv().unwrap(), names("latest"));
        registry.remove(id);
        registry.dispatch(id, names("retired"));
        assert!(rx.is_closed());
        assert!(rx.try_recv().is_err());
        let next = registry.register(uuid::Uuid::new_v4()).unwrap();
        registry.close_all();
        assert!(next.is_closed());
        assert!(registry.register(uuid::Uuid::new_v4()).is_err());
    }

    #[test]
    fn history_title_remote_events_arrive_before_ack_and_close_once() {
        use crate::agent_sessions::live_titles::HistoryTitleEvent;
        let listener = TcpListener::bind("127.0.0.1:0").unwrap();
        let addr = listener.local_addr().unwrap();
        let peer = thread::spawn(move || {
            ...
            ControlServerMsg::HelloOk(hello_ok('/'))
                .encode(&mut sock)
                .unwrap();
            let (req_id, id) = match ControlClientMsg::read(&mut sock).unwrap() {
                ControlClientMsg::Request {
                    req_id,
                    req: ControlRequest::TitleWatchOpen { id, snapshot },
                } => {
            ...
            ControlServerMsg::Event(ControlEvent::HistoryTitles {
                id,
                event: HistoryTitleEvent::Unavailable,
            })
            .encode(&mut sock)
            .unwrap();
            ControlServerMsg::Response {
            ...
            .encode(&mut sock)
            .unwrap();
            let req_id = match ControlClientMsg::read(&mut sock).unwrap() {
                ControlClientMsg::Request {
                    req_id,
                    req: ControlRequest::TitleWatchClose { id: closed },
                } => {
                    assert_eq!(closed, id);
                    req_id
                }
        ...
        let host = RemoteHost::over_tcp(
            ...
            "test:titles",
            &ControlHello::host_rpc("test", "test"),
        )
        .unwrap();
        let sub = host
            .watch_agent_session_titles(&crate::agent_sessions::HistorySnapshot {
                sessions: vec![],
                missing: vec![],
                limited: false,
            })
        ...
        assert_eq!(rx.try_recv().unwrap(), HistoryTitleEvent::Unavailable);
        drop(sub);
        assert!(rx.is_closed());
        peer.join().unwrap();
    }
