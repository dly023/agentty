...
pub trait PaneDirectory: Send + Sync {
    fn sleep_tab(
        self: Arc<Self>,
        _request: &crate::daemon::control::SleepTabRequest,
        _cancelled: &dyn Fn() -> bool,
    ) -> io::Result<crate::daemon::control::SleepTabReply> {
        Err(io::Error::new(
            io::ErrorKind::Unsupported,
            "target tab sleep is unavailable",
        ))
    }
    fn sleep_session(
        self: Arc<Self>,
        _request: &crate::daemon::control::SleepSessionRequest,
        _cancelled: &dyn Fn() -> bool,
    ) -> io::Result<()> {
        Err(io::Error::new(
            io::ErrorKind::Unsupported,
            "target session sleep is unavailable",
        ))
    }
    ...
    fn resume_stopped(
        self: Arc<Self>,
        _request: &crate::daemon::control::StoppedResumeRequest,
        _cancelled: &dyn Fn() -> bool,
    ) -> io::Result<u64> {
        Err(io::Error::new(
            io::ErrorKind::Unsupported,
            "target pane recovery is unavailable",
        ))
    }
}
...
pub struct Services {
    pub provider_titles: Option<Arc<crate::agent_sessions::live_titles::TitleWatch>>,
...
impl Services {
    pub(crate) fn with_workspace_proof<T>(
        &self,
        proof: &crate::daemon::control::AttachmentProof,
        operation: impl FnOnce() -> io::Result<T>,
    ) -> io::Result<T> {
        let _handover = self.attachments.handover();
        let valid =
            self.attachments.locked().iter().any(|live| {
                live.workspace == proof.workspace.to_string() && live.nonce == proof.nonce
            });
        if !valid {
            return Err(io::Error::new(
                io::ErrorKind::PermissionDenied,
                "attachment proof is no longer valid",
            ));
        }
        operation()
    }

    pub(crate) fn with_pane_attachment<T>(
        &self,
        proof: &crate::daemon::control::AttachmentProof,
        tab: crate::core::machine::TabId,
        pane: u64,
        attach: impl FnOnce() -> io::Result<T>,
    ) -> io::Result<T> {
        self.with_workspace_proof(proof, || {
            let machine = self
                .machine
                .as_ref()
                .ok_or_else(|| io::Error::other("pane ownership authority is unavailable"))?;
            machine.with_pane_owner(proof.workspace, tab, pane, attach)
        })
    }
    ...
    W: Write + Send + 'static,
{
    ...
    let conn = Arc::new(Conn {
        ...
        titles: title_watches::TitleWatches::default(),
    ...
    if let Some(workspace) = hello.workspace.as_deref()
        && let Err(e) = attach_workspace(&conn, workspace, true, AttachIntent::TakeOver)
    {
    ...
    conn.pool.close();
    // Interrupt blocked writers before joining target readers during teardown.
    let _ = shutdown.shutdown_link();
    conn.titles.shutdown();
    conn.watches
        .lock()
        .unwrap_or_else(|e| e.into_inner())
        .clear();
...
struct WorkspaceAttachOutcome {
    took_over_from: Option<String>,
    proof: Option<crate::daemon::control::AttachmentProof>,
    newly_acquired: bool,
}
...
fn attach_workspace(
    ...
    dedicated: bool,
    intent: AttachIntent,
) -> io::Result<WorkspaceAttachOutcome> {
    ...
    let canonical = tree_id.map(|id| id.to_string());
    let workspace = canonical.as_deref().unwrap_or(workspace);
    let (displaced, evicted, proof, newly_acquired) = {
        let _handover = conn.attachments.handover();
        if intent == AttachIntent::IfFree {
            let id = tree_id.ok_or_else(|| {
                io::Error::new(io::ErrorKind::InvalidInput, "invalid workspace ID")
            })?;
            let tree = conn.machine()?.workspace(id)?;
            let foreign_connection = conn
                .attachments
                .locked()
                .iter()
                .any(|live| live.workspace == workspace && live.conn != conn.id);
            if foreign_connection
                || tree
                    .attachment
                    .as_ref()
                    .is_some_and(|a| a.token != conn.holder.token)
            {
                return Err(io::Error::new(
                    io::ErrorKind::AlreadyExists,
                    "workspace is held by another connection",
                ));
            }
        }
        let newly_acquired = !conn
            .attachments
            .locked()
            .iter()
            .any(|live| live.workspace == workspace && live.conn == conn.id);
        ...
        let proof = tree_id.map(|id| crate::daemon::control::AttachmentProof {
            workspace: id,
            nonce: conn
                .attachments
                .locked()
                .iter()
                .find(|live| live.workspace == workspace && live.conn == conn.id)
                .expect("canonical claim installs current connection")
                .nonce,
        });
        (displaced, evicted, proof, newly_acquired)
    };
...
fn detach_workspace(conn: &Arc<Conn>, workspace: &str) -> io::Result<bool> {
    ...
    let workspace = canonical.as_deref().unwrap_or(workspace);
    let _handover = conn.attachments.handover();
    Ok(detach_workspace_locked(conn, workspace, tree_id))
}

/// Caller holds the handover lock across validation and both ownership writes.
fn detach_workspace_locked(
    conn: &Arc<Conn>,
    workspace: &str,
    tree_id: Option<crate::core::session::WorkspaceId>,
) -> bool {
    let released = conn.attachments.release(workspace, conn.id);
    if released && let (Some(machine), Some(id)) = (&conn.machine, tree_id) {
...
fn submit(conn: &Arc<Conn>, req_id: u64, req: ControlRequest, blob: Vec<u8>) {
    ...
    // Reserve/signal in incoming order, before concurrent validation jobs.
    if let ControlRequest::TitleWatchOpen { id, .. } = &req
        && let Err(error) = conn.titles.reserve(req_id, *id)
    {
        conn.finish(
            req_id,
            ControlReply::Err(WireError::from_io(&error)),
            Vec::new(),
            false,
        );
        return;
    }
    let title_close = if let ControlRequest::TitleWatchClose { id } = &req {
        conn.titles.signal_id(*id)
    } else {
        None
    };
    ...
    let queued = conn
        .pool
        .submit(move || run_job(&job_conn, req_id, req, blob, title_close));
    if !queued {
        if title_close.is_some() {
            let _ = conn.holder.shutdown.shutdown_link();
        }
        conn.finish(
            req_id,
...
fn run_job(
    ...
    req: ControlRequest,
    blob: Vec<u8>,
    title_close: Option<u64>,
) {
    // Close was already signalled by the reader. Cancelling its reply cannot
    // undo disposal, and its captured opening ID cannot target a replacement.
    if let Some(opening) = title_close {
        conn.titles.retire_request(opening);
    }
    if conn.is_cancelled(req_id) {
        conn.forget(req_id);
        conn.titles.retire_request(req_id);
        return;
    }
...
fn run_request(
    ...
    Ok(match req {
        ...
        ControlRequest::AgentSessions => (ReplyOk::AgentSessions(h.agent_sessions()?), Vec::new()),
        ControlRequest::TitleWatchOpen { id, snapshot } => {
            let source = h.watch_agent_session_titles(&snapshot)?;
            let (rx, alive) = conn.titles.install(req_id, id, source)?;
            let sink = Arc::clone(&conn.sink);
            std::thread::Builder::new()
                .name("tty7-control-titles".into())
                .spawn(move || {
                    while let Ok(event) = rx.recv_blocking() {
                        ...
                        if sink
                            .send(&ControlServerMsg::Event(ControlEvent::HistoryTitles {
                                id,
                                event,
                            }))
                            .is_err()
        ...
        ControlRequest::TitleWatchClose { .. } => (ReplyOk::Unit, Vec::new()),
        ControlRequest::ResolveAgentSession { identity } => (
            ReplyOk::ResolvedAgentSession(h.resolve_agent_session(&identity)?),
            Vec::new(),
        ),
        ControlRequest::SleepTab { request } => {
            let panes = conn.panes.clone().ok_or_else(|| {
                io::Error::new(
                    io::ErrorKind::Unsupported,
                    "target tab sleep is unavailable",
                )
            ...
            (
                ReplyOk::TabSleep(panes.sleep_tab(&request, &|| conn.is_cancelled(req_id))?),
                Vec::new(),
            )
        }
        ControlRequest::SleepSession { request } => {
            let panes = conn.panes.clone().ok_or_else(|| {
                io::Error::new(
                    io::ErrorKind::Unsupported,
                    "target session sleep is unavailable",
            ...
            panes.sleep_session(&request, &|| conn.is_cancelled(req_id))?;
            (ReplyOk::Unit, Vec::new())
        }
        ControlRequest::ResumeStoppedSession { request } => {
            ...
            let pane = panes.resume_stopped(&request, &|| conn.is_cancelled(req_id))?;
            (
                ReplyOk::StoppedSessionResumed {
                    workspace: request.proof.workspace,
                    tab: request.tab,
                    predecessor: request.pane,
                    pane,
                },
                Vec::new(),
            )
        }
        ControlRequest::ValidateAgentSession { session } => {
            h.validate_agent_session(&session)?;
            (ReplyOk::Unit, Vec::new())
        }
        ...
        ControlRequest::WorkspaceAttach { id } => (
            ReplyOk::Attached {
                took_over_from: attach_workspace(conn, &id, false, AttachIntent::TakeOver)?
                    .took_over_from,
            },
            Vec::new(),
        ),
        ControlRequest::WorkspaceAttachIfFree { id } => {
            let acquired = attach_workspace(conn, &id, false, AttachIntent::IfFree)?;
            (
                ReplyOk::WorkspaceAcquired {
                    proof: acquired
                        .proof
                        .expect("IfFree requires a valid workspace ID"),
                    newly_acquired: acquired.newly_acquired,
                },
                Vec::new(),
            )
        }
        ...
        ControlRequest::WorkspaceAttachmentProof { workspace } => {
            let _handover = conn.attachments.handover();
            conn.machine()?.workspace(workspace)?;
            let nonce = conn
                .attachments
                .locked()
                .iter()
                .find(|live| live.workspace == workspace.to_string() && live.conn == conn.id)
                .map(|live| live.nonce)
                .ok_or_else(|| {
                    io::Error::new(
                        io::ErrorKind::PermissionDenied,
                        "workspace is not held by this connection",
                    )
                })?;
            (
                ReplyOk::AttachmentProof(crate::daemon::control::AttachmentProof {
                    workspace,
                    nonce,
                }),
                Vec::new(),
        ...
        ControlRequest::WorkspaceReleaseProof { proof } => {
            let _handover = conn.attachments.handover();
            let key = proof.workspace.to_string();
            let owns = conn.attachments.locked().iter().any(|live| {
                live.workspace == key && live.conn == conn.id && live.nonce == proof.nonce
            });
            if !owns {
                return Err(io::Error::new(
                    io::ErrorKind::PermissionDenied,
            ...
            detach_workspace_locked(conn, &key, Some(proof.workspace));
            (ReplyOk::Unit, Vec::new())
        }
...
struct Conn {
    ...
    titles: title_watches::TitleWatches,
...
impl Conn {
    ...
    fn cancel(self: &Arc<Self>, req_id: u64) {
        {
        ...
        if self.titles.signal_request(req_id) {
            let conn = Arc::clone(self);
            if !self.pool.submit(move || conn.titles.retire_request(req_id)) {
                // No blocking cleanup in the live reader, and no abandoned
                // reservations when the bounded worker queue refuses cleanup.
                let _ = self.holder.shutdown.shutdown_link();
            }
        }
    }
    ...
    fn finish(&self, req_id: u64, reply: ControlReply, blob: Vec<u8>, wants_blob: bool) {
        ...
        if cancelled {
            ...
            self.titles.retire_request(req_id);
            return;
        }

        let successful = matches!(reply, ControlReply::Ok(_));
...
mod aggregate_tests {
    ...
    pub(super) fn client_with(services: Services) -> ControlClient {
        client_with_hello(services, ControlHello::host_rpc("tok", "test-host"))
    }

    pub(super) fn client_with_hello(services: Services, hello: ControlHello) -> ControlClient {
        ...
        ControlClient::over_tcp(sock, &hello, Box::new(|_| {})).unwrap()
    }
...
mod tests {
    ...
    struct SlowGit {
        ...
        titles: Option<TitleFactory>,
    }

    type TitleFactory = Arc<
        dyn Fn(
                &crate::agent_sessions::HistorySnapshot,
            )
                -> io::Result<crate::agent_sessions::live_titles::HistoryTitleSubscription>
            + Send
            + Sync,
    >;

    impl Host for SlowGit {
        fn watch_agent_session_titles(
            &self,
            snapshot: &crate::agent_sessions::HistorySnapshot,
        ) -> io::Result<crate::agent_sessions::live_titles::HistoryTitleSubscription> {
            self.titles
                .as_ref()
                .ok_or_else(|| io::Error::new(io::ErrorKind::Unsupported, "no test title source"))?(
                snapshot,
            )
        }
        fn id(&self) -> HostId {
    ...
    fn title_test_host(factory: TitleFactory) -> SharedHost {
        Arc::new(SlowGit {
            inner: LocalHost::new(),
            delay: Duration::ZERO,
    ...
    struct TitleDropped(std::sync::mpsc::Sender<()>);
    impl Drop for TitleDropped {
        fn drop(&mut self) {
            let _ = self.0.send(());
        }
    }

    fn empty_history() -> crate::agent_sessions::HistorySnapshot {
        crate::agent_sessions::HistorySnapshot {
            sessions: vec![],
            missing: vec![],
            limited: false,
    ...
    fn title_test_conn() -> Arc<Conn> {
        let sink = Arc::new(Sink::new(Vec::<u8>::new()));
    ...
    #[test]
    fn history_title_cancelled_close_still_retires_source() {
        use crate::agent_sessions::live_titles::HistoryTitleSubscription;
        let conn = title_test_conn();
        let id = uuid::Uuid::new_v4();
        let (tx, rx) = smol::channel::bounded(1);
        let (dropped, retired) = std::sync::mpsc::channel();
        conn.titles.reserve(1, id).unwrap();
        conn.titles
            .install(
                1,
                id,
                HistoryTitleSubscription::new(rx, Box::new(TitleDropped(dropped))),
            )
            .unwrap();
        let retiring = conn.titles.signal_id(id); // Ordered Close receipt precedes its queued job.
        conn.inflight.lock().unwrap().insert(2, true);
        run_job(
            &conn,
            2,
            ControlRequest::TitleWatchClose { id },
            Vec::new(),
            retiring,
        );
        assert!(tx.is_closed());
        retired.recv_timeout(Duration::from_secs(1)).unwrap();
        conn.titles.reserve(3, id).unwrap();
    }

    #[test]
    fn history_title_queue_refusal_and_failed_reply_release_reservation() {
        use crate::agent_sessions::live_titles::HistoryTitleSubscription;
        let conn = title_test_conn();
        let id = uuid::Uuid::new_v4();
        conn.pool.close();
        submit(
            &conn,
            1,
            ControlRequest::TitleWatchOpen {
                id,
                snapshot: empty_history(),
            },
            Vec::new(),
        );
        conn.titles.reserve(2, id).unwrap();
        let (tx, rx) = smol::channel::bounded(1);
        let (dropped, retired) = std::sync::mpsc::channel();
        conn.titles
            .install(
            ...
            .unwrap();
        conn.sink.retire();
        conn.finish(2, ControlReply::Ok(ReplyOk::Unit), Vec::new(), false);
        retired.recv_timeout(Duration::from_secs(1)).unwrap();
        assert!(tx.is_closed());
        conn.titles.reserve(3, id).unwrap();
    }
    ...
    #[test]
    fn history_title_delayed_close_does_not_retire_reused_token() {
        use crate::agent_sessions::live_titles::HistoryTitleSubscription;
        let conn = title_test_conn();
        let id = uuid::Uuid::new_v4();
        conn.titles.reserve(1, id).unwrap();
        let retiring = conn.titles.signal_id(id);
        conn.titles.retire_request(1);
        conn.titles.reserve(3, id).unwrap();
        let (tx, rx) = smol::channel::bounded(1);
        conn.titles
            .install(3, id, HistoryTitleSubscription::new(rx, Box::new(())))
            .unwrap();
        run_job(
            &conn,
            2,
        ...
        assert!(!tx.is_closed(), "old Close retired a later reservation");
        conn.titles.retire_request(3);
    }

    #[test]
    fn history_title_server_delivers_and_retires_target_source() {
        use crate::agent_sessions::live_titles::{HistoryTitleEvent, HistoryTitleSubscription};
        for disconnect in [false, true] {
            let (tx, rx) = smol::channel::bounded(1);
            let (dropped, retired) = std::sync::mpsc::channel();
            let p = pair_with(title_test_host(Arc::new(move |snapshot| {
                assert_eq!(*snapshot, empty_history());
                Ok(HistoryTitleSubscription::new(
                    rx.clone(),
                    Box::new(TitleDropped(dropped.clone())),
                ))
            })));
            let sub = p.host.watch_agent_session_titles(&empty_history()).unwrap();
            tx.try_send(HistoryTitleEvent::Unavailable).unwrap();
            let received = smol::block_on(smol::future::or(
                async { sub.events().recv().await.ok() },
                async {
                    smol::Timer::after(Duration::from_secs(5)).await;
                    None
                },
            ));
            assert_eq!(received, Some(HistoryTitleEvent::Unavailable));
            let observer = sub.events().clone();
            if disconnect {
                p.host.client().close();
            }
            drop(sub);
            retired.recv_timeout(Duration::from_secs(5)).unwrap();
    ...
    #[test]
    fn history_title_server_rejects_late_source_after_close_cancel_or_disconnect() {
        use crate::agent_sessions::live_titles::{HistoryTitleEvent, HistoryTitleSubscription};
        for case in ["close", "cancel", "disconnect", "late-cancel"] {
            let (entered, started) = std::sync::mpsc::channel();
            let (release, gate) = std::sync::mpsc::channel();
            let gate = Mutex::new(gate);
            let (tx, rx) = smol::channel::bounded::<HistoryTitleEvent>(1);
            let (dropped, retired) = std::sync::mpsc::channel();
            let host = title_test_host(Arc::new(move |_| {
                entered.send(()).unwrap();
                gate.lock()
                    .unwrap()
            ...
            client
                ...
                .unwrap();
            let served = std::thread::spawn(move || serve(server, host));
            ControlClientMsg::Hello(ControlHello::host_rpc("title-test", "title-test"))
                .encode(&mut client)
                .unwrap();
            assert!(matches!(
                ControlServerMsg::read(&mut client).unwrap(),
                ControlServerMsg::HelloOk(_)
            ));
            let id = uuid::Uuid::new_v4();
            ControlClientMsg::Request {
                req_id: 1,
                req: ControlRequest::TitleWatchOpen {
                    id,
                    snapshot: empty_history(),
                },
            }
            ...
            if case == "close" {
                ControlClientMsg::Request {
                    req_id: 2,
                    req: ControlRequest::TitleWatchClose { id },
                }
                .encode(&mut client)
                .unwrap();
                assert!(matches!(
    ...
    #[test]
    fn checked_attachment_requires_current_proof_and_unique_layout() {
        let (services, _dir) = workspace_services();
        let w = tree_workspace(&services);
        let workspace = w.parse().unwrap();
        let machine = services.machine.as_ref().unwrap();
        let tab = machine
            .tab_create(workspace, None, machine::PaneSeed::bare(71), None, None)
            .unwrap();
        let ((mut owner, _), _served) =
            raw_hello(services.clone(), ControlHello::host_rpc("owner", "owner"));
        round_trip(
            &mut owner,
            1,
            ControlRequest::WorkspaceAttachIfFree { id: w.clone() },
        );
        let ControlReply::Ok(ReplyOk::AttachmentProof(proof)) = round_trip(
            &mut owner,
            2,
            ControlRequest::WorkspaceAttachmentProof { workspace },
        )
        ...
        let attach = || {
            assert!(
                services.attachments.handover.try_lock().is_err(),
                "handover unlocked before attachment"
            );
        ...
        assert!(
            services
                .with_pane_attachment(&proof, tab.id, 71, attach)
                .is_ok()
        );
        let mut forged = proof.clone();
        forged.nonce = uuid::Uuid::new_v4();
        assert!(
            services
                .with_pane_attachment(&forged, tab.id, 71, attach)
                .is_err()
        );
        assert!(
            services
                .with_pane_attachment(&proof, crate::core::machine::TabId::new(), 71, attach)
                .is_err()
        );
        assert!(
            services
                .with_pane_attachment(&proof, tab.id, 72, attach)
                .is_err()
        );
        assert!(
            Services::none()
        ...
        assert!(
            Services::with_machine(machine.clone())
                .with_pane_attachment(&proof, tab.id, 71, attach)
                .is_err(),
            "a separate authority accepted the proof"
        );
        assert_eq!(calls.get(), 1, "a refused check ran the side effect");
        machine.tab_close(workspace, tab.id, None).unwrap();
        assert!(
            services
                .with_pane_attachment(&proof, tab.id, 71, attach)
                .is_err()
        ...
        assert!(
            services
                .with_pane_attachment(&proof, replacement.id, 71, attach)
                .is_err()
        );
    ...
    #[test]
    fn attachment_proof_does_not_release_a_later_attachment() {
        let (services, _dir) = workspace_services();
        let w = tree_workspace(&services);
        let workspace = w.parse().unwrap();
        let ((mut client, _), _served) =
            raw_hello(services.clone(), ControlHello::host_rpc("same", "owner"));
        let attach = || ControlRequest::WorkspaceAttachIfFree { id: w.clone() };
        let get = || ControlRequest::WorkspaceAttachmentProof { workspace };
        round_trip(&mut client, 1, attach());
        let ControlReply::Ok(ReplyOk::AttachmentProof(old)) = round_trip(&mut client, 2, get()).0
        else {
            panic!("no proof")
        };
        round_trip(&mut client, 3, attach());
        assert_eq!(
            round_trip(&mut client, 4, get()).0,
            ControlReply::Ok(ReplyOk::AttachmentProof(old.clone()))
        );
        round_trip(
            &mut client,
            5,
        ...
        round_trip(&mut client, 6, attach());
        let ControlReply::Ok(ReplyOk::AttachmentProof(current)) =
            round_trip(&mut client, 7, get()).0
        else {
            panic!("no current proof")
        };
        let refused = round_trip(
            &mut client,
            8,
            ControlRequest::WorkspaceReleaseProof { proof: old.clone() },
        )
        .0;
        assert!(
            matches!(refused, ControlReply::Err(_)),
            "stale cleanup released replacement: {refused:?}"
        );
        assert_ne!(old, current);
        assert_eq!(services.attachments.holder(&w).unwrap().1, "owner");
        assert!(
            services
                .machine
                .as_ref()
        ...
        assert_eq!(
            round_trip(
                &mut client,
                9,
                ControlRequest::WorkspaceReleaseProof { proof: current }
            )
            .0,
    ...
    #[test]
    fn attachment_proof_is_connection_scoped_and_redacted() {
        let (services, _dir) = workspace_services();
        let w = tree_workspace(&services);
        let workspace = w.parse().unwrap();
        let ((mut owner, _), _a) =
            raw_hello(services.clone(), ControlHello::host_rpc("shared", "owner"));
        let ((mut other, _), _b) =
            raw_hello(services.clone(), ControlHello::host_rpc("shared", "other"));
        let get = || ControlRequest::WorkspaceAttachmentProof { workspace };
        assert!(matches!(
            round_trip(&mut owner, 1, get()).0,
            ControlReply::Err(_)
        ));
        round_trip(
            &mut owner,
        ...
        let ControlReply::Ok(ReplyOk::AttachmentProof(proof)) = round_trip(&mut owner, 3, get()).0
        else {
            panic!("no proof")
        };
        assert!(matches!(
            round_trip(&mut other, 1, get()).0,
            ControlReply::Err(_)
        ));
        assert!(matches!(
            round_trip(
        ...
        assert_eq!(services.attachments.holder(&w).unwrap().1, "owner");
        assert!(!format!("{proof:?}").contains(&proof.nonce.to_string()));
        assert!(
            !format!(
                "{:?}",
                ControlRequest::WorkspaceReleaseProof {
            ...
            .contains(&proof.nonce.to_string())
        );
        let mut wrong_nonce = proof.clone();
        wrong_nonce.nonce = uuid::Uuid::new_v4();
        let mut wrong_workspace = proof.clone();
        wrong_workspace.workspace = crate::core::session::WorkspaceId::new();
        for (index, invalid) in [wrong_nonce, wrong_workspace].into_iter().enumerate() {
            assert!(matches!(
                round_trip(
                    &mut owner,
                    index as u64 + 10,
                    ControlRequest::WorkspaceReleaseProof { proof: invalid }
                )
                .0,
                ControlReply::Err(_)
            ));
        ...
        assert_eq!(services.attachments.holder(&w).unwrap().1, "owner");
        let snapshot =
            serde_json::to_string(&services.machine.as_ref().unwrap().machine()).unwrap();
        assert!(!snapshot.contains("nonce"));
        assert!(!snapshot.contains(&proof.nonce.to_string()));
        round_trip(
            &mut other,
            3,
        ...
        assert_eq!(services.attachments.holder(&w).unwrap().1, "other");
    }

    #[test]
    fn if_free_acquisition_reports_fresh_and_reused_proof() {
        let (services, _dir) = workspace_services();
        let w = tree_workspace(&services);
        let ((mut owner, _), _served) =
            raw_hello(services.clone(), ControlHello::host_rpc("owner", "owner"));
        let mut first = None;
        for id in 1..=2 {
            let ControlReply::Ok(reply) = round_trip(
                &mut owner,
                id,
                ControlRequest::WorkspaceAttachIfFree { id: w.clone() },
            )
            .0
                ...
                panic!("acquisition refused")
            };
            let value = serde_json::to_value(&reply).unwrap();
            let acquisition = &value["workspace_acquired"];
            assert_eq!(acquisition["newly_acquired"], id == 1);
            let proof: crate::daemon::control::AttachmentProof =
                serde_json::from_value(acquisition["proof"].clone()).unwrap();
            assert_eq!(proof.workspace.to_string(), w);
            assert!(!format!("{reply:?}").contains(&proof.nonce.to_string()));
        ...
        let ControlReply::Ok(reply) = round_trip(
            &mut owner,
            4,
            ControlRequest::WorkspaceAttachIfFree { id: w },
        )
        .0
        else {
            panic!("reacquisition refused")
        };
        let value = serde_json::to_value(reply).unwrap();
        assert_eq!(value["workspace_acquired"]["newly_acquired"], true);
        let replacement: crate::daemon::control::AttachmentProof =
            serde_json::from_value(value["workspace_acquired"]["proof"].clone()).unwrap();
        assert_ne!(replacement.nonce, first.nonce);
        assert!(matches!(
            round_trip(
                &mut owner,
                5,
                ControlRequest::WorkspaceReleaseProof { proof: first }
            )
            .0,
            ControlReply::Err(_)
        ));
    ...
    #[test]
    fn tracked_take_back_preserves_shared_and_unknown_consumers() {
        use super::aggregate_tests::{client_with, client_with_hello};
        for same_token in [false, true] {
            let (services, _dir) = workspace_services();
            let w = tree_workspace(&services);
            let workspace = w.parse().unwrap();
            let client = client_with(services.clone());
            let foreign = client_with_hello(
                services.clone(),
                ControlHello::host_rpc(if same_token { "tok" } else { "foreign" }, "other-host"),
            );

            let (first, who) = client.take_back_workspace_use(workspace).unwrap();
            assert!(who.is_none());
            let (second, who) = client.take_back_workspace_use(workspace).unwrap();
            assert!(who.is_none());
            assert_eq!(first.proof(), second.proof());
            assert!(!client.finish_workspace_use(first.clone()).unwrap());
            assert!(services.attachments.holder(&w).is_some());
            assert!(client.finish_workspace_use(second).unwrap());
            assert!(services.attachments.holder(&w).is_none());

            let old = client.acquire_workspace_use(workspace).unwrap();
            let (other, _) = foreign.take_back_workspace_use(workspace).unwrap();
            let (taken, who) = client.take_back_workspace_use(workspace).unwrap();
            assert_eq!(who.is_some(), !same_token);
            assert_ne!(old.proof(), taken.proof());
            assert!(!client.finish_workspace_use(old).unwrap());
            assert!(!client.finish_workspace_use(first).unwrap());
            let reused = client.acquire_workspace_use(workspace).unwrap();
            assert!(!client.finish_workspace_use(taken).unwrap());
            assert!(client.finish_workspace_use(reused).unwrap());
            // A foreign stale proof cannot release the new owner's claim.
            assert!(foreign.finish_workspace_use(other).is_err());
            assert!(services.attachments.holder(&w).is_none());

            // Raw same-owner claims, even without a recorded proof, must stay pinned.
            for record_proof in [false, true] {
                client
                    .call(if record_proof {
                        ControlRequest::WorkspaceAttachIfFree { id: w.clone() }
                    } else {
                        ControlRequest::WorkspaceAttach { id: w.clone() }
                    })
                    .unwrap();
                let (usage, who) = client.take_back_workspace_use(workspace).unwrap();
                assert!(who.is_none());
                assert!(!client.finish_workspace_use(usage.clone()).unwrap());
                assert!(services.attachments.holder(&w).is_some());
                client
                    .call(ControlRequest::WorkspaceReleaseProof {
                        proof: usage.proof().clone(),
                    })
                    .unwrap();
            }
            client.close();
    ...
    #[test]
    fn tracked_workspace_uses_release_only_the_last_owned_user() {
        use super::aggregate_tests::client_with;
        let (services, _dir) = workspace_services();
        let w = tree_workspace(&services);
        let workspace = w.parse().unwrap();
        let client = client_with(services.clone());
        let first = client.acquire_workspace_use(workspace).unwrap();
        let second = client.acquire_workspace_use(workspace).unwrap();
        assert_eq!(first.proof(), second.proof());
        assert!(!client.finish_workspace_use(first.clone()).unwrap());
        assert!(
            !client.finish_workspace_use(first).unwrap(),
            "duplicate finish"
        );
        assert!(services.attachments.holder(&w).is_some());
        let foreign = client_with(services.clone());
        assert!(!foreign.finish_workspace_use(second.clone()).unwrap());
        assert!(client.finish_workspace_use(second.clone()).unwrap());
        assert!(services.attachments.holder(&w).is_none());
        let next = client.acquire_workspace_use(workspace).unwrap();
        assert!(!client.finish_workspace_use(second).unwrap());
        assert!(services.attachments.holder(&w).is_some());
        client
            .call(ControlRequest::WorkspaceAttachIfFree { id: w.clone() })
            .unwrap();
        assert!(
            !client.finish_workspace_use(next.clone()).unwrap(),
            "untracked consumer still owns the claim"
        );
        assert!(services.attachments.holder(&w).is_some());
        client
            .call(ControlRequest::WorkspaceReleaseProof {
                proof: next.proof().clone(),
            })
            .unwrap();
        client.close();
        foreign.close();
    ...
    #[test]
    fn client_cancellation_racing_reacquire_preserves_new_claim() {
        use super::aggregate_tests::client_with;
        let (services, _dir) = workspace_services();
        let w = tree_workspace(&services);
        let workspace = w.parse().unwrap();
        let client = client_with(services);
        let acquire = || client.acquire_workspace_use(workspace).unwrap();
        for _ in 0..16 {
            let old = acquire();
            ...
            let next = std::thread::scope(|scope| {
                let cleanup = scope.spawn(|| {
                    barrier.wait();
                    client.finish_workspace_use(old).unwrap()
                });
                let reopen = scope.spawn(|| {
                    barrier.wait();
                    acquire()
                });
                let next = reopen.join().unwrap();
                cleanup.join().unwrap();
                next
            });
            assert!(
                matches!(client.call(ControlRequest::WorkspaceAttachmentProof { workspace }).unwrap(),
                ReplyOk::AttachmentProof(proof) if &proof == next.proof())
            );
            assert!(client.finish_workspace_use(next).unwrap());
        }
        client.close();
    }

    #[test]
    fn client_cancellation_preserves_other_workspace_users() {
        use super::aggregate_tests::client_with;
        let (services, _dir) = workspace_services();
        let w = tree_workspace(&services);
        let workspace = w.parse().unwrap();
        ...
        assert_eq!(
            reused.proof(),
            next.proof(),
            "a new consumer reuses the live claim"
        );
        assert!(!client.finish_workspace_use(next.clone()).unwrap());
        assert!(
            matches!(client.call(ControlRequest::WorkspaceAttachmentProof { workspace }).unwrap(),
            ReplyOk::AttachmentProof(proof) if &proof == next.proof())
        );
        let foreign = client_with(services.clone());
        assert!(!foreign.finish_workspace_use(reused.clone()).unwrap());
        // Explicit release is distinct from canceled-operation cleanup.
        client
            .call(ControlRequest::WorkspaceReleaseProof {
                proof: reused.proof().clone(),
            })
            .unwrap();
        assert!(!client.finish_workspace_use(reused).unwrap());
        let fresh = acquire();
        assert!(client.finish_workspace_use(fresh).unwrap());
        assert!(services.attachments.holder(&w).is_none());
        let before_takeover = acquire();
        client
            .call(ControlRequest::WorkspaceAttach { id: w.clone() })
            .unwrap();
        assert!(
            !client
                .finish_workspace_use(before_takeover.clone())
                .unwrap()
        );
        assert!(services.attachments.holder(&w).is_some());
        client
            .call(ControlRequest::WorkspaceReleaseProof {
                proof: before_takeover.proof().clone(),
            })
            .unwrap();
        client.close();
        foreign.close();
    ...
    #[test]
    fn reused_workspace_proof_is_not_a_view_lifetime() {
        let (services, _dir) = workspace_services();
        let w = tree_workspace(&services);
        let workspace = w.parse().unwrap();
        let machine = services.machine.as_ref().unwrap();
        ...
        let ((mut client, _), _served) =
            raw_hello(services.clone(), ControlHello::host_rpc("same", "owner"));
        let mut acquire = |id, expected_new| {
            let ControlReply::Ok(ReplyOk::WorkspaceAcquired {
                proof,
                newly_acquired,
            }) = round_trip(
                &mut client,
            ...
            .0
                ...
                panic!("workspace acquisition refused")
            };
            assert_eq!(newly_acquired, expected_new);
            proof
        };
        let original_view = acquire(1, true);
        let reopened_view = acquire(2, false);
        assert_eq!(original_view, reopened_view);
        assert!(
            services
                .with_pane_attachment(&reopened_view, tab.id, 71, || Ok(()))
                .is_ok()
        );

        // This is the protocol's boundary, not a safe GUI cleanup algorithm:
        // a window reopening on the same connection has not replaced the claim.
        assert!(matches!(
            round_trip(
                &mut client,
                3,
        ...
        let retained = machine.workspace(workspace).unwrap();
        assert!(retained.attachment.is_none());
        assert_eq!(retained.tabs.len(), 1);
        assert_eq!(retained.tabs[0], tab);
        assert!(
            machine.pane(71).is_some(),
            "release must preserve pane records"
        );
        assert!(services.attachments.holder(&w).is_none());
    }

    #[test]
    fn if_free_attachment_preserves_foreign_owner() {
        let (services, _dir) = workspace_services();
        let w = tree_workspace(&services);
        let ((mut owner, _), _owner_thread) = raw_hello(
            services.clone(),
            ControlHello::host_rpc("shared-token", "owner"),
        );
        let request = || ControlRequest::WorkspaceAttachIfFree { id: w.clone() };
        for id in 1..=2 {
            assert!(matches!(
                round_trip(&mut owner, id, request()).0,
                ControlReply::Ok(ReplyOk::WorkspaceAcquired { newly_acquired, proof })
                    if newly_acquired == (id == 1) && proof.workspace.to_string() == w
            ));
        }
        let before = services
            .machine
        ...
        // A token is not ownership of this exact connection.
        let ((mut contender, _), _contender_thread) = raw_hello(
            services.clone(),
            ControlHello::host_rpc("shared-token", "contender"),
        );
        let reply = round_trip(&mut contender, 1, request()).0;
        assert!(
            matches!(reply, ControlReply::Err(_)),
            "occupied request succeeded: {reply:?}"
        );
        ...
        assert!(
            matches!(reply, ControlReply::Err(_)),
            "UUID alias bypassed ownership: {reply:?}"
        );
        round_trip(
            &mut contender,
            5,
        ...
        let (pong, events) = round_trip(&mut owner, 3, ControlRequest::Ping);
        assert_eq!(pong, ControlReply::Ok(ReplyOk::Pong));
        assert!(
            !events
                .iter()
                .any(|event| matches!(event, ControlEvent::Preempted { .. }))
        );
        // Refusal does not remove explicit takeover capability.
        assert!(matches!(
            round_trip(
                &mut contender,
                2,
        ...
        assert_eq!(services.attachments.holder(&w).unwrap().1, "contender");
    }

    #[test]
    fn if_free_attachment_has_one_concurrent_winner() {
        let (services, _dir) = workspace_services();
        let w = tree_workspace(&services);
        let barrier = std::sync::Barrier::new(2);
        let results = std::thread::scope(|scope| {
            let handles: Vec<_> = ["first", "second"]
                ...
                .map(|name| {
                    ...
                    scope.spawn(move || {
                        let ((mut client, _), _served) =
                            raw_hello(services, ControlHello::host_rpc(name, name));
                        barrier.wait();
                        let reply = round_trip(
                            &mut client,
                            1,
                        ...
                        .0;
                        // Keep the connection alive until both outcomes are observed.
                        (client, reply)
                    })
                })
                .collect();
        ...
        assert_eq!(
            results
                .iter()
                .filter(|(_, reply)| matches!(
                    reply,
                    ControlReply::Ok(ReplyOk::WorkspaceAcquired {
                        newly_acquired: true,
                        ..
                    })
                ))
        ...
        assert_eq!(
            results
                .iter()
                .filter(|(_, reply)| matches!(reply, ControlReply::Err(_)))
                .count(),
            1
        );
    ...
    #[test]
    fn if_free_attachment_rejects_missing_workspace() {
        let (services, _dir) = workspace_services();
        let ((mut client, _), _thread) =
            raw_hello(services.clone(), ControlHello::host_rpc("token", "client"));
        for (n, id) in [
            "malformed".to_string(),
            crate::core::session::WorkspaceId::new().to_string(),
        ]
        .into_iter()
        .enumerate()
        {
