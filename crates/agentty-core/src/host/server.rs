...
impl Conn {
    ...
    fn start_git_stream(&self, id: u64, cwd: PathBuf, args: Vec<String>) {
        ...
        let spawned = std::thread::Builder::new()
            .name("agentty-control-git-stream".into())
            .spawn(move || {
                let _slot = slot;
                let borrowed: Vec<&str> = args.iter().map(String::as_str).collect();
...
fn spawn_layout_forwarder(
    ...
    let spawned = std::thread::Builder::new()
        .name("agentty-control-layout".into())
        .spawn(move || {
...
fn spawn_watch_forwarder(id: u64, rx: smol::channel::Receiver<Vec<PathBuf>>, sink: Arc<Sink>) {
    let spawned = std::thread::Builder::new()
        .name("agentty-control-watch".into())
        .spawn(move || {
    ...
    fn submit(&self, job: impl FnOnce() + Send + 'static) -> bool {
        ...
        if st.wants_another_worker() {
            ...
            match std::thread::Builder::new()
                .name("agentty-control-worker".into())
...
pub const CONTROL_SOCK_ENV: &str = "AGENTTY_CONTROL_SOCK";
...
mod sock {
    ...
    ///
    /// It used to sit in `$XDG_RUNTIME_DIR/agentty` or `~/.local/share/agentty`,
        ...
        for stream in listener.incoming() {
            match stream {
                Ok(stream) => {
                    ...
                    let spawned = std::thread::Builder::new()
                        .name("agentty-control-conn".into())
                        .spawn(move || {
...
mod tests {
    ...
    fn binding_does_not_re_permission_a_directory_it_did_not_create() {
        ...
        assert_eq!(
            ...
            0o1777,
            "a directory agentty did not create must keep its own permissions"
        );
        drop(listener);
    }
