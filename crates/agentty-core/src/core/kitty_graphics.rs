...
//!   that survives agentty's socket + SSH topology.
...
impl GraphicsSniffer {
    ...
    /// is local when agentty spawned a local shell and no foreground `ssh` owns the
...
/// The kitty action key (`a=`). Only the variants agentty acts on are named; the
...
///
/// Only the keys agentty needs are surfaced; unknown keys are ignored so forward
...
/// agentty targets — most often `d=A` (delete everything) — actually uses.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
impl GraphicsParser {
    ...
    pub fn feed(&mut self, command: &[u8]) -> Option<Event> {
        ...
        match control.action {
            ...
            // get nothing; `a=T` (the shape every sender agentty targets uses)
            // is unaffected.
            Action::Display => None,
...
mod tests {
    ...
    fn file_transfer_resolves_and_temp_file_is_removed() {
        ...
        let path = dir.join(format!("agentty-kitty-test-{}.rgba", std::process::id()));
