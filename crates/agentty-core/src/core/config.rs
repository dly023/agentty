...
pub struct Config {
    ...
    /// Whether the GUI puts the bundled `agentty` CLI on PATH at launch (see
    ...
    /// Off is for people who keep their own `agentty` — a `cargo install` build, a
...
mod tests {
    ...
    impl TestDir {
        fn new(name: &str) -> Self {
            let dir =
                std::env::temp_dir().join(format!("agentty-test-{name}-{}", std::process::id()));
