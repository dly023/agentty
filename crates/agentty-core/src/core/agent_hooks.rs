...
pub const AGENTTY_ENV_MARKER: &str = "AGENTTY";
...
pub fn run_agent_hook(agent: &str, event: &str) {
    detach_console();
    if std::env::var_os(AGENTTY_ENV_MARKER).is_none() {
        return;
    }
...
fn write_to_controlling_tty(bytes: &[u8]) -> bool {
    ...
    let shell = ancestors.iter().copied().find(|&pid| {
        procs
            ...
            .is_some_and(|n| is_agentty_host_exe(&n))
    });
...
fn is_agentty_host_exe(name: &str) -> bool {
    matches!(
        name,
        "agentty-app.exe" | "agentty-server.exe" | "agentty.exe"
    )
}
...
const OWNED_FILE_STEM_JSON: &str = "agentty.json";
const OWNED_FILE_STEM_JS: &str = "agentty.js";
...
fn hook_map_state(
    ...
    for (hook_event, agentty_event) in events {
        let ours = root
            .get("hooks")
        ...
        match ours {
            Some(cmd) => {
                any = true;
                if cmd != target.hook_command(agent, agentty_event) {
                    complete = false;
                }
            }
            None => complete = false,
...
fn hook_map_install(
    ...
    let marker = agent.marker();
    for (hook_event, agentty_event) in events {
        let command = target.hook_command(agent, agentty_event);
        let entries = hooks
            .as_object_mut()
            .unwrap()
            .entry(*hook_event)
...
fn hook_map_uninstall(
    ...
    if removed == 0 {
        return Ok("No agentty hooks found; nothing to remove".to_string());
    }
    target.write(path, serde_json::to_string_pretty(&root)?.as_bytes())?;
...
fn owned_file_install(
    ...
    {
        return Err(anyhow::anyhow!(
            "{} exists but wasn't written by agentty; not touching it",
            path.display()
        ));
    }
...
fn owned_file_uninstall(target: &HookTarget, path: &Path, marker: &str) -> anyhow::Result<String> {
    ...
    if !contents.contains(marker) {
        return Err(anyhow::anyhow!(
            "{} wasn't written by agentty; not touching it",
            path.display()
        ));
    }
    target.host.remove(path, false)?;
    if let Some(parent) = path.parent()
        && parent.file_name().is_some_and(|n| n == "agentty")
    {
...
fn opencode_plugin_js(target: &HookTarget) -> Option<String> {
    ...
    .ok()?;
    Some(format!(
        r#"// agentty agent-hook opencode bridge — generated by agentty, do not edit.
// Bridges OpenCode plugin events onto `agentty agent-hook opencode <event>`,
// which is inert outside agentty (gated on the AGENTTY env var).
export const AgenttyPresence = async ({{ $ }}) => {{
  if (!process.env["AGENTTY"]) return {{}}
  const cmd = {prefix}
...
fn pi_extension_ts(target: &HookTarget) -> Option<String> {
    ...
    Some(format!(
        r#"/* agentty agent-hook pi bridge — generated by agentty, do not edit. */
...
mod tests {
    ...
    #[test]
    fn every_agentty_daemon_host_takes_the_console_fast_path() {
        for name in ["agentty-app.exe", "agentty-server.exe", "agentty.exe"] {
            assert!(is_agentty_host_exe(name), "{name} hosts agentty shells");
        }
        for name in [
            "explorer.exe",
            "cmd.exe",
            ...
            assert!(
                !is_agentty_host_exe(name),
                "{name} is not a agentty host process"
            );
        }
    }
    ...
    fn marker_detection_matches_our_entries_only() {
        let ours = serde_json::json!({
            "hooks": [{ "type": "command", "command": "\"/x/agentty\" agent-hook claude stop" }]
        });
    ...
    fn remote_paths_are_built_in_the_remote_machine_s_spelling() {
        ...
        for (agent, expected) in [
            ...
            (HookAgent::Copilot, "/home/me/.copilot/hooks/agentty.json"),
            (
                HookAgent::OpenCode,
                "/home/me/.config/opencode/plugins/agentty.js",
            ),
            (
                HookAgent::Pi,
                "/home/me/.pi/agent/extensions/agentty/index.ts",
            ),
            (HookAgent::Grok, "/home/me/.grok/hooks/agentty.json"),
        ] {
            assert_eq!(
    ...
    fn the_hook_command_names_the_binary_on_that_machine() {
        ...
        assert_eq!(
            ...
            format!("\"/home/me/.local/share/agentty/bin/{name}\" agent-hook claude stop")
        );
    ...
    fn a_remote_install_round_trips_through_the_host() {
        let dir = std::env::temp_dir().join(format!("agentty-remote-hooks-{}", std::process::id()));
    ...
    fn owned_file_round_trip_and_ownership_guard() {
        let dir = std::env::temp_dir().join(format!("agentty-owned-test-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("agentty.json");
    ...
    fn install_is_idempotent_and_preserves_user_hooks() {
        let dir = std::env::temp_dir().join(format!("agentty-hooks-test-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        ...
        assert_eq!(
            stop.iter()
                ...
                .count(),
            1,
            "exactly one agentty entry after two installs"
        );
        assert!(
            stop.iter()
        ...
        for (event, _) in CLAUDE_HOOK_EVENTS {
            assert!(
                ...
                "{event} carries the agentty hook"
            );
        }
        ...
        assert!(
            ...
            "an event list that held only the agentty hook is dropped"
        );
