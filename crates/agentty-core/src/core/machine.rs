...
pub const DATA_DIR_ENV: &str = "AGENTTY_DATA_DIR";
...
impl MachineStore {
    ...
    fn ensure_flusher(self: &Arc<Self>) {
        ...
        let spawned = std::thread::Builder::new()
            .name("agentty-machine-flush".into())
            .spawn(move || {
                loop {
...
fn data_dir() -> io::Result<PathBuf> {
    ...
    base.map(|b| b.join("agentty")).ok_or_else(|| {
...
mod tests {
    ...
    fn tabs_rename_move_and_regroup_in_place() {
        ...
        store
            ...
            .unwrap();
        store
            .tab_set_group(ws, first.id, Some("/repo/agentty".into()), None)
            .unwrap();
