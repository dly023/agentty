...
fn managed_root(host: &dyn Host, main_root: &Path) -> PathBuf {
    host.join(&host.join(main_root, ".agentty"), "worktrees")
}
...
pub fn managed(host: &dyn Host, cwd: &Path) -> Option<ManagedWorktree> {
    ...
    let suffix = host.join(Path::new(".agentty"), "worktrees");
...
pub fn create(host: &dyn Host, cwd: &Path, req: &WorktreeRequest) -> Result<NewWorktree, String> {
    ...
    let ignore = host.join(
        dir.parent().expect(".agentty/worktrees has a parent"),
        ".gitignore",
    );
...
mod tests {
    ...
    fn scratch(name: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!("agentty-wt-{name}-{}", std::process::id()));
    ...
    fn defaults_proposes_fresh_name_current_branch_and_target_dir() {
        ...
        assert_eq!(plain(&d.dir), canon.join(".agentty").join("worktrees"));
    ...
    fn create_makes_worktree_on_new_branch_inside_the_repo() {
        ...
        assert_eq!(
            plain(&wt.path),
            canon.join(".agentty/worktrees/quiet-otter")
        );
        let head = git(&*h, &wt.path, &["rev-parse", "--abbrev-ref", "HEAD"]).unwrap();
        assert_eq!(head, wt.branch);
        assert_eq!(
            std::fs::read_to_string(canon.join(".agentty/.gitignore")).unwrap(),
            "*\n"
        );
