...
pub const DEFAULT_REMOTE_SERVER_CMD: &str = "agentty-server --stdio";
...
const ENV_MARKER: &str = "__agentty_env__";

pub const REMOTE_ENV_PROBE: &str = concat!(
    "sh -c 'printf \"__agentty_env__ %s\\n\" ",
    "\"sock=${AGENTTY_CONTROL_SOCK-}\" \"cfg=${AGENTTY_CONFIG_DIR-}\" ",
    "\"xdg=${XDG_RUNTIME_DIR-}\" ",
...
mod tests {
    ...
    fn only_a_pane_link_asks_for_the_pane_socket() {
        let server = "/home/me/.local/share/agentty/bin/agentty-server-26.7.6";
        assert_eq!(
    ...
    fn the_entry_falls_back_exactly_when_streamlocal_cannot_be_used() {
        let cmd = "agentty-server --stdio";
        assert_eq!(
            choose_entry(Some("/run/user/1000/agentty/daemon.sock"), true, cmd),
            RemoteEntry::StreamLocal {
                socket: "/run/user/1000/agentty/daemon.sock".into()
            }
        );
        assert_eq!(
            choose_entry(Some("/run/user/1000/agentty/daemon.sock"), false, cmd),
    ...
    fn the_env_probe_survives_a_chatty_remote() {
        let out = "Welcome to Ubuntu!\n\
                   __agentty_env__ sock=\n\
                   __agentty_env__ xdg=/run/user/1000\n\
                   __agentty_env__ home=/home/me\n\
                   __agentty_env__ tmp=\n\
                   You have mail.\n";
    ...
    fn the_remote_socket_path_matches_the_servers_own_order() {
        ...
        assert_eq!(
            ...
            Some("/tmp/mine.sock"),
            "an explicit $AGENTTY_CONTROL_SOCK outranks everything"
        );
