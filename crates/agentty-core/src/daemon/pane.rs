...
struct ChosenShell {
    program: String,
    args: Vec<String>,
    args_are_agentty_defaults: bool,
}

fn has_custom_args(chosen: Option<&ChosenShell>) -> bool {
    chosen.is_some_and(|c| !c.args.is_empty() && !c.args_are_agentty_defaults)
}
...
fn choose_shell(
    ...
    spawn_override
        .map(|s| ChosenShell {
            program: s.program,
            args: s.args,
            args_are_agentty_defaults: s.args_are_agentty_defaults,
        })
        .or_else(|| {
            configured.map(|(program, args)| ChosenShell {
                program,
                args,
                args_are_agentty_defaults: false,
            })
        })
}
...
const TERM_PROGRAM_NAME: &str = "agentty";
...
const AGENTTY_PANE_ENV: &str = "AGENTTY_PANE";
const AGENTTY_WS_ENV: &str = "AGENTTY_WS";
...
fn pane_environment(
    ...
    let mut env = vec![
        ...
        (AGENTTY_PANE_ENV.to_string(), pane.to_string()),
    ];
    if let Some(ws) = workspace {
        env.push((AGENTTY_WS_ENV.to_string(), ws.to_string()));
    }
...
impl DaemonPane {
    ...
    fn spawn_reader(
        ...
        std::thread::Builder::new()
            .name("agentty-daemon-pane-reader".to_string())
            .spawn(move || {
    ...
    fn spawn_exit_monitor(
        ...
        std::thread::Builder::new()
            .name("agentty-daemon-pane-exit-monitor".to_string())
            .spawn(move || {
...
fn join_bounded(handle: JoinHandle<()>, timeout: Duration) -> bool {
    ...
    if std::thread::Builder::new()
        .name("agentty-daemon-pane-join".to_string())
        .spawn(move || {
            let _ = handle.join();
            let _ = tx.send(());
        })
...
mod tests {
    ...
    fn initial_working_directory_skips_paths_that_are_not_directories() {
        ...
        let file = real.join("agentty-iwd-probe");
    ...
    fn only_user_authored_args_block_shell_integration() {
        let chosen = |args: Vec<&str>, agentty: bool| ChosenShell {
            program: r"C:\Program Files\Git\bin\bash.exe".to_string(),
            args: args.into_iter().map(str::to_string).collect(),
            args_are_agentty_defaults: agentty,
        };
    ...
    fn env_only_integration_keeps_default_login_shell_builder() {
        ...
        env.insert(
            "ZDOTDIR".to_string(),
            "/tmp/agentty-zdotdir-test".to_string(),
        );
        let injection = shell_integration::Injection {
            env,
            args: Vec::new(),
    ...
    fn at_prompt_covers_prompt_draw_gap_across_chunks() {
        ...
        let chunk = s.feed(
            b"\x1b]133;A\x07\x1b]7;file://host/repo/agentty\x07\r\nagentty git:(main) \xe2\x9e\x9c ",
        );
        assert!(
    ...
    fn sentinel_events_drive_agent_session_state() {
        ...
        let mut sniffer = OscSniffer::new();
        let stream = concat!(
            "\x1b]777;notify;agentty://cli-agent;",
    ...
    fn probed_cwd_corrects_a_pane_whose_shell_never_reports_osc7() {
        ...
        apply_probed_cwd(&mut st, Some(PathBuf::from("/Users/alice/dev/agentty")));

        assert_eq!(
            st.cwd.as_deref(),
            Some(Path::new("/Users/alice/dev/agentty"))
        );
        assert!(
            matches!(rx.try_recv(), Ok(DaemonMsg::Cwd(p)) if p == PathBuf::from("/Users/alice/dev/agentty"))
        );
    }
    ...
    fn probed_cwd_keeps_the_shells_spelling_for_a_symlinked_path() {
        let tmp = std::env::temp_dir().join(format!("agentty-cwd-{}", std::process::id()));
    ...
    fn probed_cwd_matching_the_reported_one_says_nothing() {
        let mut st = test_state(true);
        st.cwd = Some(PathBuf::from("/Users/alice/dev/agentty"));
        let (tx, rx) = mpsc::channel();
        st.subscriber = Some(tx);
    ...
    fn attach_replays_initial_cwd_even_before_shell_reports_osc7() {
        let mut st = test_state(true);
        st.cwd = Some(PathBuf::from("/Users/alice/clone/agentty"));
        ...
        assert!(
            matches!(rx.try_recv(), Ok(DaemonMsg::Cwd(p)) if p == PathBuf::from("/Users/alice/clone/agentty"))
        );
    }
    ...
    fn reader_poll_applies_the_probed_cwd() {
        ...
        let handle = DaemonPane::spawn_reader(
            ...
            ForegroundProbes {
                remote: Box::new(|| None),
                agent: Box::new(|| None),
                cwd: Box::new(|| Some(PathBuf::from("/Users/alice/dev/agentty"))),
            },
            Arc::new(DeathReporter::new(|| {})),
        );
        handle.join().unwrap();
        ...
        assert!(
            matches!(sub_rx.try_recv(), Ok(DaemonMsg::Cwd(p)) if p == PathBuf::from("/Users/alice/dev/agentty"))
        );
    }
    ...
    fn pane_environment_advertises_the_terminal_under_the_standard_names() {
        ...
        assert_eq!(env.get("TERM_PROGRAM").map(String::as_str), Some("agentty"));
        assert_eq!(
            env.get("TERM_PROGRAM_VERSION").map(String::as_str),
            Some(version)
        );
        assert_eq!(
            env.get(crate::core::agent_hooks::AGENTTY_ENV_MARKER)
                .map(String::as_str),
            Some(version)
        );
        assert_eq!(
    ...
    fn pane_environment_hands_the_shell_its_own_address() {
        ...
        assert_eq!(
            env.get(AGENTTY_PANE_ENV).map(String::as_str),
            Some("42"),
            "a CLI inside the pane needs its own pane id for address-free verbs"
        );
        assert_eq!(
            env.get(AGENTTY_WS_ENV).map(String::as_str),
            Some("ws-main"),
        ...
        let unfiled: std::collections::HashMap<_, _> =
            pane_environment(&std::collections::HashMap::new(), 42, None)
                ...
                .collect();
        assert!(
            !unfiled.contains_key(AGENTTY_WS_ENV),
    ...
    #[test]
    fn spawned_shell_carries_the_agentty_marker() {
        let cmd = build_shell_command(None, &Some(PathBuf::from("/tmp")), 42, Some("ws-main"))
            .expect("build default shell command")
            .0;
        let agentty = cmd
            .get_env(crate::core::agent_hooks::AGENTTY_ENV_MARKER)
            .and_then(|v| v.to_str());
        assert_eq!(
            agentty,
            Some(env!("CARGO_PKG_VERSION")),
            "the daemon must inject AGENTTY into every spawned shell"
        );
        assert_eq!(
            cmd.get_env(AGENTTY_PANE_ENV).and_then(|v| v.to_str()),
            Some("42"),
            "the daemon must tell every spawned shell which pane it is"
        );
        assert_eq!(
            cmd.get_env(AGENTTY_WS_ENV).and_then(|v| v.to_str()),
            Some("ws-main"),
    ...
    fn the_two_endpoints_are_siblings_derived_from_one_config_dir() {
        ...
        // and sit in ~/.local/share/agentty as `daemon.sock`, the same basename the
        ...
        let dir = std::path::Path::new("/tmp/agentty-endpoint-test");
