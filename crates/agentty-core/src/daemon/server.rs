...
fn spawn_orphan_sweep(registry: Arc<Registry>) {
    let spawned = std::thread::Builder::new()
        .name("agentty-orphan-sweep".into())
        .spawn(move || {
        ...
        match stream {
            Ok(stream) => {
                ...
                std::thread::Builder::new()
                    .name("agentty-daemon-conn".to_string())
                    .spawn(move || {
...
fn serve_sigterm(registry: Arc<Registry>) {
    ...
    std::thread::Builder::new()
        .name("agentty-daemon-sigterm".to_string())
        .spawn(move || {
    ...
    match first {
        ClientMsg::Spawn {
            ...
            let on_dead = {
                ...
                move || {
                    std::thread::Builder::new()
                        .name("agentty-daemon-pane-reap".to_string())
                        .spawn(move || {
                            registry.remove(id);
                        })
                        .ok();
...
fn spawn_writer(
    ...
    std::thread::Builder::new()
        .name("agentty-daemon-writer".to_string())
        .spawn(move || {
            crate::core::threads::promote_to_user_interactive();
