    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::MissingBundled { asset, searched } => write!(
                f,
                "this build of agentty does not ship a Linux server binary, so it cannot \
            ...
            Self::Declined { host, path } => write!(
                f,
                "installing agentty-server at {path} on {host} was not confirmed; nothing was written"
            ),
            Self::Write { path, reason } => {
                write!(f, "could not write {path} on the remote machine: {reason}")
            }
            Self::Launch { reason } => {
                write!(f, "the remote agentty-server did not start: {reason}")
            }
            Self::DialectMismatch {
                ...
                write!(
                    f,
                    "this build needs a agentty-server speaking control v{} and protocol v{}, \
    ...
    fn is_first_install(&self, paths: &RemotePaths) -> bool {
        match self.ops.list_dir(&paths.bin_dir) {
            Ok(Some(entries)) => !entries
                .iter()
                .any(|name| name.starts_with("agentty-server-")),
            Ok(None) => true,
            Err(_) => true,
        }
    }
