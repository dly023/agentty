...
pub const ASSET_X86_64: &str = "agentty-server-linux-x86_64-musl";
pub const ASSET_AARCH64: &str = "agentty-server-linux-aarch64-musl";
pub const CHECKSUMS_ASSET: &str = "checksums.txt";

pub const RELEASE_BASE: &str = "https://github.com/dly023/agentty/releases/download";

pub const INSTALL_DIR_COMPONENTS: [&str; 4] = [".local", "share", "agentty", "bin"];
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::NotLinux { raw } => write!(
                f,
                "a remote agentty workspace needs a Linux host; this machine reports `uname -sm` = {raw:?}"
            ),
            Self::UnknownMachine { raw } => write!(
                f,
                "no agentty-server is published for this architecture (`uname -sm` = {raw:?}); \
...
pub fn binary_name(control: u32, protocol: u32) -> String {
    format!("agentty-server-c{control}p{protocol}")
}
...
pub fn dialect_from_path(path: &str) -> Option<(u32, u32)> {
    ...
    let (control, protocol) = name.strip_prefix("agentty-server-c")?.split_once('p')?;
...
mod tests {
    ...
    fn download_urls_point_at_the_release_the_tag_names() {
        assert_eq!(
            ...
            "https://github.com/dly023/agentty/releases/download/v26.7.5/agentty-server-linux-x86_64-musl"
        );
        assert_eq!(
            download_url(&release_tag("26.7.6-nightly.20260727"), CHECKSUMS_ASSET),
            "https://github.com/dly023/agentty/releases/download/nightly/checksums.txt"
        );
    }

    #[test]
    fn asset_names_are_the_ones_the_release_workflow_publishes() {
        assert_eq!(ASSET_X86_64, "agentty-server-linux-x86_64-musl");
        assert_eq!(ASSET_AARCH64, "agentty-server-linux-aarch64-musl");
    ...
    fn remote_paths_are_posix_and_named_by_dialect() {
        ...
        assert_eq!(p.bin_dir, "/home/me/.local/share/agentty/bin");
        assert_eq!(
            p.binary,
            "/home/me/.local/share/agentty/bin/agentty-server-c3p4"
        );
        assert_eq!(
            p.temp,
            "/home/me/.local/share/agentty/bin/.agentty-server-c3p4.tmp"
        );
        assert_eq!(
            p.dir_chain,
            vec![
                "/home/me/.local",
                "/home/me/.local/share",
                "/home/me/.local/share/agentty",
                "/home/me/.local/share/agentty/bin",
            ]
        );
        assert!(
    ...
    fn trailing_slash_on_home_is_absorbed() {
        assert_eq!(
            ...
            "/root/.local/share/agentty/bin/agentty-server-c1p1"
        );
        assert_eq!(remote_paths("/", 1, 1).bin_dir, "/.local/share/agentty/bin");
    }

    #[test]
    fn dialects_are_recoverable_from_an_install_path() {
        assert_eq!(
            dialect_from_path("/home/me/.local/share/agentty/bin/agentty-server-c3p4"),
            Some((3, 4))
        );
        assert_eq!(dialect_from_path("agentty-server-c12p30"), Some((12, 30)));
        assert_eq!(dialect_from_path("/usr/bin/agentty-server"), None);
        assert_eq!(dialect_from_path("/bin/bash"), None);
        assert_eq!(dialect_from_path("agentty-server-c3"), None);
        assert_eq!(dialect_from_path("agentty-server-cxpy"), None);
    }

    #[test]
    fn legacy_version_named_binaries_carry_no_dialect() {
        for legacy in [
            "/home/me/.local/share/agentty/bin/agentty-server-26.7.4",
            "agentty-server-26.7.6-nightly.20260727",
            "agentty-server-0.1.0",
            "/usr/local/bin/agentty-server-",
        ] {
