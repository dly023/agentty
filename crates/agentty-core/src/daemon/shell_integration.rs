...
# --- agentty shell integration (zsh) ---
if [[ -o interactive ]] && [[ -z "$AGENTTY_SHELL_INTEGRATION" ]]; then
  export AGENTTY_SHELL_INTEGRATION=1

  __agentty_osc() { builtin printf '\e]%s\a' "$1"; }
  ...
  # the Esc widget name.
  __agentty_report_edit_mode() {
    if [[ "$(builtin bindkey -lL main)" == *viins* ]]; then
      __agentty_osc "133;V;1"
    else
      __agentty_osc "133;V;0"
    fi
  }
  ...
  __agentty_report_cwd() { builtin printf '\e]7;file://%s%s\a' "${HOST:-localhost}" "${PWD//\%/%25}"; }

  # D (command finished + its exit code) gets its own hook, *prepended* to
  # precmd_functions rather than bundled into __agentty_precmd below: the app only
  ...
  __agentty_precmd_d() {
    local ret=$?
    if [[ -n "$__agentty_cmd_active" ]]; then
      __agentty_osc "133;D;$ret"
      unset __agentty_cmd_active
    fi
  }
  ...
  __agentty_preexec() {
    ...
    cmd=${cmd//$'\r'/%0D}
    cmd=${cmd//$'\n'/%0A}
    __agentty_osc "133;C;$cmd"
  }

  autoload -Uz add-zsh-hook
  add-zsh-hook precmd __agentty_precmd
  add-zsh-hook preexec __agentty_preexec
  ...
  precmd_functions=(__agentty_precmd_d $precmd_functions)
  ...
  __agentty_restore_zdotdir() {
    ZDOTDIR=${AGENTTY_USER_ZDOTDIR:-$HOME}
    add-zsh-hook -d precmd __agentty_restore_zdotdir
    unfunction __agentty_restore_zdotdir
  }
  add-zsh-hook precmd __agentty_restore_zdotdir
fi
# --- end agentty shell integration ---
"#;

const FISH_INTEGRATION: &str = r#"
# --- agentty shell integration (fish) ---
...
if status is-interactive; and test -z "$AGENTTY_SHELL_INTEGRATION"
  set -gx AGENTTY_SHELL_INTEGRATION 1

  function __agentty_osc
    printf '\e]%s\a' $argv[1]
  end

  function __agentty_report_edit_mode
  ...
  function __agentty_preexec --on-event fish_preexec
    set -g __agentty_cmd_active 1
  ...
  function __agentty_precmd --on-event fish_prompt
    set -l ret $status
    if set -q __agentty_cmd_active
      __agentty_osc "133;D;$ret"
      set -e __agentty_cmd_active
    end
    __agentty_report_cwd
    __agentty_report_edit_mode
    __agentty_osc "133;A"
  ...
  functions -c fish_prompt __agentty_original_fish_prompt
  function fish_prompt
    __agentty_original_fish_prompt
    __agentty_osc "133;B"
  end
end
# --- end agentty shell integration ---
...
# --- agentty shell integration (bash) ---
if [[ $- == *i* ]] && [[ -z "$AGENTTY_SHELL_INTEGRATION" ]]; then
  export AGENTTY_SHELL_INTEGRATION=1
  ...
  if [[ "$OSTYPE" == msys* || "$OSTYPE" == cygwin* ]]; then
    ...
    __agentty_report_cwd() { builtin printf '\e]7;file://%s%s\a' "${HOSTNAME:-localhost}" "${PWD//\%/%25}"; }
  fi
  ...
  precmd_functions=(__agentty_precmd_d "${precmd_functions[@]}")
  precmd_functions+=(__agentty_precmd)
  preexec_functions+=(__agentty_preexec)
fi
# --- end agentty shell integration ---
"#;

const POWERSHELL_INTEGRATION: &str = r#"
# --- agentty shell integration (PowerShell) ---
if (-not $env:AGENTTY_SHELL_INTEGRATION) {
  $env:AGENTTY_SHELL_INTEGRATION = '1'

  $global:__AgenttyEsc = [char]0x1b
  $global:__AgenttyBel = [char]0x07
  # Whatever prompt the user's profile settled on; we call through to it.
  $global:__AgenttyOrigPrompt = $function:prompt
  # Gates the D marker so the first prompt (no command yet) emits no bogus exit.
  $global:__AgenttyCmdActive = $false
  ...
  function global:prompt {
    ...
    if ($global:__AgenttyCmdActive) {
      ...
      Write-Host -NoNewline "$($global:__AgenttyEsc)]133;D;$code$($global:__AgenttyBel)"
    }
    ...
    if ($PWD.Provider.Name -eq 'FileSystem') {
      ...
      Write-Host -NoNewline "$($global:__AgenttyEsc)]7;file://$($env:COMPUTERNAME)$p$($global:__AgenttyBel)"

      # OSC 0 window/tab title "user@host:dir". PowerShell profiles don't set a
      # title the way macOS's default zsh does, so without this every agentty tab on
      # Windows stays generic. Forward slashes (so agentty's tab-label parser can take
      ...
      Write-Host -NoNewline "$($global:__AgenttyEsc)]0;$($env:USERNAME)@$($env:COMPUTERNAME):$titlePath$($global:__AgenttyBel)"
    }
    ...
    $base = & $global:__AgenttyOrigPrompt
    if ($base -is [array]) { $base = $base -join [char]0x0a }
    $global:LASTEXITCODE = $lastExit

    "$($global:__AgenttyEsc)]133;A$($global:__AgenttyBel)$base$($global:__AgenttyEsc)]133;B$($global:__AgenttyBel)"
  }
  ...
  if ((Test-Path Function:\PSConsoleHostReadLine) -and -not $global:__AgenttyReadLineWrapped) {
    $global:__AgenttyReadLineWrapped = $true
    $global:__AgenttyOrigReadLine = $function:global:PSConsoleHostReadLine
    function global:PSConsoleHostReadLine {
      $line = & $global:__AgenttyOrigReadLine
      if (-not [string]::IsNullOrWhiteSpace($line)) {
        $global:__AgenttyCmdActive = $true
        # Carry the submitted line on the C mark (agentty extension): the daemon
        ...
        try {
          ...
          Write-Host -NoNewline "$($global:__AgenttyEsc)]133;C;$cmd$($global:__AgenttyBel)"
        } catch {
          Write-Host -NoNewline "$($global:__AgenttyEsc)]133;C$($global:__AgenttyBel)"
        }
      }
      $line
    }
...
fn zsh_redirectors() -> [(&'static str, String); 4] {
    let redirect = |name: &str, tail: &str| {
        format!(
            "__agentty_ztmp=$ZDOTDIR\n\
             if [[ -n \"$AGENTTY_USER_ZDOTDIR\" ]]; then ZDOTDIR=$AGENTTY_USER_ZDOTDIR; else unset ZDOTDIR; fi\n\
             [[ -f \"${{ZDOTDIR:-$HOME}}/{name}\" ]] && source \"${{ZDOTDIR:-$HOME}}/{name}\"\n\
             {tail}ZDOTDIR=$__agentty_ztmp\n\
             unset __agentty_ztmp\n"
        )
    };
    [
        (
            ".zshenv",
            redirect(".zshenv", "export AGENTTY_USER_ZDOTDIR=${ZDOTDIR:-$HOME}\n"),
        ),
...
const ZDOTDIR_PREFIX: &str = "agentty-zdotdir-";
...
fn real_user_zdotdir() -> Option<String> {
    if let Ok(z) = std::env::var("AGENTTY_USER_ZDOTDIR") {
        if !z.is_empty() {
            return Some(z);
        }
    }
...
fn setup_zsh() -> Option<Injection> {
    ...
    if let Some(user_zdotdir) = real_user_zdotdir() {
        env.insert("AGENTTY_USER_ZDOTDIR".to_string(), user_zdotdir);
    }
...
# necessary because agentty spawns bash non-login (see shell_integration.rs) so
...
fn setup_bash() -> Option<Injection> {
    let dir = throwaway_dir("agentty-bashrc-")?;
...
const WSL_RCFILE_ENV: &str = "AGENTTY_RC";

const WSL_EXEC_SCRIPT: &str = concat!(
    r#"case "${SHELL:-}" in "#,
    r#"*/bash) exec "$SHELL" --rcfile "$AGENTTY_RC" -i ;; "#,
...
fn setup_wsl(args: &[String]) -> Option<Injection> {
    ...
    let dir = throwaway_dir("agentty-wslrc-")?;
    let rcfile = dir.join("bashrc");
    std::fs::write(&rcfile, bash_rcfile()).ok()?;
...
pub fn setup(program: Option<&str>, args: &[String], has_custom_args: bool) -> Option<Injection> {
    ...
    injection
        .env
        .insert("AGENTTY_SHELL_INTEGRATION".to_string(), String::new());
...
pub mod remote {
    ...
    pub const PROBE_COMMAND: &str = "echo __agentty_shell; echo $SHELL";

    const PROBE_MARKER: &str = "__agentty_shell";

    const HEREDOC: &str = "__AGENTTY_RC_EOF__";
    ...
    fn write_file(out: &mut String, name: &str, body: &str) {
        out.push_str(&format!(
            "command cat > \"$__agentty_d/{name}\" <<'{HEREDOC}'\n{}\n{HEREDOC}\n",
    ...
    fn zsh_bootstrap(shell_path: &str) -> String {
        ...
        out.push_str("__agentty_d=${TMPDIR:-/tmp}/agentty-zdotdir-$$\n");
        out.push_str("command mkdir -p \"$__agentty_d\" 2>/dev/null\n");
        ...
        for (name, contents) in zsh_redirectors() {
            ...
            guard.push_str(&format!("[ -s \"$__agentty_d/{name}\" ] && "));
        }
        out.push_str(&format!(
            "{guard}export ZDOTDIR=\"$__agentty_d\" AGENTTY_RM_DIR=\"$__agentty_d\"\n"
        ));
    ...
    fn bash_bootstrap(shell_path: &str) -> String {
        ...
        let mut out = String::new();
        out.push_str("__agentty_d=${TMPDIR:-/tmp}/agentty-bashrc-$$\n");
        out.push_str("command mkdir -p \"$__agentty_d\" 2>/dev/null\n");
        write_file(
            &mut out,
            "bashrc",
        ...
        out.push_str("if [ -s \"$__agentty_d/bashrc\" ]; then\n");
        out.push_str("export AGENTTY_RM_DIR=\"$__agentty_d\"\n");
        out.push_str(&format!(
            "exec {quoted} --rcfile \"$__agentty_d/bashrc\" -i\n"
        ));
        out.push_str("fi\n");
...
# --- agentty remote cleanup (zsh) ---
if [[ -n "$AGENTTY_RM_DIR" ]]; then
  typeset -g __agentty_rm_dir=$AGENTTY_RM_DIR
  unset AGENTTY_RM_DIR
  __agentty_rm_rcdir() {
    [[ -n "$__agentty_rm_dir" ]] || return 0
    command rm -rf -- "$__agentty_rm_dir"
    unset __agentty_rm_dir
  }
  autoload -Uz add-zsh-hook
  add-zsh-hook precmd __agentty_rm_rcdir
fi
# --- end agentty remote cleanup ---
"#;

    const BASH_CLEANUP_HOOK: &str = r#"
# --- agentty remote cleanup (bash) ---
if [[ -n "$AGENTTY_RM_DIR" ]]; then
  __agentty_rm_dir=$AGENTTY_RM_DIR
  unset AGENTTY_RM_DIR
  __agentty_rm_rcdir() {
    [[ -n "$__agentty_rm_dir" ]] || return 0
    command rm -rf -- "$__agentty_rm_dir"
  ...
  precmd_functions+=(__agentty_rm_rcdir)
fi
# --- end agentty remote cleanup ---
"#;
    ...
    mod tests {
        ...
        fn probe_declines_anything_that_isnt_a_shell_we_know() {
            assert_eq!(parse_probe("__agentty_shell\n$SHELL\n"), None);
            assert_eq!(parse_probe("__agentty_shell\n\n"), None);
            assert_eq!(parse_probe("__agentty_shell\n/bin/ksh\n"), None);
        ...
        fn zsh_bootstrap_gates_zdotdir_on_every_redirector_landing() {
            ...
            for name in [".zshenv", ".zprofile", ".zshrc", ".zlogin"] {
                assert!(
                    script.contains(&format!("[ -s \"$__agentty_d/{name}\" ] &&")),
            ...
            assert!(export < exec);
            assert!(script.contains("__agentty_report_cwd"));
        }
        ...
        fn bash_bootstrap_forces_a_non_login_shell_through_the_rcfile() {
            ...
            assert!(script.contains("exec '/bin/bash' --rcfile \"$__agentty_d/bashrc\" -i"));
...
mod tests {
    ...
    fn is_our_zdotdir_matches_only_our_prefix() {
        assert!(is_our_zdotdir("/tmp/agentty-zdotdir-1234-0"));
        assert!(is_our_zdotdir("agentty-zdotdir-x"));
        assert!(!is_our_zdotdir("/home/alice/.config/zsh"));
        assert!(!is_our_zdotdir("/tmp/other-zdotdir"));
        assert!(!is_our_zdotdir(""));
        assert!(!is_our_zdotdir("/tmp/not-agentty-zdotdir-1"));
    }
    ...
    fn wsl_setup_never_contacts_the_distro() {
        let args: Vec<String> = vec![
            "--distribution".into(),
            "agentty-no-such-distro-exists".into(),
            "--cd".into(),
            "~".into(),
        ];
        ...
        assert_eq!(
            ...
            &[
                ...
                "agentty-no-such-distro-exists".to_string(),
                "--cd".to_string(),
                "~".to_string()
            ]
        );
    ...
    fn wslenv_preserves_the_users_own_entries() {
        assert_eq!(
            wslenv_with(Some("MYVAR/p:OTHER"), &["AGENTTY_RC/p"]),
            "MYVAR/p:OTHER:AGENTTY_RC/p"
        );
        assert_eq!(wslenv_with(None, &["AGENTTY_RC/p"]), "AGENTTY_RC/p");
        assert_eq!(wslenv_with(Some(""), &["AGENTTY_RC/p"]), "AGENTTY_RC/p");
        assert_eq!(
            wslenv_with(Some("AGENTTY_RC/l"), &["AGENTTY_RC/p"]),
            "AGENTTY_RC/l"
        );
    }
    ...
    fn bash_rcfile_path_uses_forward_slashes_on_windows() {
        let rendered = bash_path(Path::new(
            r"C:\Users\a\AppData\Local\Temp\agentty-bashrc-1-0\bashrc",
        ));
        if cfg!(windows) {
            assert_eq!(
                rendered,
                "C:/Users/a/AppData/Local/Temp/agentty-bashrc-1-0/bashrc"
            );
        }
        assert_eq!(
            bash_path(Path::new("/tmp/agentty-bashrc-1-0/bashrc")),
            "/tmp/agentty-bashrc-1-0/bashrc"
        );
    }
    ...
    fn zsh_redirectors_source_user_files_and_append_integration() {
        ...
        for (name, body) in &files {
            assert!(
                body.contains("$AGENTTY_USER_ZDOTDIR"),
        ...
        let zshrc = &files[2].1;
        assert!(zshrc.contains("__agentty_precmd"));
        assert!(zshrc.contains("133;A"));
        assert!(!files[0].1.contains("__agentty_precmd"));
    }
    ...
    fn zshenv_recaptures_a_user_relocated_zdotdir() {
        ...
        let recapture = zshenv
            .find("export AGENTTY_USER_ZDOTDIR=${ZDOTDIR:-$HOME}")
            .unwrap();
        let restore = zshenv.rfind("ZDOTDIR=$__agentty_ztmp").unwrap();
        assert!(
        ...
        for (name, body) in &files[1..] {
            assert!(
                !body.contains("export AGENTTY_USER_ZDOTDIR"),
                "{name} must not re-export AGENTTY_USER_ZDOTDIR"
            );
        }
    }
    ...
    fn zsh_integration_restores_real_zdotdir_after_startup() {
        assert!(ZSH_INTEGRATION.contains("__agentty_restore_zdotdir"));
        assert!(ZSH_INTEGRATION.contains("ZDOTDIR=${AGENTTY_USER_ZDOTDIR:-$HOME}"));
        assert!(
            ZSH_INTEGRATION.contains("add-zsh-hook -d precmd __agentty_restore_zdotdir"),
    ...
    fn every_integration_guards_install_on_empty_sentinel() {
        for (shell, body) in [
            ...
            assert!(
                body.contains(r#"-z "$AGENTTY_SHELL_INTEGRATION""#),
        ...
        assert!(
            !FISH_INTEGRATION.contains("set -q AGENTTY_SHELL_INTEGRATION"),
    ...
    fn d_emitter_is_prepended_ahead_of_user_precmd_hooks() {
        assert!(
            ZSH_INTEGRATION.contains("precmd_functions=(__agentty_precmd_d $precmd_functions)"),
            "zsh must prepend the D emitter (add-zsh-hook can only append)"
        );
        assert!(
            BASH_INTEGRATION
                .contains(r#"precmd_functions=(__agentty_precmd_d "${precmd_functions[@]}")"#),
    ...
    fn setup_fish_injects_startup_command_without_files() {
        ...
        assert!(inj.args[1].contains("__agentty"));
    ...
    fn setup_dispatches_by_shell_and_sets_sentinel() {
        ...
        assert_eq!(
            inj.env.get("AGENTTY_SHELL_INTEGRATION").map(String::as_str),
            Some("")
        );
        ...
        assert!(inj.env.contains_key("AGENTTY_SHELL_INTEGRATION"));
    ...
    fn powershell_integration_emits_every_osc_133_mark_and_cwd() {
        ...
        assert!(s.contains("if (-not $env:AGENTTY_SHELL_INTEGRATION)"));
        ...
        assert!(s.contains("$global:__AgenttyOrigPrompt = $function:prompt"));
        assert!(s.contains("& $global:__AgenttyOrigPrompt"));
    ...
    fn throwaway_dir_is_unique_per_call() {
        let a = throwaway_dir("agentty-test-").expect("dir a");
        let b = throwaway_dir("agentty-test-").expect("dir b");
        assert_ne!(a, b);
        assert!(
            a.file_name()
                .unwrap()
                .to_string_lossy()
                .starts_with("agentty-test-")
        );
