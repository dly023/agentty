...
#[test]
fn protocol_reports_compiled_source_identity_without_starting_daemon() {
    use tty7_core::daemon::install::{RemoteProtocol, build_identity, client_version};
    ...
    let identity = RemoteProtocol::parse(&stdout).unwrap();
    assert_eq!(identity, RemoteProtocol::of_this_build());
    assert_eq!(identity.build, build_identity());
    assert_ne!(identity.build, client_version());
