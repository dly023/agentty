...
fn a_routed_connection_reaches_a_real_server() {
    ...
    assert!(
        args.windows(2)
            .any(|pair| pair == ["--config-dir", config_path.as_ref()]),
        "refuse to launch routed fixture without its private config directory"
    );
    let fixture = tty7_core::core::machine::MachineStore::open(dir.path().join("machine.json"));
    let expected = fixture
        .workspace_create(None, Some("router fixture".into()), None)
        .unwrap();
    fixture.flush();
    assert!(dir.path().join("machine.json").is_file());
    ...
    let header = RouteHeader::local_stdio(EXE, &args);
    ...
    let tty7_core::daemon::control::ReplyOk::MachineTree(machine) = host
        .client()
        .call(tty7_core::daemon::control::ControlRequest::MachineGet)
        .unwrap()
    else {
        panic!("expected the isolated fixture tree");
    };
    assert_eq!(machine.workspaces.len(), 1);
    assert_eq!(machine.workspaces[0].id, expected.id);
    assert_eq!(
        machine.workspaces[0].name.as_deref(),
        Some("router fixture")
    );
