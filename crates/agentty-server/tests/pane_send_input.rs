...
use agentty_core::daemon::protocol::{DaemonMsg, ShellSpec, WinSize};
    ...
    fn await_ready(&self) {
        ...
        loop {
            ...
            assert!(
                ...
                "agentty-server did not open its pane endpoint within {READY_WITHIN:?}"
            );
...
fn collect_until(session: &mut agentty_core::client::PaneSession, marker: &[u8]) -> Vec<u8> {
...
fn send_input_reaches_the_shell_without_displacing_the_controller() {
    ...
    panes
        .send_input(pane_id, b"echo agentty_send_oneshot\r")
        .expect("one-shot input is acknowledged");
    collect_until(&mut session, b"agentty_send_oneshot");

    session
        .input(b"echo agentty_still_controller\r")
        .expect("the controller keeps its seat");
    collect_until(&mut session, b"agentty_still_controller");
...
fn a_preempting_attach_closes_the_displaced_controller_and_drops_its_input() {
    ...
    first
        ...
        .expect("bound the stream reads");
    first
        .input(b"echo agentty_first_seated\r")
        .expect("the first controller types");
    collect_until(&mut first, b"agentty_first_seated");
    ...
    let _ = first.input(b"echo agentty_stale_input\r");
    ...
    second
        .input(b"echo agentty_second_alive\r")
        .expect("the new controller types");
    let seen = collect_until(&mut second, b"agentty_second_alive");
    assert!(
        !windows_contain(&seen, b"agentty_stale_input"),
