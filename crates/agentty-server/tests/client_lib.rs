...
use agentty_core::core::machine::{LayoutDelta, PaneSeed};
use agentty_core::daemon::control::{ControlEvent, ControlHello, ControlRequest, ReplyOk, feature};
use agentty_core::daemon::protocol::{DaemonMsg, PROTOCOL_VERSION, ShellSpec, WinSize};
...
impl Daemon {
    fn start() -> Daemon {
        ...
        let child = Command::new(env!("CARGO_BIN_EXE_agentty-server"))
            .arg("--daemon")
            .arg("--config-dir")
            .arg(dir.path())
            .env("AGENTTY_DATA_DIR", dir.path())
            .env("AGENTTY_CONTROL_SOCK", dir.path().join("control.sock"))
            .stdin(Stdio::null())
            .stdout(Stdio::null())
            .stderr(Stdio::null())
            .spawn()
            .expect("start agentty-server --daemon");
    ...
    fn await_ready(&self) {
        ...
        loop {
            ...
            assert!(
                ...
                "agentty-server did not open its endpoints within {READY_WITHIN:?}"
            );
...
fn collect_until(
    session: &mut agentty_core::client::PaneSession,
    marker: &[u8],
...
fn drain_until_exit(session: &mut agentty_core::client::PaneSession) -> Vec<u8> {
    let mut seen: Vec<u8> = Vec::new();
    loop {
        match session.recv() {
            Ok(DaemonMsg::Output(bytes)) | Ok(DaemonMsg::Snapshot(bytes)) => {
...
fn a_spawned_pane_streams_its_output_and_its_exit() {
    ...
    let mut session = daemon
        ...
        .spawn(
            None,
            size(),
            Some(one_shot_shell("echo agentty_pane_roundtrip")),
    ...
    assert!(
        windows_contain(&seen, b"agentty_pane_roundtrip"),
        "output was {:?}",
        String::from_utf8_lossy(&seen)
    );
}
...
fn input_reaches_the_shell_and_a_reattach_replays_it() {
    ...
    session
        .input(b"echo agentty_attach_replay\r")
        .expect("type into the pane");
    let (_, exit) = collect_until(&mut session, b"agentty_attach_replay");
    ...
    let (_, exit) = collect_until(&mut reattached, b"agentty_attach_replay");
