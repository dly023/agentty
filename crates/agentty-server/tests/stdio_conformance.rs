...
use agentty_core::daemon::control::{ControlHello, LinkShutdown};
use agentty_core::host::SharedHost;
use agentty_core::host::conformance::Sandbox;
...
fn stdio_host() -> (SharedHost, TempSandbox) {
    ...
    let mut child = Command::new(env!("CARGO_BIN_EXE_agentty-server"))
        .args(["--stdio", "--serve"])
        .env("AGENTTY_DATA_DIR", sandbox.path())
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::null())
        .spawn()
        .expect("could not start agentty-server --stdio");
    ...
    let host = RemoteHost::connect_with(stdout, stdin, Some(closer), "stdio:conformance", &hello)
        .expect("handshake with agentty-server --stdio");

    (host.into_shared(), sandbox)
}

agentty_core::host_conformance_suite!(remote_stdio, stdio_host);

#[test]
fn the_whole_suite_ran_against_the_server() {
    let names: Vec<&str> = agentty_core::host::conformance::CASES
        .iter()
        .map(|(n, _)| *n)
        .collect();
    assert!(
